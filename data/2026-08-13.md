...
TL;DR: StateFlow是一个以显式可编辑3D世界为核心的状态中心生成式预可视化框架。区别于现有的一次性图像/视频合成方法，它通过持久化的结构化3D状态组织场景元素、演化和相机配置，将生成式预可视化分为状态构建、状态演化、状态访问三个阶段，实现更强的可控性和迭代编辑能力。


<details>
  <summary>Details</summary>
Motivation: 现有生成式预可视化方法依赖简单提示词通过一次性图像或视频合成来联合控制场景、动作、相机和时空动态，存在可控性弱、缺乏迭代编辑支持的问题。论文认为根本原因在于缺少一个显式且持久的工作状态——世界的本质是多个具有几何、外观等属性的元素与相机的组合，不同帧通过对共享状态进行局部修改或重组产生。

Method: StateFlow包含三个阶段：(1) 状态构建：通过先验引导、冲突感知的双视图初始化，将生成的2D内容提升为一致的3D世界；(2) 状态演化：将用户意图转化为结构化状态转换，同时保留世界记忆，避免每次编辑都重新生成完整场景；(3) 状态访问：使用渲染反馈的反思机制将相机计划精炼为视觉上可行的轨迹，不单纯依赖VLM语义判断。离线视频模型可在需要更高保真度时增强视觉质量。

Result: 实验表明StateFlow能够为视频创作和类游戏原型设计生成高质量3D世界。

Conclusion: 论文验证了以显式持久3D状态作为工作表示的状态中心框架对于生成式预可视化是有效的，该框架在保证高质量视觉输出的同时提供了更强的可控性和灵活的迭代编辑能力，弥补了现有生成方法在预可视化场景中的不足。
...
TL;DR: 提出AVA-Encoder框架，通过代理自动编码将视频转换为知识图谱表示再重建回视频，利用文本梯度优化策略学习电影级视频表征，使创作智能体能够从人类高质量影片中学习。


<details>
  <summary>Details</summary>
Motivation: 创作型智能体缺乏从高质量人类电影中学习的有效途径，根本原因在于缺少一种既忠实于电影内容又可直接用于智能体推理与操作的视频结构化表示。

Method: 采用代理自动编码范式：将视频编码为知识图谱表示（包含层次结构节点、状态节点、资产层及类型化边），再将其重建为视频。重建差异驱动文本梯度优化框架，在外循环中进行数据无关的编码策略伪训练，在内循环（测试时）进行可选的依赖数据的知识图谱表示精炼。

Result: 相比最强外部基线提升20.7个百分点；在受控的纯策略设置中，伪训练的镜头级编码策略优于人工调优策略，同时系统提示词token减少74.3%。发布了完整框架、可靠的代理视频重建基准以及首个高质量电影知识图谱表示数据集。

Conclusion: AVA-Encoder为创作智能体提供了一种有效的代理原生视频学习方案，验证了知识图谱视频表示与文本梯度优化相结合的训练范式在电影级视频生成领域的可行性与优越性。
...
TL;DR: 本文提出"Agentic Self-Improvement"框架，将黑盒文本到视频（I2V）生成重构为闭环的目标导向优化过程。该框架采用两阶段方法：第一阶段通过多模态大语言模型（mLLM）进行迭代提示词优化，利用Davidson场景图（DSG）查询确保语义一致性并检测伪影；第二阶段使用贝叶斯优化联合优化随机种子和CFG尺度，由包括新颖的Video-Text Adherence (VTA)分数在内的质量指标引导。在人类偏好研究中，该方法相比基线实现高达69%的胜率。


<details>
  <summary>Details</summary>
Motivation: 现有黑盒I2V模型虽然生成能力强，但缺乏细粒度控制和可靠性，微小的提示词或超参数变化会导致截然不同的输出，迫使专业人员依赖低效的暴力试错过程，难以满足专业工作流的要求。

Method: 提出Agentic Self-Improvement闭环框架，分为两个阶段：(1) 迭代提示词优化阶段，利用mLLM结合Davidson Scene Graph (DSG)查询和Common Mistake Questions (CMQ)两种自动化评估来精炼输入提示；(2) 贝叶斯优化阶段，协同优化随机种子和CFG缩放参数，由包括新提出的Video-Text Adherence (VTA)分数在内的质量指标套件引导搜索。

Result: 该框架显著优于无引导的搜索方法，在人类偏好研究中，智能体方法生成的视频对比基线输出获得强烈偏好，胜率高达69%。

Conclusion: 该工作为增强最先进视频生成模型的可预测性和可控性提供了实用且可扩展的方法论，推动该领域从投机性的技术好奇向可靠的生产级工具演进。
...
### [4] [Few-Shot Ordinal Learning for Day-Wise Freshness Estimation with Hyperspectral Fish Images](https://arxiv.org/abs/2608.12230v1)
*Kazi Nabiul Alam,Pooneh Bagheri Zadeh,Akbar Sheikh-Akbari*

Main category: cs.CV

TL;DR: 本文提出首个用于高光谱成像（HSI）食品质量估计的少样本学习框架。每个鱼片定义为一个独立的情节任务，采用CORAL风格的有序预测头通过累积阈值建模捕捉新鲜度进展的排序特性，并利用生物学启发的单调性与嵌入平滑性约束引导预测。在16天三文鱼HSI数据集上，严格的未见鱼片协议下，仅需每鱼片三个标注日即可达到1.58天的平均绝对误差和72.3%的两天准确率。


<details>
  <summary>Details</summary>
Motivation: 现有基于HSI的新鲜度预测深度学习方法全部依赖全监督训练，需要大量逐产品级别的密集标注数据，成本高昂。同时，不同鱼片间差异大且每个产品的标注数据稀少，导致按天估计新鲜度仍具挑战性。

Method: 1）构建少样本学习框架，将每个鱼片定义为独立的情节任务（episodic task）；2）采用CORAL风格的有序预测头，通过累积阈值建模（cumulative threshold modelling）捕捉新鲜度进展的排序（ranked）属性；3）引入生物学启发的单调性约束和嵌入平滑性约束，引导预测朝向合理的新鲜度变化轨迹。

Result: 在16天三文鱼HSI数据集上，采用严格的未见鱼片（unseen-fillet）协议，该方法以每鱼片仅三个标注日实现1.58天的平均绝对误差（MAE）和72.3%的两天准确率，显著优于相同协议下的标量回归和标签分布基线方法。

Conclusion: 该工作证明少样本学习结合有序回归策略能够有效应对HSI食品质量评估中的数据稀缺与样本间差异大的挑战，为高光谱食品新鲜度预测提供了一种低成本、高精度的可行方案。
...
### [5] [SCOUT: Unlocking Enhanced Spatial Reasoning via Structured Chain-of-Thought and Multi-Objective Process Reward](https://arxiv.org/abs/2608.12220v1)
*Zile Zhou,Huining Yuan,Weichen Zhang,Xinlei Chen,Xiao-ping Zhang*

Main category: cs.CV

TL;DR: 本文提出SCOUT框架，通过融合结构化的3D环境感知思维链（CoT）与多目标过程奖励的强化学习训练，解决了VLM在空间推理中的两大瓶颈：奖励稀疏/信用分配不足，以及缺乏深度感知的结构化推理。SCOUT-3B在通用空间基准上提升16.85%，SCOUT-7B超越GPT-4o达4.28%，且仅用单图像训练即展现出对多图像和视频场景的跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型（VLMs）在空间推理方面存在关键瓶颈。一方面，基于强化学习（RL）的方法虽可利用可验证结果缩小差距，但面临中间推理步骤间信用分配差的问题；另一方面，结构化推理方法忽略了三维理解所需的深度感知能力。这两大不足共同制约了VLM实现全面的3D空间理解与推理。

Method: 1) 设计结构化CoT框架，显式建模3D环境感知以保证空间理解与推理的鲁棒性；2) 提出新型RL算法，包含多目标过程奖励(multi-objective process rewards)与定制化的优势估计方法，实现推理轨迹不同段落间的细粒度信用分配；3) 构建SCOUT-24k数据集，通过定制化pipeline合成结构化空间推理CoT数据用于训练。

Result: SCOUT-3B在通用空间基准和复杂空间推理任务上分别比基线模型提升16.85%和6.3%；更大规模的SCOUT-7B超越GPT-4o达4.28%的绝对优势；尽管仅以单图像训练，SCOUT-7B仍展现了对多图像和视频场景的稳健的域外泛化能力。

Conclusion: SCOUT通过将结构化3D感知思维链与过程监督强化学习相结合，有效克服了VLM空间推理中信用分配和深度感知两方面的缺陷，实验结果表明该方法显著提升了空间推理性能，并具备良好的跨模态跨场景泛化能力，为下一代具备空间感知能力的VLM提供了关键一步。
...
TL;DR: GAS：一个生成引导的训练框架，将视觉生成重新诠释为表示学习的辅助监督，通过解耦的Mixture-of-Transformers架构和Next Embedding Prediction范式，在不引入推理开销的前提下提升多模态大语言模型的感知与空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型（MLLMs）中视觉理解与视觉生成通常被视为发散的目标，现有统一框架依赖离散视觉tokenization或扩散目标，其生成目标与理解模型所用的连续表示不一致，导致无法直接迁移以增强已有的预训练MLLMs。

Method: 提出GAS框架，将Next Embedding Prediction（NEP）作为跨模态生成范式，融入解耦的Mixture-of-Transformers（MoT）架构；通过共享下层主干和并行上层，使生成损失丰富共享视觉通路（获得更精细的空间精度与更强的视觉保留），同时屏蔽上层理解层免受直接生成梯度的干扰；并构造需要深度认知基础的强相关生成任务来最大化协同效应。

Result: 在多种模型规模和训练阶段下，GAS提升了整体多模态理解能力，尤其在感知和空间理解上有最可靠的增益；生成分支在训练后被丢弃，因此零推理开销；通过受控对比实验和表示层面分析阐明了生成引导训练何时以及为何有益于理解。

Conclusion: 生成引导训练可作为增强多模态理解的一条实用路径，在无推理成本的前提下显著提升感知与空间理解能力。
...
### [7] [GeoFlow: Efficient Driving Video Generation via Geometry-Aligned Priors](https://arxiv.org/abs/2608.12203v1)
*Jiazheng Liu,Hang Li,Jiawei Zhang,Jiahe Li,Xiaohan Yu,Shengyin Fan,Jin Zheng,Xiao Bai*

Main category: cs.CV

TL;DR: GeoFlow提出利用显式几何先验（Geometry-Aligned Prior, GAP）替代标准高斯分布作为扩散/流匹配模型的初始化分布，通过多视图几何和空间自适应噪声注入构建对齐先验，显著缩短采样轨迹，实现高效驾驶视频生成。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型和流匹配模型虽能生成高质量驾驶视频，但推理延迟高。作者指出其低效根源在于使用标准高斯源分布，导致连续帧被初始化为独立高斯噪声，忽略了驾驶视频固有的时空相关性，迫使模型从噪声中重新生成已有场景结构，既计算冗余又易产生几何不一致性。

Method: GeoFlow框架利用多视图几何和空间自适应噪声注入构建Geometry-Aligned Prior (GAP)分布作为采样起点，取代标准高斯噪声初始化。该先验分布弥合了源分布与数据分布之间的差距，从而产生更直、更短的采样轨迹。

Result: 实验表明GeoFlow在训练和推理效率上均有显著提升：仅需数小时微调基线模型即可大幅提升少步生成质量；完全收敛的训练可将SOTA视频生成所需的推理步数大幅减少。

Conclusion: 通过显式几何先验缩小源分布与数据分布的距离，能够从根本上缓解扩散模型在驾驶视频生成中的采样效率瓶颈，实现更高效的训练与推理。
...
### [8] [HSTGFormer: Hyper Spatial-Temporal Graph Transformer for 3D Human Pose Estimation](https://arxiv.org/abs/2608.12187v1)
*Ruochen Li,Shuang Chen,Wenke E,Farshad Arvin,Amir Atapour-Abarghouei*

Main category: cs.CV

TL;DR: 本文提出HSTGFormer，一种图增强的Transformer框架用于单目3D人体姿态估计。它通过超时空图(HSTG)将全局时空推理分解为关节-时间节点周围的局部时空感受野，并结合自适应双尺度时间图(ADSTG)捕获短长窗口的关节特定时间依赖，最后通过轻量节点级融合模块进行整合。在Human3.6M和MPI-INF-3DHP上取得高精度和高计算效率。


<details>
  <summary>Details</summary>
Motivation: 现有Transformer方法将空间和时间推理分为独立阶段，这削弱了人体运动中固有的统一时空相互依赖性，并在时间建模之前压缩了帧级结构信息。

Method: 1) 超时空图(HSTG)：将每帧骨架图扩展到时间邻域，将全局时空推理分解为各关节-时间节点周围的局部时空感受野，实现结构感知的耦合推理并保留局部结构运动信息；2) 自适应双尺度时间图(ADSTG)：在互补的短窗口和长窗口上捕获关节特定的时间依赖；3) 轻量节点级融合模块：自适应地将两种图表示整合到每个关节-时间节点。

Result: 在Human3.6M和MPI-INF-3DHP基准上，HSTGFormer取得了强准确的性能，同时保持高计算效率。

Conclusion: 将空间-时间推理重构为关节-时间节点上的局部耦合图聚合，能够更有效地建模统一时空依赖关系，在保持高精度的同时降低计算开销，验证了图增强Transformer在3D人体姿态估计中的有效性。
...
### [9] [Map-Det3D: Metric Feed-Forward 3D Reconstruction Prior for Multi-view 3D Object Detection from Streaming Inputs](https://arxiv.org/abs/2608.12179v1)
...
TL;DR: 提出Map-Det3D，一种在线多视角3D目标检测模型，通过将短时间窗口的RGB帧映射到多视角，利用前馈度量3D重建模型作为几何骨干，直接在度量3D空间中预测边界框，无需传统的2D-to-3D提升，在多个基准上展现强在线性能和鲁棒域迁移能力。


<details>
  <summary>Details</summary>
Motivation: 单体3D检测因单一图像中深度和绝对尺度欠约束而面临根本性困难，现有"2D检测+3D属性预测"范式脆弱：测距误差主导3D定位，且学习到的尺度先验在相机、运动或环境域偏移时容易失效。需要一个不依赖深度传感器但能在度量3D空间中稳定检测的方案。

Method: Map-Det3D将短视频时间窗口映射为多视角图像，复用前馈度量3D重建模型作为几何骨干并微调其面向对象的能力，在此基础上直接在度量3D空间中预测3D边界框，避免了广泛使用的2D-to-3D提升步骤。

Result: 在多个基准上的实验表明，Map-Det3D在在线性能上表现优异，且无需适配即可实现鲁棒的跨域迁移，证明将重建先验用于检测是可行的路线。

Conclusion: 训练重建先验用于检测，是实现从单目视频进行稳定度量3D检测的实用途径；直接在地图重建的3D空间中检测优于传统的2D检测后再提升3D属性的范式。
...
### [10] [Autonomous Telerehabilitation via Skeletal Motion Prediction and Joint-Level Performance Assessment](https://arxiv.org/abs/2608.12145v1)
*Lara Pereira,João Ruivo Paulo,Pedro Santos,Paulo Peixoto*

Main category: cs.CV

TL;DR: 本文提出了一种远程康复流水线，将基于骨架的运动质量评估与短期运动预测集成到两个模块中，在无标记RGB视频上运行。质量评估模块采用自注意力双向LSTM结合MMD-NCA度量学习，在PROZIS数据集上达到96.45%的均值类别准确率；运动预测模块采用STARS图变换器，在Human3.6M上实现75.8 mm的MPJPE。该系统面向辅助机器人和家庭康复场景，旨在提供无需治疗师持续监督的自主反馈。


<details>
  <summary>Details</summary>
Motivation: 自主康复系统需要不仅能识别人体运动，还能提供结构化反馈以支持用户在没有治疗师持续监督的情况下进行康复训练，因此需要一种将运动质量评估与运动预测相结合的端到端解决方案。

Method: 采用两模块架构：1）自注意力双向LSTM（BiLSTM）结合MMD-NCA度量学习进行运动质量分类；2）基于图的运动预测模块（STARS预测器），通过计算预测姿态与观测姿态之间的逐关节位置误差，生成空间局部化的偏差信号。

Result: 分类器在PROZIS数据集的深蹲序列上达到96.45%的均值类别准确率；STARS预测器在Human3.6M上560ms预测时域内达到75.8 mm的均值MPJPE，在所有预测时域上均优于图和循环基线模型。

Conclusion: 该框架为自主、反馈驱动的远程康复系统迈出了重要一步，面向更可及、可扩展的康复解决方案。端到端集成和临床验证是未来研究的重要方向。
...
### [11] [Better Slots, Better Worlds: Representation Quality & Robustness in Object-Centric World Models](https://arxiv.org/abs/2608.12078v1)
*Shukrullo Nazirjonov,Sai Prasanna,Anna Manasyan,Georg Martius*

Main category: cs.CV

TL;DR: 该论文对以对象为中心的world models（OCWMs）进行了受控研究，探讨其在视觉模型预测控制（MPC）中的表示质量和泛化能力。研究发现：规划成功率与无监督槽位质量指标（FG-ARI、mBO）正相关但存在饱和效应；槽位质量良好时，先验方法依赖的辅助本体感觉输入和掩码归纳偏置不再必要；在分布偏移下，槽位良好的OCWM整体鲁棒性优于端到端训练的scene-centric模型LeWM，而基于冻结预训练特征的DINO-WM仍具竞争力，表明预训练特征是鲁棒性的关键贡献者。


<details>
  <summary>Details</summary>
Motivation: 尽管面向对象的world models（OCWMs）被提出作为提高样本效率和泛化能力的归纳偏置，但先前的OCWMs将槽位编码器视为给定模块且仅在分布内评估，未能验证对象中心归纳偏置是否真正有益于规划决策，以及OcWM中究竟是哪一部分驱动了其表现。本文旨在填补这一空白，通过受控实验系统评估OCWMs的优势来源。

Method: 作者沿两个维度对OCWMs进行受控研究：（1）对象中心表示质量，使用无监督槽位质量指标（FG-ARI、mBO）与规划成功率进行相关性分析；（2）分布偏移下的泛化能力，与scene-centric模型（LeWM）及基于冻结预训练特征的DINO-WM进行对比。同时通过逐步去除辅助输入（本体感觉）和掩码归纳偏置来消融验证其必要性。

Result: 研究发现：（i）规划成功率与FG-ARI、mBO等无监督槽位质量指标呈正相关，但在高槽位质量下收益趋于饱和；（ii）当槽位绑定良好时，辅助本体感觉输入和掩码归纳偏置不再是必要的；（iii）在未见过的分布移位下，具有良好槽位绑定的OCWM整体上比端到端训练的LeWM更鲁棒，而DINO-WM（基于相似的冻结预训练特征）与其竞争相当，提示预训练特征对鲁棒性有重要贡献。

Conclusion: 对象中心表示的质量（而非辅助机制）是驱动OCWM规划性能的关键因素；良好的槽位质量足以弥补额外的归纳偏置需求；预训练特征在分布偏移下的鲁棒性中扮演重要角色。该工作为OCWMs的设计提供了可操作的指导，明确了对象中心归纳偏置在规划中的实际价值及其局限。
...
### [12] [Localizing to Debias: A Patch-Level Benchmark and Baseline for Weakly Supervised Spatial Anomaly Detection](https://arxiv.org/abs/2608.12045v1)
*Sara Abdulaziz,Abdulrahman Al-Abri,Giacomo D'Amicantonio,Egor Bondarev*

Main category: cs.CV

TL;DR: 本文提出SST-WSVADL，一个稀疏时空框架，通过动态稀疏化逐步聚焦异常相关时空区域，抑制背景主导内容，同时发布帧级空间标注和评估协议，用于审计WSVAD模型的场景偏差。


<details>
  <summary>Details</summary>
Motivation: 弱监督视频异常检测存在粗粒度时间监督与细粒度空间推理之间的鸿沟，时间检测器容易依赖背景和场景级线索而非真正的判别性异常证据。这种背景偏差可能将异常与社会或环境背景错误关联，产生伦理问题。由于缺乏空间grounding，这类偏差难以被发现和审计。

Method: 提出SST-WSVADL稀疏时空框架：(1) 通过动态稀疏化逐步聚焦最异常相关的时空区域，抑制背景主导内容；(2) 时间与空间分支通过运动感知正则化端到端耦合，引导稀疏化指向动态信息丰富的区域；(3) 不依赖外部检测器或视觉语言提示；(4) 发布三个数据集（UCF-Crime, XD-Violence, MSAD）的帧级空间标注及方法无关的评估协议。

Result: 实验表明SST-WSVADL在多个基准上与现有方法表现相当，同时实现了定位能力和patch级场景偏差可审计性，为WSVAD模型的可解释性评估提供了可复现基础。

Conclusion: 该工作不仅提升了弱监督视频异常检测的性能，更通过空间标注资源和评估协议推动了模型偏差的可审计性，为更伦理和负责任的异常检测铺平了道路。
...
### [13] [Auditing Frame-Level AUC in Weakly Supervised Video Anomaly Detection: Granularity, Resolution, and Scene Bias](https://arxiv.org/abs/2608.11985v1)
*Sara Abdulaziz,Egor Bondarev*

Main category: cs.CV

TL;DR: 本文批判性地审计了弱监督视频异常检测（WSVAD）中广泛使用的池化AUC评估协议，指出其未能可靠地反映帧级（视频内）异常定位性能。作者在UCF-Crime基准上对多种SOTA模型进行了三项粒度的配对比较（全局、按异常类别、视频内），并通过对数级自举统计证明：池化AUC在测试集规模下缺乏分辨率以支撑所报告的性能差距；视频内AUC能在相同预测上区分更多比较；模型的表示中视频内异常结构与检测器定位是解耦的；此外所有模型在正常片段上都能根据录制属性（分辨率、色彩编码）区分视频，表明场景敏感性是该设置下的共性而非架构特性。作者发布了粒度感知的评估协议和场景因子标注。


<details>
  <summary>Details</summary>
Motivation: WSVAD领域的标准评估指标——池化AUC——可能掩盖了模型在帧级异常定位上的真实能力差异，因为它在跨视频聚合帧对时既度量了事件定位也度量了视频源之间的差异。现有文献中报告的SOTA差距可能缺乏统计分辨率支撑，需要系统性地审计这一协议。

Method: 在UCF-Crime上对多个SOTA模型（覆盖不同骨干架构）进行审计：固定模型的帧分数，在三种配对粒度（全局/按异常类别/视频内）下重新评估；同时对模型内部表示计算零样本分数执行相同的三粒度读法；使用配对视频自举（paired video bootstrap）评估排序可靠性。

Result: 三个主要发现：（1）池化AUC不能可靠预测视频内异常定位，相似池化分数的模型在更严格粒度下表现出显著定位差异和排名反转；（2）在测试集规模下，池化AUC缺乏分辨率支撑文献中的SOTA差距，骨架家族内无法解析这些边际差异，而视频内AUC能解析多个；（3）所有模型都能依据录制属性（分辨率、色彩编码）在正常片段上分离视频，表明场景敏感性是该设置下的共有属性。

Conclusion: 现有WSVAD评估协议（池化AUC）在帧级异常定位评估上不可靠且缺乏统计分辨率。建议采用粒度感知的评估协议（尤其是视频内AUC），并注意场景因子（如录制属性）对模型评分的系统性影响。作者公开发布了可计算的粒度感知协议及相关场景因子标注，供社区复现和扩展。
...
### [14] [HarmoniDPO: Video-guided Audio Generation via Preference-Optimized Diffusion](https://arxiv.org/abs/2608.11913v1)
*Wenshuo Peng,Kaipeng Zhang*

Main category: cs.CV

TL;DR: HarmoniDPO 是一个面向视频到音频（V2A）生成的新框架，通过引入基于偏好的优化来克服现有方法在时间同步和感知质量上的不足。该框架结合了双视频表征（全局上下文+逐帧特征）保留时间动态与语义细节，采用在线直接偏好优化（online-DPO）微调扩散模型以对齐人类感知偏好，并引入双尺度扩散搜索（DDS）作为测试时缩放算法进一步优化输出保真度。实验表明其超越了现有最优方法。


<details>
  <summary>Details</summary>
Motivation: V2A生成面临两个核心挑战：一是视频输入被压缩为单一特征表示，导致时间动态和细粒度视觉信息的显著丢失；二是基于重建的训练目标与人类对音频质量和适配性的主观感知判断相关性差。现有方法在这两方面均有不足，因此需要新的框架以同时改善时间同步和感知质量。

Method: (1) 采用双视频表征，融合全局上下文与逐帧特征，以同时保留时间动态和语义细节；(2) 受RLHF启发，引入在线直接偏好优化（online-DPO）对基于扩散的V2A模型进行偏好微调，以提升感知质量和视听对齐；(3) 提出双尺度扩散搜索（DDS）作为测试时缩放算法，在推理阶段自适应优化输出保真度。

Result: 实验结果显示，HarmoniDPO在音频-视频同步性和主观音频质量方面均优于现有最先进方法，能够生成更真实、更符合人类偏好的视频伴生音频。

Conclusion: HarmoniDPO通过将偏好优化整合进扩散式V2A生成，有效解决了时间同步和感知质量问题，为从视频生成符合人类偏好且高质量音频提供了稳健的解决方案。
...
### [15] [Warping Earth Observations for better ice labeling in the Marginal Marginal Ice Zone](https://arxiv.org/abs/2608.11883v1)
*Tom Kelly,Martin S. J. Rogers*

Main category: cs.CV

TL;DR: 本文提出了一种基于互信息( mutual information)warping的新型架构，用于对齐多卫星（Sentinel-1和MODIS）多模态（可见光、热红外、雷达）影像，以解决南极边缘冰带中冰体漂移导致的像素级失配问题。作者引入了一个包含2,088个像素级标注的稀疏专家数据集，实验表明在分割前进行空间对齐与模态配准可提升分类精度，并实现从稀疏点级监督中学习稠密海冰分割。


<details>
  <summary>Details</summary>
Motivation: 多模态卫星影像可提供互补的地球观测信息，但在动态环境中（如南极边缘冰带），由于冰体在不同卫星过境之间快速漂移，导致像素级对应关系被破坏，传统多模态推理和分类流程的这一前提假设失效，因此亟需解决时空失配问题。

Method: 提出基于互信息warping的架构对齐多卫星多模态影像（Sentinel-1与MODIS）；引入包含43个场景、2,088个像素级标注（7,046个专家点分类）的稀疏专家标注数据集，标注位于冰-水交界区域；在分割前对模态进行空间对齐与配准。

Result: 空间对齐与模态配准后再进行分割能提升分类精度，并能在仅有点级稀疏监督的条件下实现准确的稠密海冰分割。

Conclusion: 在动态环境下，多模态卫星影像的空间对齐是有效利用多传感器信息的前提；稀疏的点级专家监督结合对齐策略可替代传统依赖粗分辨率冰图的稠密标注，为海冰分割提供更高效、更准确的方案。
...
### [16] [TD-VAD: Breaking Visual Dependence in Video Anomaly Detection with Text-Driven Learning](https://arxiv.org/abs/2608.11820v1)
*Shuangqing Zhang,Lei-Lei Ma,Zhao Wang,Wen Dong,Xinyi Xu,Guo-Sen Xie,Caifeng Shan,Fang Zhao*

Main category: cs.CV

TL;DR: 本文提出TD-VAD方法，利用LLM生成的带时序特征的文本来训练视频异常检测模型，完全摆脱对目标域异常视频数据的依赖，在XD-Violence和UCF-Crime两个大规模数据集上大幅超越以往的一类分类和无监督VAD方法。


<details>
  <summary>Details</summary>
Motivation: 现有视频异常检测方法依赖大量标注的异常视频数据作为训练前提，但异常事件稀有且种类繁多，获取充足且可扩展的标注数据极其困难且成本高昂。相比之下，异常事件的文本描述易于收集且类别标签可直接获得，因此有必要探索以文本替代视频来训练VAD模型的可行性。

Method: 该方法利用LLM生成带有时间特性的视频似文本描述来训练VAD模型，不使用任何目标域异常数据。设计了事件演化因果注意力模块（event evolution causal attention module）来建模跨时间的上下文依赖，以捕捉事件的长程和短程时序逻辑。推理阶段，鉴于文本与视频序列之间的域差距，使用冻结的CLIP编码器提取视频帧嵌入以对齐文本模态，同时保留关键的视觉信息。

Result: 在两个大规模VAD数据集XD-Violence和UCF-Crime上的综合实验表明，该方法在性能上大幅超过先前的一类分类（one-class）和无监督（unsupervised）VAD方法。

Conclusion: 本文将视频异常检测的训练数据从视频模态拓展到文本模态，验证了文本驱动VAD的可行性和有效性，为缓解异常数据稀缺问题提供了新的思路。
...
### [17] [VOLA: Improving Open-World Driving by VLM-Based Semantic Attribute Prediction](https://arxiv.org/abs/2608.11777v1)
*Yuchen Zhang,Yuan Gao,Sebastian Schmidt,Johannes Betz*

Main category: cs.CV

TL;DR: 本文提出将场景感知从类别标签转向稠密的动作相关属性（可行驶性与脆弱性），利用VLM图像token的隐藏状态直接作为空间语义表征，通过轻量级边界感知解码器生成高分辨率属性图，无需自回归文本生成或外部掩码模型。在CARLA中训练并迁移至真实场景和未见过的障碍物，在脆弱性等级召回率上达到69.4%，优于纯视觉分割器和提示VLM分割器。


<details>
  <summary>Details</summary>
Motivation: 真实世界的驾驶是开放世界的，车辆可能遇到训练数据之外的物体（如掉落的床垫、鹿等）。仅对物体命名不够，系统必须知道每个区域应如何处理：能否驶过，碰撞严重程度如何。传统基于类别标签的场景感知无法应对开放世界中的未知物体。

Method: 将场景感知从类别标签重新定义为稠密动作相关属性，每个像素标注其对运动的影响而非物体名称。具体采用两个有序属性：7级可行驶性和5级脆弱性。直接读取Qwen3.5图像token的隐藏状态作为空间语义表征，用轻量级边界感知解码器将粗粒度token网格转换为高分辨率属性图。整个流程无需自回归文本生成或SAM等外部掩码模型。在CARLA中构建稠密属性标签进行训练。

Result: 在熟悉类别上匹配强视觉分割器；迁移到真实开放世界异常时，达到69.4%的平均脆弱性等级召回率，优于最佳纯视觉基线的57.1%和最佳提示VLM基线的53.9%。

Conclusion: VLM图像token能为驾驶属性向训练词汇之外的物体迁移提供有用的语义线索，验证了从类别标签转向动作相关属性表征这一范式的有效性。
...
### [18] [UniSwap: Streaming Audio-Visual Identity Swapping for Talking Videos](https://arxiv.org/abs/2608.11752v1)
*Yuxuan Zhang,Haozhong Xiong,Jiayi Song,Jinpeng Yu,Yang Shi,Jiaming Liu,Ruihua Huang,Liwei Wang*

Main category: cs.CV

TL;DR: UniSwap是首个用于视频中流式联合音视频身份替换的框架，在单一音视频扩散Transformer中同时传递参考外观和声音音色，并通过swap-and-reconstruct管道及多项优化技术实现高效的流式生成和长序列稳定推理。


<details>
  <summary>Details</summary>
Motivation: 现有的说话视频身份替换方法对视觉和语音两个模态使用分别优化独立模型，导致音视频一致性难以保证。此外，跨身份对齐训练对的稀缺是联合训练的主要障碍。

Method: UniSwap引入了swap-and-reconstruct训练管道，从真实剪辑中移除视觉和声音身份并以原剪辑为重建目标；采用双向骨干网络，逐步通过内向预训练(In-context Pretraining)进行联合替换、条件流式适配(Conditional Streaming Adaptation)实现块因果KV缓存生成、高效自强制DMD(Efficient Self-forcing DMD)缓解曝光偏差并将每块去噪步数从30降至3；通过高效多LoRA切换(Efficient Multi-LoRA Switching)使三种DMD角色共享单一冻结骨干，特征RoPE分解(Feature-RoPE Decomposition)保持缓存位置在训练范围内以支持长序列推理。

Result: 实验表明UniSwap在音视频同步、身份保持、流式效率和长序列生成稳定性方面均表现优异。

Conclusion: UniSwap通过统一的音视频扩散Transformer架构和多项针对性的训练与推理优化，有效解决了说话视频中跨模态身份替换的协同问题，实现了高质量的流式联合音视频身份替换。
...
### [19] [STAR: A Spatial-Topology Aware Routing Framework for Generalizable 3D Scene Understanding](https://arxiv.org/abs/2608.11699v1)
*Mingwei Xing,Xinliang Wang,Yifeng Shi*

Main category: cs.CV

TL;DR: 提出STAR（空间-拓扑感知路由框架），一种统一3D场景理解模型，通过多属性自监督预训练分支锚定跨域结构先验，并设计域感知专家分支（包含DSR路由机制和EDA动态分配机制），解决多模态3D理解中MoE架构下特征路由无法表征局部采样拓扑的问题。在ScanNet和S3DIS上分别达到80.1%和77.2% mIoU。


<details>
  <summary>Details</summary>
Motivation: 统一3D场景理解模型长期受限于不同传感器模态间的拓扑差异。虽然MoE架构是处理多域3D理解灵活方案，但传统仅基于特征的路由器在语义监督下可能无法充分表征局部采样拓扑，导致在语义一致性与几何异质性并存时专家分配困难。

Method: STAR包含两个核心分支：(1) 多属性自监督预训练分支，覆盖拓扑和纹理变化，用于锚定跨域结构先验；(2) 域感知专家分支，包含两种机制——Domain-Spatial-Guided Routing (DSR)从空间上下文捕获局部拓扑变化，以及Entropy-controlled Dynamic Allocation (EDA)根据路由不确定性动态调整激活专家数量。

Result: 在多种任务和室内外场景上的大量实验验证了STAR的有效性。在ScanNet验证集上达到80.1% mIoU，在S3DIS上达到77.2% mIoU，均优于强基线。

Conclusion: STAR通过将稳定的跨域表示学习与自适应的专家分配相结合，有效解决了多模态3D场景理解中拓扑差异带来的挑战，并在室内外场景中取得显著性能提升。
...
### [20] [Topology-Aware Query Selection for Surgical Instrument Instance Segmentation](https://arxiv.org/abs/2608.11607v1)
*Ze Zhang,Yang Zhang*

Main category: cs.CV

TL;DR: 该论文提出将最终前景掩码的查询选择视为关系性、可变基数的集合问题，而非独立候选决策。作者将固定Mask2Former的非空候选构建为完整图，学习关系型候选与配对表示，预测集合基数，并求解精确的结构化子集问题。在封闭的22例源测试中，三种发现种子均提升了实例集性能（实例F1提升0.0504–0.0612，正帧集合失败率降低0.0848–0.1060），ROBUST-MIPS直接迁移完全复现了结果，但Endoscapes仅支持三种种子之一，跨域稳定迁移未获确立。


<details>
  <summary>Details</summary>
Motivation: 高精度的前景掩码仍可能产生错误的器械实例集合（重复、碎片化、合并、遗漏或空帧预测），这些错误在保持良好像素重叠的同时违反对象身份和计数约束。最终查询选择本质上是关系性、可变基数的集合问题，而非独立候选决策的简单堆叠，这促使作者探索拓扑感知的查询选择方法。

Method: 将固定Mask2Former的非空候选表示为完整图，学习关系型候选表示和配对表示，预测集合基数，并求解精确的结构化子集问题。形式比较是完整关系路径与节点特征匹配路径之间的对比，评估成对几何、消息传递及关系路径容量的综合效应。

Result: 在封闭的22例源测试上，三种发现种子均支持实例集性能提升，同时保持分割保真度和预定义技术安全性：实例F1提升0.0504–0.0612，正帧集合失败率降低0.0848–0.1060。ROBUST-MIPS直接迁移在三种种子中完全复现结果；Endoscapes仅支持三种种子之一，未建立稳定的直接迁移。

Conclusion: 该完整路径在特定原生实例契约下，从固定Mask2Former候选中改善了连贯实例集合的构建；但跨域稳定迁移及各组件的独立效应尚未确立，因此结论是有限的。
...
### [21] [How Can Driving World Models Do Counterfactual Prediction?](https://arxiv.org/abs/2608.11601v1)
*Jiaru Zhang,Can Cui,Yi Xu,Xin Ye,Ruqi Zhang,Ziran Wang*

Main category: cs.CV

TL;DR: 本文指出驾驶世界模型作为反事实模拟器时存在根本性缺陷：直接动作条件预测只利用共享历史和替代动作，忽略了事实后续延续，因此无法保证生成的反事实结果与真实发生的驾驶事件一致。作者通过因果"溯因-动作-预测"框架形式化这一差距，构建受控仿真基准进行量化验证，并提出一个训练自由的简单管线（将观测证据移入反事实视角，让冻结模型补全未知部分），显著提升了反事实恢复率和感知一致性。


<details>
  <summary>Details</summary>
Motivation: 驾驶世界模型的核心应用场景是对给定驾驶日志进行反事实推演（即"如果将来的动作不同会怎样"），但研究者发现直接的动作条件预测与这一目标存在根本性不匹配——预测器只看到共享历史和替代动作，没有用到事实段落在历史之后实际发生的延续，因此可能生成看似合理但并未发生在该具体驾驶事件中的未来。这一差距从未被系统形式化和量化，作者希望揭示并推动更好的反事实预测方法。

Method: 1) 使用因果"溯因-动作-预测"（abduction-action-prediction）框架形式化直接预测与正确反事实推断之间的差距；2) 在短时间窗口设定下（替代 ego 动作不影响周围智能体演化）构造受控仿真基准，包含事实结果和匹配的反事实结果；3) 在两个代表性世界模型上进行评估，验证直接预测无法匹配反事实基准；4) 提出一个训练自由的简单管线：将观测证据迁移到反事实视角，再让冻结模型补全剩余未知部分，作为建设性的验证。

Result: 在两个代表性世界模型上，直接动作条件预测均未能匹配反事实地面真值，证实了作者的分析。即使采用该简单且训练自由的管线（将观测证据移入反事实视图），也能在两个模型上显著提高整体恢复比例并降低与匹配反事实之间的感知距离。

Conclusion: 直接动作条件预测无法真正实现驾驶世界模型的反事实模拟目标，需要专门关注反事实推断的建模改进；本文的证据迁移式简单管线表明即使不重训模型也能显著改善反事实一致性，希望以此激励更好的反事实预测方法。
...
### [22] [A Hybrid Framework of Vision Transformer and Gated Recurrent Unit for Detection of Mosquito Diseases](https://arxiv.org/abs/2608.11582v1)
*Danial Sharifrazi,Saadat Behzadi,Nouman Javed,Roohallah Alizadehsani,Prasad N. Paradkar,Asim Bhatti*

Main category: cs.CV

TL;DR: 本文提出一个三阶段框架用于从对照蚊子中识别登革热病毒感染蚊子：使用YOLO 11M模型识别蚊子并去除背景，利用Vision Transformer (ViT)提取视觉特征，最后通过卷积GRU (ConvGRU)分类器对视频进行分类。对比RNN、LSTM、GRU及其卷积版本，ConvGRU表现最佳，准确率88.88%，精确率84.45%，召回率82.82%，F1分数82.81%。


<details>
  <summary>Details</summary>
Motivation: 由于蚊子体型小且视频背景复杂，从对照蚊子中识别登革热病毒感染蚊子的运动行为分析面临重大挑战。传统AI方法难以从视频帧中提取准确特征，产生错误特征，因此需要一种更可靠的框架来准确分类。

Method: 提出三阶段框架：(1) 使用YOLO 11M模型识别蚊子并去除视频背景；(2) 利用Vision Transformer (ViT)提取视觉特征；(3) 使用卷积GRU (ConvGRU)分类器对视频进行分类。同时对比了RNN、LSTM、GRU及其卷积变体的性能。

Result: ConvGRU模型在所有对比模型中表现最佳，达到88.88%准确率、84.45%精确率、82.82%召回率和82.81% F1分数。

Conclusion: 将卷积模型与序列网络相结合，尤其是ConvGRU模型，能够同时从蚊子运动中提取精确的空间特征和长期时间依赖关系。该框架为在复杂环境中分析蚊子行为提供了可靠解决方案。
...
### [23] [From Synthesis to Removal: Physics-Grounded Reflection Simulation and Diffusion-Based Video Dereflection](https://arxiv.org/abs/2608.11562v1)
*Zepeng Wang,Jiagao Hu,Fuhao Li,Yuxuan Chen,Fei Wang,Daiguo Zhou*

Main category: cs.CV

TL;DR: 本文提出了一个闭环框架S2R，包含三个部分：S2R-Synthesis用于物理驱动的反射视频仿真与数据生成，S2R-Removal是首个基于扩散模型的视频反射去除模型，S2R-Bench是首个视频反射去除基准评测集。实验表明该方法在多个基准上达到SOTA性能，且推理速度甚至快于非扩散基线。


<details>
  <summary>Details</summary>
Motivation: 视频反射去除长期未被充分探索，主要受限于三个瓶颈：缺乏成对的视频数据（单图反射去除已被广泛研究但视频领域缺少数据集）、缺乏时间一致性的去除模型、缺乏专门的评测基准。现有方法无法有效处理玻璃反射带来的视觉质量下降问题。

Method: 提出闭环框架S2R：(1) S2R-Synthesis：在结构空间进行物理驱动的数据增强，模拟玻璃相关效应（粗糙度导致的模糊、厚度导致的鬼影、反射率变化），并用训练好的视频扩散渲染器生成逼真的成对反射视频；(2) S2R-Removal：首个基于扩散模型的视频反射去除模型，通过反射感知的潜在空间适配和单步像素-几何细化来适应预训练的视频扩散先验，在单次去噪步骤中恢复干净的透射内容；(3) S2R-Bench：首个视频反射去除基准，同时支持全参考评估和真实世界的主观感知评估。

Result: 在S2R-Bench和多个公开图像基准上的实验表明，该方法达到SOTA性能，推理速度甚至快于非扩散基线方法，并验证了S2R-Synthesis数据生成管道的有效性。

Conclusion: 该工作通过统一仿真、去除和评测三个环节，首次系统性地解决了视频反射去除问题，证明了物理驱动的合成数据与扩散模型的结合能够高效且高质量地完成视频去反射任务。
...
### [24] [Generative Semantic Segmentation via an Observable Semantic-Image Interface and Hierarchical Generator Evidence Alignment](https://arxiv.org/abs/2608.11537v1)
*Weize Cai,Yongqi Dong,Zhida Shao,Zixin Fu*

Main category: cs.CV

TL;DR: 本文提出Semantic Prism，一个条件语义图像生成与精炼框架，通过扩散蒸馏的单步生成器渲染语义RGB图像，利用颜色到固定类色码本的距离作为显式概率接口，并通过层级生成器证据对齐（HGEA）在接口logit空间中预测加性残差以精炼分布。此外提出C-IHD固定读出器用于像素错误排名，在Cityscapes、BDD100K、ACDC三个数据集上取得了显著效果。


<details>
  <summary>Details</summary>
Motivation: 生成式语义分割将结构化预测暴露为图像，但直接颜色解码易受颜色漂移和边界混合影响；而潜在特征解码器预测独立输出分布可能将渲染图像降级为中间可视化。现有方法无法同时兼顾生成式渲染的直观性、颜色解码的稳定性以及可靠的概率接口。

Method: 1）使用扩散蒸馏的一步生成器渲染语义RGB图像；2）计算渲染颜色到固定类色码本的逐像素距离，构造显式概率接口；3）提出Hierarchical Generator Evidence Alignment（HGEA），空间对齐多层级生成器特征，用零初始化输出投影在接口logit空间中预测加性残差；4）提出Contextual Interface--Hierarchy Disagreement（C-IHD），一种无需辅助预测器或额外前向传播的固定读出器，用于对剩余像素误差进行排序。

Result: 在Cityscapes 500张验证集上达到72.07% mIoU，比直接接口解码高11.39个mIoU点，E CE为0.41%。BDD100K上单独训练的模型达62.22% mIoU，Cityscapes训练的模型在ACDC源冻结迁移下达到46.89% mIoU。C-IHD在三个数据集上一致优于最大softmax概率的像素误差排名，在ACDC上将AUPR从0.6580提升至0.7557。

Conclusion: Semantic Prism证明了通过保留图像定义的接口作为最终分布参考，同时用对齐的多层级生成器证据精炼logit空间，可以有效兼顾生成式语义分割的直观性与预测精度；C-IHD提供了无需额外训练的可靠的像素级不确定性估计方法。
...
### [25] [Test-Time Hallucination Control in Large Vision-Language Models](https://arxiv.org/abs/2608.11474v1)
*Mehran Tamjidi,Hamidreza Dastmalchi,Ali Cheraghian,Mohammadreza Alimoradijazi,Aijun An,Hossein Rahmani*

Main category: cs.CV

TL;DR: 本文提出了一种名为TTH（Test-Time Hallucination Mitigation）的免训练幻觉缓解方法，通过引入零样本多模态分类器作为token验证器，在推理阶段融合辅助logits来抑制大型视觉语言模型（LVLM）的对象幻觉，兼顾了计算效率与通用性。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型（LVLMs）在生成时会产生对象幻觉（生成与输入图像不符的内容），严重制约其可靠性。现有缓解策略中，基于训练的方法性能好但成本高昂（计算资源、大规模数据、微调耗时）；免训练方法虽然高效，但要么需要多轮解码增加计算开销，要么以模型特定的方式修改内部状态而有损预训练知识。因此亟需一种既能保持效率又能广泛适用的免训练缓解方法。

Method: 提出测试时幻觉缓解（TTH）方法，属于免训练方法。核心组件是一个token验证器，由零样本多模态分类器（MMC）实现，用于生成与输入图像对齐的辅助logits。这些辅助logits在token级别上与原始LVLM输出进行融合（仅针对从候选池中选择的对象token），并通过基于熵的加权方案来确保预测的鲁棒性和准确性。

Result: 在多个LVLM家族和多样化基准上的大量实验表明，TTH方法在准确性和鲁棒性方面均取得了一致的改进，验证了其良好的泛化能力和实际有效性。

Conclusion: TTH作为一种免训练的测试时幻觉缓解方法，通过零样本多模态分类器融合辅助logits并结合熵加权，有效解决了现有免训练方法计算开销大和模型特异性修改的局限，在多个LVLM上展现出通用性与实际应用价值。
...
### [26] [COGENT: Counterfactual Gaussian Explanations for Volumetric Medical Images](https://arxiv.org/abs/2608.11422v1)
...
TL;DR: 提出COGENT框架，在基于高斯的三维体积表示参数空间中生成反事实解释，用于医学体积深度学习模型的可解释性分析。


<details>
  <summary>Details</summary>
Motivation: 现有医学体积成像的可解释性方法主要在体素空间操作，忽略了三维场景建模中引入的结构化表示。需要一种能够利用显式三维场景表示来生成更具解剖学一致性和临床意义的解释方法。

Method: 基于MedGS和Sybil肺癌风险预测模型，通过可微渲染管线优化选定的高斯基元，利用下游预测器的梯度来识别影响模型决策的表示组件。将可解释性定义为显式三维场景表示上的反事实优化问题。

Result: 在肺部CT扫描上，通过定量比较现有可解释性方法和医学专家的定性分析进行评估。结果表明表示空间的反事实优化能提供临床有意义的解释。

Conclusion: 表示空间的反事实优化为解释体积深度学习模型提供了新视角，生成的解释具有稀疏性、空间局部性和解剖一致性，具有临床价值。
...
### [27] [Gaze Target Estimation Anywhere with Concepts](https://arxiv.org/abs/2608.11367v1)
*Xu Cao,Houze Yang,Vipin Gunda,Zhongyi Zhou,Tianyu Xu,Adarsh Kowdle,Inki Kim,James M. Rehg*

Main category: cs.CV

TL;DR: 本文提出了一种全新的、基于概念驱动的"可提示注视目标估计"(PGE)任务范式，通过自然语言或视觉提示指定注视分析对象，端到端地同时完成主体定位、画面内外判断和注视目标热图估计。作者构建了Gaze-Co数据集（120K提示标注图像对）并提出了GazeAnywhere模型，采用基于Transformer的检测器融合冻结编码器特征，在多个PGE基准上取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有注视目标估计方法依赖多阶段、脆弱的流水线（需要头部边界框、人体姿态等显式输入），检测错误会级联放大导致失败；且缺乏通过自然语言提示灵活指定注视分析任务的能第，限制了可扩展性和便捷性。

Method: 1) 定义PGE任务：基于文本或视觉提示（如"穿红衣服的男孩"或指定坐标点）定位注视主体并进行注视目标估计。2) 构建可扩展数据引擎生成Gaze-Co数据集（120K高质量提示标注图像对）。3) 提出GazeAnywhere模型：使用Transformer-based检测器融合冻结的编码器特征，同时解决主体定位、画面内外存在性判断和注视目标热图估计三个子任务。

Result: GazeAnywhere在多个PGE基准上取得SOTA性能，即使在困难的跨域真实临床数据集上也建立了强基线。

Conclusion: GazeAnywhere作为首个PGE模型，验证了概念驱动的注视估计范式的有效性，消除了对中间分析阶段的刚性依赖，并为该新问题设立了强大基线。
...
### [28] [Dual-Domain Cross-Modal Decoding for Clinical Text-Guided Medical Image Segmentation](https://arxiv.org/abs/2608.11335v1)
*Md Maklachur Rahman,Tracy Hammond*

Main category: cs.CV

TL;DR: 本文提出DD-CMD（双域跨模态解码）方法用于临床文本引导的肺部感染分割，在空间域通过文本引导空间交叉注意力（TGSA）对齐多尺度视觉token与文本语义，在频域通过频谱-文本自适应调制（STAM）利用2D DCT计算频带能量统计并预测文本条件FiLM参数进行频感知解码。在QaTa-COV19和MosMedData+数据集上分别达到91.46% Dice/84.26% mIoU和81.95% Dice/69.42% mIoU，平均比最强基线提升+1.96 Dice和+2.67 mIoU。


<details>
  <summary>Details</summary>
Motivation: 现有文本引导的医学图像分割方法主要关注空间对齐，忽视了控制纹理和边界的频率信息，导致分割精度受限。作者旨在将空间域和频域的语言引导整合到解码过程中，以更全面地利用临床文本先验信息。

Method: DD-CMD包含两大核心模块：(1)空间域TGSA，通过文本引导的空间交叉注意力对齐多尺度视觉token与文本语义，并使用门控残差融合更新特征；(2)频域STAM，应用2D DCT计算可学习的频带能量统计，预测文本条件下的FiLM参数以重校准解码器通道。两者嵌入粗到细解码器（7x7到56x56），并用轻量级两阶段细化模块恢复全分辨率掩码。

Result: 在QaTa-COV19上达91.46% Dice/84.26% mIoU，在MosMedData+上达81.95% Dice/69.42% mIoU，平均比最强先验基线提升+1.96 Dice和+2.67 mIoU。代码已开源。

Conclusion: 通过同时利用空间域和频域两种互补的文本引导方式，DD-CMD有效提升了临床文本引导的肺部感染分割性能，验证了频域信息在文本引导分割中的价值，超越了现有强基线方法。
...
### [29] [From Sports to Safety: Benchmarking Proactive Risk Inference in MLLMs](https://arxiv.org/abs/2608.05560v1)
*Jiawei Qiu,Yichen Xu,Jianzhe Ma,Mingyang Yu,Wenbin Zhu,Yang Han,Pinzheng Lv,Wenxuan Wang*

Main category: cs.CV

TL;DR: 本文提出SPRINT基准测试集，包含2,888个真实世界体育视频（2,440个事故视频、448个安全对照视频），覆盖14种运动与3种环境，用于评估多模态大语言模型（MLLM）在物理危险主动预判方面的能力。研究发现当前最先进的MLLM在危险信号感知上可超过95%准确率，但在识别事故原因上低于50%，且显式危险查询会引发严重误报，表明现有模型仅具备表面性的主动安全能力。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评测主要关注有害内容识别或一般性风险评估，缺乏对积极主动物理危险预判的评估。而及时预判物理危险对现实世界安全至关重要，体育视频提供了理想测试场景，因为其事故成因覆盖多种伤害维度，且事故前的时空线索推理能力可与自动驾驶、跌倒检测等更广泛的安全领域共享。

Method: 构建SPRINT基准：收集2,888个真实世界体育视频，包含2,440个事故视频（带有早期危险线索、事故时间点、层级化事故原因的细粒度标注）和448个经人工验证无事故的安全对照视频（用于诊断提示引发的误报）。在不同提示语和时空窗口设置下评估多种最先进的MLLM模型。

Result: 最优模型在信号危险方面超过95%的准确率，但在识别事故原因方面低于50%。诊断实验进一步表明，显式的危险查询会在无事故视频上诱发严重的误报。

Conclusion: 当前MLLM仅表现出表面的主动安全能力，缺乏稳定、基于因果推理的早期预警能力。这凸显了在动态物理环境中实现可靠主动安全的必要性。
...
### [30] [OmniMech: All-in-one Multimodal Mechanical Benchmark for 3D Reconstruction](https://arxiv.org/abs/2608.05539v1)
...
TL;DR: 本文提出OmniMech，首个百万级规模的工业制造CAD生成基准，包含251,000+张带完整尺寸和公差标注的2D工程图纸，配套原生CAD模型、多视角渲染、网格、STEP和B-rep表示及丰富语义标注，并定义四项核心任务。实验表明当前VLM和CAD专用模型在可执行程序合成、细粒度3D重建及尺寸公差约束执行上仍存在显著不足。


<details>
  <summary>Details</summary>
Motivation: 现有视觉-语言模型（VLM）虽然能从图像生成可执行的CAD程序，但主要针对粗粒度的通用3D物体，未能覆盖工业机械设计中对细粒度几何和毫米级公差的要求，缺乏专门的工业级基准来评估VLM在这方面的能力。

Method: 构建OmniMech基准数据集：包含超过251,000个完全标注尺寸和公差的2D正交工程图纸，与原生CAD模型、多视图渲染、网格、STEP和B-rep表示及丰富语义标注配对。定义四个评估任务：(1)从工程图纸合成参数化CAD程序；(2)图表到3D推理实现几何和结构一致的3D重建；(3)基于标注的尺寸、符号、特征标注和制造约束推理；(4)利用可视化、测量、CAD执行和验证工具进行工具增强的智能体推理。

Result: 实验结果显示，当前的VLM和CAD专用模型在可执行程序合成、细粒度3D重建以及尺寸和公差的可靠执行方面仍然表现不佳，暴露了现有模型在该领域的主要瓶颈。

Conclusion: OmniMech填补了工业级CAD生成评估的空白，将公开基准数据、评估代码和工具接口以支持未来研究，推动VLM在工业机械设计领域的细粒度CAD生成与推理能力发展。
...
### [31] [HERA: Historical Evidence Routing Adapter for Physical Prediction in Latent World Models](https://arxiv.org/abs/2608.05523v1)
*Yuanruyi,Yue Cao,Haojia Gao,Guanqiu Guo,Ziyuezhang,Shangqin,Junbo Tan,Bokui Chen,Zhuo Zou,Xueqian Wang*

Main category: cs.CV

TL;DR: 本文提出HERA（Historical Evidence Routing Adapter）框架，通过寄存器路由适配器（RRPM）将保留的历史证据选择性路由到冻结的潜在预测模型中，解决遮挡下物理事件预测依赖不可见证据的问题。在IntPhys2 Main基准上将V-JEPA 2-G的AvgSurprise准确率从52.57%提升至54.35%，尤其在固定相机连续性和不变性任务上分别提升至57.69%和63.46%。


<details>
  <summary>Details</summary>
Motivation: 现有预测视频模型在遮挡条件下难以预测物理事件，因为后续预测可能依赖当前视角已不可见的物体证据。现有方法通过扩大时间上下文、缓存通用视频特征或施加显式对象中心状态来改进历史保留，但未直接解决如何选择性检索相关历史证据并集成到预训练预测器中，而不干扰其原生潜在工作空间的问题。

Method: HERA框架将保留的历史证据路由到冻结的潜在预测器中，具体实例化为RRPM（Register-Routed Patch Memory）轻量适配器，包含三个组件：结构化记忆库（Structured Memory Bank）、记忆寄存器（Memory Registers）和工作区寄存器（Workspace Registers），实现历史证据的选择性检索与集成。

Result: 在IntPhys2 Main分割上，HERA with RRPM将V-JEPA 2-G的成对AvgSurprise准确率从52.57%提升到54.35%。子组分析显示固定相机连续性从46.15%提升到57.69%，固定相机不变性从46.15%提升到63.46%，改进显著。

Conclusion: 历史证据路由是一种实用的适应策略，能够在冻结的潜在世界模型中有效改善物理预测性能，尤其在遮挡和固定相机场景下表现突出。
...
### [32] [DynaPix: Can Vision-Language Models Identify the Exact Future?](https://arxiv.org/abs/2608.05505v1)
*Thong Nguyen,Vinh-Hien Do,Quynh Vo,Cong-Duy Nguyen,See-Kiong Ng*

Main category: cs.CV

TL;DR: DynaPix是一个让物理场景未来状态预测可检验的基准测试。它使用物理模拟器的视频片段，要求模型从相似候选项或大型图库中选出真实的未来图像。研究发现模型在可见事件标记目标时刻时表现良好，但仅靠时间流逝标记时接近随机；人类则能很好处理后者，说明困难在模型而非问题本身。


<details>
  <summary>Details</summary>
Motivation: 当前对物理场景预测的评估往往只接受文字描述或看似合理的图像，预测结果从未与真实状态核对。作者希望建立一个可以严格检验预测是否正确（与真实未来画面对比）的基准测试，以更真实地衡量模型对物理世界的理解能力。

Method: 构建DynaPix基准：从物理模拟器中生成视频片段（在关键事件前停下），测试时模型需从高度相似的候选图或大型图库中选出对应特定日后时刻的真实未来图像。由于场景来自物理模拟器，正确图像及其精确时间已知，错误的选项被刻意设计为非常相似。另外还探索了用模拟器真实记录训练模型以修复性能。

Result: 模型在可见事件标记目标时刻时表现良好，但仅靠经过的时间标记时接近随机猜测；在大型图库搜索中，真实图像很少排在第一位；人类受试者能很好地完成基于时间流逝的项目。用模拟器真实记录训练可大幅修复性能，但更长时间流逝的情况仍未解决。

Conclusion: DynaPix揭示了"时间锚定差距"：模型将预测锚定到具体事件的能力远强于锚定到时间本身的能力。现有模型缺乏对物理场景随时间演化的精确时间感知，这是未来需要解决的挑战。
...
### [33] [One Frozen Simulator Is Not Enough: Simulator Collapse in Multi-Agent RL](https://arxiv.org/abs/2608.12253v1)
...
TL;DR: 本文指出基于单一LLM模拟用户的多智能体强化学习方法存在系统性泛化失败，根源在于"模拟器坍缩"（simulator collapse）——模拟器LLM模式坍缩导致策略过拟合其主导模式，从而在真实用户上表现不佳。作者提出两个互补方案：推理时的Verbalized Sampling（通过言语化响应分布采样拓宽模拟器行为）和训练时的Co-Training（联合优化策略对抗可训练模拟器群体），在三个多轮基准和人类研究上分别提升最多9%和14%的held-out成功率。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习用于人机交互时通常依赖单一LLM模拟用户行为，但这种做法系统性泛化失败。作者将失败根源归结为"模拟器坍缩"：模式坍缩的模拟器导致策略过拟合其主导模式，进而无法迁移到未见过的模拟器和真实用户。当前缺乏对该问题的理论形式化分析和有效解决方案。

Method: 首先从理论上形式化定义了"模拟器坍缩"问题。随后提出两个互补方案：(1) 推理时的Verbalized Sampling——从模拟器的言语化响应分布中采样，拓宽行为空间并削弱模式坍缩；(2) 训练时的Co-Training——让策略针对一组可训练的模拟器群体进行联合优化，避免过度拟合单一模拟器的模式。作者还发布了SCOPE开源框架，用于支持种群Co-Training多智能体RL。

Result: 在Persuasion for Good、τ²-bench和CooperBench三个多轮基准上验证：Verbalized Sampling较单模拟器RL在held-out成功率上提升最高9%；Co-Training进一步将增益推至14%；人类研究在真实用户上观察到相似增益。两种方案均保持了单模拟器RL下会崩溃的策略多样性。

Conclusion: 训练环境的多样性（而非仅策略本身）对多轮RL泛化到真实世界部署至关重要。模拟器群体的多样性是提升RL策略在真实用户上泛化能力的关键因素。
...
### [34] [Located but Not Releasable: Silent Gate Inversion and Bounded Linear Release](https://arxiv.org/abs/2608.11822v1)
*Xining Xun*

Main category: cs.CL

TL;DR: 该论文对"检测-定位-释放"完整管线（先定位潜在因果结构、再将其转化为行为）进行了完全预注册的压力测试。测试对象是一个2.57M参数的transformer，在因果证据判别任务上训练，且已知存在"潜在因果结构已存在但行为上未使用"的抑制现象。结果发现三点：(i)定位成功——对中间层观测证据通道的干预能恢复目标行为；(ii)门控机制在分布外彻底失效——检测器完全倒置，在真正需要释放的场景上触发率为零，导致门控管线静默退化为基座模型；(iii)线性释放存在上限——无条件注入每实例线性方向时剂量-反应曲线单调增加但在预设释放阈值下方就饱和，每实例自适应仅增加不到±0.03。两种失败可分离，均不推翻定位的成功结论。


<details>
  <summary>Details</summary>
Motivation: 已有大量研究表明语言模型表征中存在与任务相关的潜在结构但未被模型行为使用。然而，这种结构一旦被定位后能否被转化为实际行为，是一个很少被端到端检验的独立问题。本文旨在对这一完整转化管线进行严格的压力测试。

Method: 在2.57M参数的因果证据判别transformer上，对"检测(detect)-定位(localize)-释放(release)"完整管线进行全预注册测试。预先哈希并归档所有阈值、声明模板和决策树分支。在已知存在抑制现象的中间层观测证据通道上进行干预实验，检验门控检测器与线性释放路径的行为。

Result: (i)定位成功：中间层观测证据通道的干预恢复目标行为，配对释放优势为0.563和0.854（97.5%置信区间排除0），最佳位点释放率为0.889。(ii)门控机制分布外失效：检测器在零个分布外校准世界上触发，却在6.9-7.3%的分布内生成上触发，且在需要释放的2,400个held-out生成上触发率为零——完全倒置，使门控管线静默退化为基座模型。(iii)线性释放受限：无条件注入线性方向的剂量-反应曲线单调增加但在预设释放阈值（≤0.08）下方饱和（截距从0.382降至0.311再降至0.264），每实例自适应的增益不足±0.03。

Conclusion: 两种失败模式（检测器OOD倒置、线性释放方向族不充分）是可分离的，且均不推翻定位的成功结论。任何企图将潜在结构转化为行为的管线，其检测与释放环节都可能独立失效，即便定位完全成功。所有数字均可追溯至发布审计链中的哈希归档工件。
...
### [35] [CT-$Δ$Bench: A Benchmark for Longitudinal 3D Medical Imaging Difference Reporting with Vision-Language Models](https://arxiv.org/abs/2608.11534v1)
*Kegeng Tang,Jingbo Wang,Shaogang Ren,Zihao Wang*

Main category: cs.CL

TL;DR: 本文首次系统研究纵向影像差异报告任务，提出CT-ΔBench基准（含患者级划分防止信息泄露）、针对临床纵向变化设计的change-aware评估指标、独立医师验证流程，并比较直接配对CT推理与间接两阶段管线，最后提出基线模型DeltaMed进行直接配对CT差异报告。


<details>
  <summary>Details</summary>
Motivation: 现有医学基础模型局限于单次影像（single-study）理解，缺乏对同一患者不同时间点扫描影像的时序交叉比较能力，而纵向比较（如疾病演变、疗效评估、复发检测）恰恰是临床决策中的核心环节，亟待填补这一空白。

Method: 1) 提出纵向影像差异报告任务定义；2) 构建CT-ΔBench基准数据集，采用患者级划分防止信息泄露；3) 设计change-aware评估指标以捕捉临床有意义的纵向变化，而非仅依赖表面文本相似度；4) 进行独立医师验证以评估合成参考与事件提取管线的可靠性；5) 对比直接配对CT推理与两阶段管线（先生成单时间点报告再做文本差分）；6) 提出DeltaMed基线模型并训练。

Result: 构建了专用的CT-ΔBench基准和change-aware评估体系，通过医师验证确认了合成参考与事件提取管线的可靠性，并对比了直接与间接两种推理范式，训练了DeltaMed基线模型。

Conclusion: 这些贡献为具备时序感知能力、更贴近真实世界纵向临床推理的医学基础模型奠定了研究基础。
...
### [36] [Knowledge-Graph-Guided Retrieval-Augmented LLMs for Explainable Root Cause Analysis in Automotive HiL Validation](https://arxiv.org/abs/2608.11277v1)
*Hamza Ouarrad,Mohammad Abboush,Andreas Rausch*

Main category: cs.CR

TL;DR: 本文提出一个知识图谱引导的检索增强大语言模型（KG-guided RAG-LLM）框架，用于汽车硬件在环（HiL）测试中多变量时间序列数据的根因分析（RCA）和故障定位。该方法将原始时间序列转换为诊断证据，结合传感器-位置-传播知识，检索历史相似案例，利用LLM作为决策与解释层输出排名的故障位置预测及可解释的RCA说明。在汽油发动机和电动汽车两个HiL案例中，Top-1准确率分别达90%和94%。


<details>
  <summary>Details</summary>
Motivation: 现有汽车HiL系统验证产生大规模多变量时间序列数据，人工分析耗时且多限于异常检测和故障分类而非根因分析。深度学习方法虽性能好，但需要任务特定的训练或重训练，且将定位视为分类任务，缺乏对故障位置、传感器和下游子系统效应之间空间与功能关系的显式表示，限制了泛化性和工程诊断实用性。

Method: 框架包含三个核心步骤：(1) 将原始时间序列记录转换为紧凑的诊断证据；(2) 利用传感器-位置映射和故障传播知识丰富证据（知识图谱引导）；(3) 检索相似历史案例以支持最终推理。LLM作为决策和解释层，而非直接的时间序列分类器，生成排名的故障位置预测及可解释的RCA解释。

Result: 在两个汽车HiL案例（ASM汽油发动机和电动汽车系统）上评估，最佳模型Top-1准确率分别达到90%和94%；记录级聚合在评估子集上实现了完美的文件级故障定位。

Conclusion: KG引导的RAG-LLM推理框架在可解释性和泛化性方面均展现了用于HiL根因分析的潜力，为汽车软件系统的自动化诊断提供了有效方案。
...
### [37] [Evolutionary tracks of giant planets formed by disk instability](https://arxiv.org/abs/2608.11984v1)
*Mirco Bussmann,Ravit Helled*

Main category: astro-ph.EP

TL;DR: 本文首次在统一数值框架下模拟了由盘不稳定性形成的巨行星的完整演化过程，包括塌缩前阶段、动力学塌缩和长期收缩。研究发现金属丰度是年龄-光度关系推断行星质量的主要不确定性来源（可达1.5木星质量），并验证了盘不稳定性和核吸积两条形成路径可以产生相似的长周期演化轨迹。模型预测的质量与HR 8799 e、AF Lep b、Beta Pic b和Beta Pic c的动力学质量约束一致，表明盘不稳定性仍是巨行星形成的可行途径。


<details>
  <summary>Details</summary>
Motivation: 核心吸积形成的巨行星已有自洽的演化模型，但盘不稳定性形成的行星缺乏类似的演化模型。为评估盘不稳定性作为巨行星形成路径的可行性，需要建立统一的演化框架来模拟这类行星从形成到长期演化的全过程，特别是塌缩前阶段和后续收缩阶段的行为。

Method: 使用改进的MESPA代码在统一数值框架中模拟盘不稳定性形成的巨行星演化，涵盖塌缩前（pre-collapse）气体团块、动力学塌缩和长期收缩三个阶段。考虑1至12木星质量的参数范围，金属丰度从原恒星值的0.5到2倍。通过模拟演化轨迹，将其与年龄-光度关系以及已知系外行星的动力学质量约束进行对比验证。

Result: 确认塌缩前的时间尺度强烈依赖于行星质量；动力学塌缩后物体进入可维持数十亿年的长期收缩状态。金属丰度是年龄-光度关系质量推断的主要不确定性来源，在所考虑的金属丰度范围内，给定年龄和光度时质量推断差异可达1.5木星质量。演化轨迹预测的质量与HR 8799 e、AF Lep b、Beta Pic b和Beta Pic c的动力学质量测量一致。核吸积和盘不稳定性可产生非常相似的长周期演化轨迹，仅凭光度演化无法区分两种形成路径。

Conclusion: 盘不稳定性仍是巨行星形成的可行途径，模型与动力学质量测量的一致性支持了这一结论。仅依赖亮度演化无法区分两种形成路径，因此推断年轻巨行星质量时必须考虑行星金属丰度的影响，因为它显著影响行星演化。
...
### [38] [Making Collaborative Signals Count: Graph-Aware Large Language Models for Sequential Recommendation](https://arxiv.org/abs/2608.12184v1)
*Fenglin Yan,Bohao Wang,Jian Zhang,Yu Cui,Tongya Zheng,Ye Feng,Can Wang,Jiawei Chen*

Main category: cs.IR

TL;DR: 提出GALLM框架，通过在LLM注意力机制中引入基于协作图的注意力偏置，使大语言模型能够捕捉用户-物品交互中的全局协作信号，用于序列推荐，在四个基准数据集上相比最强基线在HR@5上平均提升9.76%。


<details>
  <summary>Details</summary>
Motivation: 大语言模型以语言为中心预训练，难以捕捉用户-物品交互中的隐式协作信号，而协作信号对个性化推荐至关重要。现有方法要么依赖外部推荐器注入协作表示，要么仅建模序列内部依赖，限制了全局协作模式的利用。

Method: GALLM在图谱（文本token与物品token构成的协作图）上建模三种类型的关系：Text-Text关系（保留语义依赖）、Item-Text关系（对齐物品token与其文本描述）、Item-Item关系（基于物品全局共现模式）。这些关系被转化为轻量级可学习注意力偏置，直接整合进LLM注意力机制中，无需额外图编码器。

Result: 在四个真实世界基准数据集上的实验表明，GALLM在所有对比基线中取得最佳性能，在HR@5上平均比最强基线提升9.76%。

Conclusion: 通过在LLM内部注入注意力偏置的方式，GALLM能够在不引入额外图编码器的情况下，使语言模型感知全局协作模式，有效提升序列推荐的性能。
...
### [39] [Sci-Surf: Navigating Scientific Literature Discovery through Human Feedback and Intelligent Summarizatio](https://arxiv.org/abs/2608.11973v1)
*Fang Guo,Qi Zhu,Rongcan Pei,Shuqi He,Hui Chen,Yue Zhang*

Main category: cs.IR

TL;DR: Sci-Surf是一个意图驱动的学术知识发现系统，将基于反馈的个性化推荐与多模态博客式论文消化相结合，通过LLM用户画像细化意图表示，并生成综合论文文本与视觉信息的结构化摘要。真实用户评估显示，引入语言化画像后，在为期一个月的在线评估中预测对齐平均提升了10.4%。


<details>
  <summary>Details</summary>
Motivation: 科学出版物快速增长使研究者难以识别和深入理解相关新研究。现有学术发现平台依赖静态主题订阅或嵌入相似度，且仅提供摘要或简短概述，无法支持细粒度意图建模和深度论文总结。

Method: 提出了Sci-Surf系统，包含两个核心模块：(1) 基于LLM的用户画像推理，从用户反馈中提炼语言化意图表示，用于个性化推荐；(2) 多模态博客式论文消化，生成综合论文文本与视觉信息的结构化摘要。系统通过反馈驱动不断细化用户意图。

Result: 通过真实用户评估验证了端到端学术发现管线的有效性。语言化用户画像的引入在为期一个月的在线评估中，使预测与真实用户偏好的对齐平均提升了10.4%，同时在推荐质量和消化质量上均有可测的改进。

Conclusion: Sci-Surf展示了意图中心范式在学术发现系统中的潜力，即通过LLM驱动的用户画像和多模态摘要生成来同时提升推荐精准度和内容消化深度，为下一代学术知识发现工具提供了可行设计方向。
...
### [40] [From Overlooked to Explored: Recovering Item Relations via Mixture of Perspectives for Sequential Recommendation](https://arxiv.org/abs/2608.11846v1)
*Junyoung Kim,Wonbin Kweon,Woojoo Kim,Jaehyung Lim,Dongha Kim,Hwanjo Yu*

Main category: cs.IR

TL;DR: 本文提出PRISM模块，通过多视角透镜（亲和视角与对比视角）校准自注意力中的相似性偏差，从而同时捕捉同质和异质物品关系，在七个真实基准上超越现有最优方法。


<details>
  <summary>Details</summary>
Motivation: 序列推荐的核心挑战是从交互序列中捕捉用户偏好，而偏好本质上源于物品间的关系。尽管自注意力因其能计算两两物品交互而成为主流，但实证分析发现点积注意力分数存在相似性偏差，过度偏向相似物品，系统性忽视了含有偏好信号的有意义异质关系，从而限制了推荐性能。

Method: 提出PRISM（基于视角的关系洞察合成模块），采用K个视角透镜从不同角度校准注意力。该模块包含两个关键视图：亲和视图（Affinity View）用于精炼同质关系，对比视图（Contrast View）用于暴露被相似性偏差抑制的异质关系，从而使模型能够捕捉完整的用户偏好光谱。

Result: 在七个真实世界基准数据集上的大量实验表明，PRISM持续优于最先进（SOTA）的基线方法。

Conclusion: PRISM通过多视角注意力校准有效解决了自注意力固有的相似性偏差问题，能够更全面地捕捉物品间关系中的用户偏好，为序列推荐提供了更准确的建模方案。
...
### [41] [SelectLight: Learning to Select Signal Plans Generated by Distributed Model Predictive Control for Urban Traffic Networks](https://arxiv.org/abs/2608.12256v1)
*Lyuzhou Luo,Chaopeng Tan,Zhengyong Gao,Hong Zhu,Andrea D'Ariano,Keshuang Tang*

Main category: eess.SY

TL;DR: 提出SelectLight框架，将多智能体强化学习（MARL）策略直接用于从分布式模型预测控制（DMPC）在线生成的非支配候选信号方案集合中进行后优化选择，既保留了可解释性与约束可行性，又能从闭环结果中学习，在两个28交叉口SUMO网络中取得最优延迟性能与最低迁移损失。


<details>
  <summary>Details</summary>
Motivation: 多目标分布式模型预测控制（DMPC）能在线上构造可行的信号配时方案，但其在权衡解之间的选择规则是预先规定的，无法从实际闭环结果中学习改进。已有方法未能将学习能力与约束可行的规划能力有效结合。

Method: 提出SelectLight框架：(1) 状态剪枝多目标动态规划（SP-MODP）用Newellian点-空间队列模型评估方案，返回关于总排队延迟、峰值排队积聚和总停车次数的有界互不支配候选方案集合；(2) 基于拓扑感知注意力策略，使用独立近端策略优化（IPPO）训练，从每个变长集合中选择一个未修改方案；(3) 学习仅限定于候选选择，保留预设信号配时约束，所选方案及其预测目标权衡可被检验。

Result: 在28交叉口SUMO网络上，SelectLight取得最佳延迟相关性能，且优势随需求增加而扩大。在2倍基线需求下，相对最强基线减少排队延迟5.57%、等待时间6.44%。在全部测试需求迁移场景下迁移损失最低。120秒预测时域下，每交叉口99%分位SP-MODP求解时间为5.408毫秒，远低于5秒控制间隔。

Conclusion: 将MARL嵌入DMPC后优化选择流程是可行且高效的，它兼顾了学习能力、约束可行性、实时性、可解释性和泛化能力，为城市网络协调信号控制提供了一种新范式。
...
### [42] [Urban logistics dynamics: a user-centric approach to traffic modelling and kinetic parameter analysis](https://arxiv.org/abs/2608.11866v1)
*Emilienne Lardy,Eric Ballot,Mariam Lafkihi*

Main category: stat.AP

TL;DR: 本文从用户视角重新审视城市物流中的交通条件建模，利用Art.Kinema参数描述车辆驾驶行为的运动学特征，并通过因子分析和广义线性模型探讨如何基于有限的外生驾驶环境因素（如时间、日期、道路类型、坡度、天气等）预测车辆的运动学行为，旨在为供应链管理研究提供开放可解释的预测模型。


<details>
  <summary>Details</summary>
Motivation: 现有高精度交通预测服务（如路径规划中的实时信息）对用户而言是"黑箱"，其依赖AI处理的计数数据无法容纳管理研究（尤其是供应链管理）所需的开放设计参数。因此需要一个可解释的、从车辆个体而非交通流角度出发的交通条件建模方法。

Method: 1) 利用Art.Kinema参数框架来全面描述车辆驾驶行为的运动学和动力学方面，指标超出单一车速；2) 基于真实驾驶数据（高频车速测量），选取有限的外生环境变量（时间、日期、道路类型、朝向、坡度、天气）作为自变量；3) 采用因子分析（Factor Analysis）和广义线性模型（GLM）建立运动学参数与分类情境变量之间的关联模型；4) 对模型拟合质量和稳健性进行评估。

Result: 研究给出了所建立模型的拟合优度评估结果、模型稳健性评估结果，以及模型输出的概述，展示了基于有限外生因素预测车辆运动学行为的可行性和预测精度。

Conclusion: 该研究证明了通过有限的可解释外生情境因素即可较为准确地预测车辆在城市物流中的运动学行为，为供应链管理提供了一个开放、透明、可参数化的交通条件建模途径，弥补了现有"黑箱"预测服务的不足。
...
### [43] [Spectral graph clustering with inhomogeneous latent geometry](https://arxiv.org/abs/2608.11321v1)
*Konstantin Avrachenkov,Lucas S. Sibemberg,Alexander Van Werde*

Main category: cs.SI

TL;DR: 本文研究了混合潜在几何存在下的谱聚类问题。在块潜在空间模型中，证明社区可以从谱中更深的特征向量中恢复，并提出DBSPEC算法——一种仅需近似定位信息特征值、对特征值分离不敏感且能处理一般潜在几何的基于密度的谱聚类算法。


<details>
  <summary>Details</summary>
Motivation: 传统谱聚类在存在混淆性潜在几何时会失效，因为前导特征向量被潜在几何主导而非社区结构。已有方法局限于均匀环面模型，无法处理一般潜在几何，需要更通用的解决方案。

Method: 作者在块潜在空间模型中分析邻接矩阵的谱性质，通过极限积分算子来刻画谱结构，据此开发DBSPEC（基于密度的谱聚类算法）。该算法仅需近似定位信息特征值，对特征值分离质量不敏感，可处理一般潜在几何而无需假设特殊的均匀拓扑结构。

Result: 理论预测的信息特征值位置与真实世界实验观察结果一致，验证了方法的有效性。

Conclusion: 在潜在几何干扰下，社区信息并非仅存于前导特征向量中，而是可以通过谱中更深层次的特征向量恢复；DBSPEC算法为一般潜在几何下的谱聚类提供了稳健且通用的解决方案。
...
### [44] [Fine-Tuning Generative Models for Extreme Events via CVaR-Penalized Wasserstein Gradient Flows](https://arxiv.org/abs/2608.11544v1)
*Thejani Gamage,Hyemin Gu,Zhizhen Zhang,Ziyu Chen,Markos Katsoulakis,Luc Rey-Bellet*

Main category: stat.ML

TL;DR: 提出CVaR-GPA算法，通过Wasserstein梯度流最小化Lipschitz正则化KL散度与CVaR惩罚项之和，用于将预训练生成模型微调到重尾分布并捕捉极端事件，无需对目标分布尾部有任何先验知识或估计。


<details>
  <summary>Details</summary>
Motivation: 标准生成模型（如基于Lipschitz传输映射的生成器）从轻尾源分布采样，其Lipschitz性质会保持轻尾行为，难以学习重尾目标分布或捕捉极端事件；且重尾目标在欠采样尾部区域，标准梯度流的粒子速度过早消失，导致尾部学习失效。

Method: 将Wasserstein梯度流定义为Lipschitz正则化KL散度与CVaR散度惩罚项之和。Lipschitz正则化KL散度在最小假设下实现稳健学习；CVaR惩罚项恢复在欠采样尾部过早消失的粒子速度。该惩罚流产生有界但非Lipschitz的速度场，突破标准生成器Lipschitz传输映射的限制，实现向更重尾目标的传输。从Rockafellar-Uryasev表示推导CVaR的一阶变分次梯度（在经典密度公式失效处仍然有效），定义经验测度上的流。算法无需访问预训练模型架构，基于动能停止准则的自适应时间范围运行。

Result: 在合成各向同性和各向异性Student-t目标分布、Neal's漏斗分布以及真实高维Fama-French 25投资组合数据集上，CVaR-GPA相比预训练基线显著提升了重尾目标的全局精度和尾部精度。

Conclusion: CVaR-GPA是一种稳健、尾部无关的生成模型微调算法，能够有效学习重尾分布并捕捉极端事件，同时不需要对目标分布的尾部特征进行先验估计，在合成与真实数据集上均取得显著改进。
...
### [45] [Statistics of Solar Filament Mass based on CHASE Sun-as-a-star Spectroscopic Observations](https://arxiv.org/abs/2608.11826v1)
*T. Y. Xie,Z. H. Zhao,X. Cheng,Y. H. Chen,Z. Zheng,Q. Hao,C. Li,M. D. Ding*

Main category: astro-ph.SR

TL;DR: 本文利用CHASE卫星的全日面Hα光谱观测，对1346个太阳暗条进行了太阳-as-a-star统计研究，建立了三维形态尺度比例（L:W_app:D_LOS ≈ 4.5:1:1.7），并发现太阳-as-a-star质量估计与实测内禀质量高度一致（log空间回归斜率为1.07），为恒星暗条质量估计提供了关键参考。


<details>
  <summary>Details</summary>
Motivation: 暗条是太阳及其他恒星日冕中的致密低温等离子体，准确估计其质量对理解后续爆发及空间天气效应至关重要。然而，暗条固有的几何不确定性（尤其在空间不可分辨的恒星观测中）严重阻碍了质量估计的准确性，因此需要对恒星暗条质量估计方法进行测试和校准。

Method: 利用CHASE卫星的全日面Hα光谱观测，通过机器学习分割模型识别了2024年1月至2025年10月期间的1346个暗条。将暗条区域进行空间积分构建虚拟太阳-as-a-star光谱，通过云模型拟合获得光学参数，并校正投影效应后建立了三维形态尺度关系。

Result: 建立了暗条三维形态尺度比例L:W_app:D_LOS ≈ 4.5:1:1.7，暗条中值深度约8000 km。太阳-as-a-star估计质量与实测内禀质量在全体样本中高度一致，log空间回归斜率为1.07。

Conclusion: 作为首个大样本太阳暗条的太阳-as-a-star研究，该结果为暗条几何和质量的估计提供了经验约束，为基于Hα光谱估计恒星暗条质量提供了关键参考依据。
...
### [46] [Testing Covarying Coupling Constants (CCC) against the full SPARC rotation-curve sample: a like-for-like comparison with MOND and NFW](https://arxiv.org/abs/2608.11575v1)
*Rajendra P. Gupta,Nikolaos Samaras*

Main category: astro-ph.GA

TL;DR: 本文在完整的SPARC样本（175个盘状星系）上检验协变耦合常数（CCC）框架，将其旋转曲线预测能力与MOND以及单/双参数NFW晕在同等条件下比较。作者指出现有研究中的尖锐密度截止是非物理的，并引入平滑过渡（密度空间的MOND插值函数类似物）而不引入新参数。结果表明单参数平滑CCC与逐星系拟合的MOND性能相当（平均χ²ν为2.58对2.65），优于双参数NFW（平均χ²ν≈7）。CCC的转折加速度a_t的散布（0.33 dex）与MOND尺度a₀（0.34 dex）相当，量级约为2×10⁻¹⁰ m/s²，且消除了尺寸相关性。


<details>
  <summary>Details</summary>
Motivation: CCC框架最初为解释JWST高红移观测而提出，其通过依赖局部密度的协变常数有效质量场来修改星系动力学，无需粒子暗物质。此前仅对少数星系做过测试，本文将其系统性地应用于完整SPARC样本，与主流替代方案（MOND和NFW晕）进行公平对比，并修正了先前研究中非物理的尖锐密度截止问题。

Method: 采用逆问题方法：每个模型由观测到的旋转曲线预测重子旋转曲线。在完全相同的条件下，使用约化χ²ν比较三种模型：单参数平滑CCC、逐星系拟合的MOND、以及单/双参数NFW晕。将原研究中非物理的尖锐密度截止替换为平滑过渡函数（密度空间的MOND插值函数类似物），不引入额外参数。

Result: 单参数平滑CCC在56%的星系中给出更低χ²ν，平均χ²ν为2.58 vs MOND的2.65（配对差异不显著）。双参数NFW的拟合质量分布更宽且有更多差/边界受限拟合（平均χ²ν≈7）。CCC转折密度非普适（0.82 dex散布）且与星系尺寸相关（可由扁平盘系统的球对称重建定性解释）；但重写为加速度a_t = V_flat²/R_t后，散布降至0.33 dex（91个可解析星系子集），与MOND尺度a₀的0.34 dex相当，量级约2×10⁻¹⁰ m/s²，尺寸相关性消失。

Conclusion: 尽管CCC并非为星系动力学而设计，但它描述旋转曲线的能力与逐星系拟合的MOND相当，且优于双参数NFW晕。CCC的转折密度虽非普适常数，但转化为加速度后与MOND的a₀在散布和量级上惊人地一致，暗示CCC与MOND之间存在深层联系。
...
### [47] [A Perspective on Phase Space Electronic Structure Theory : From Its Surface Hopping Origins Through To Its Future Promise](https://arxiv.org/abs/2608.11448v1)
...
TL;DR: 本文追溯了相位空间电子结构理论（PSEST）的发展历史，阐述了它如何从半经典表面跳跃动力学的基础问题中演化而来，成为基于Born-Oppenheimer近似的标准电子结构理论（使用移动而非冻结的原子核）的替代方案。文章以直觉化、少公式的方式向新入行者介绍在非惯性参考系中求解薛定谔方程所涉及的物理，以及PSEST在理解涉及自旋的化学问题中的必要性。


<details>
  <summary>Details</summary>
Motivation: 为PSEST领域的新入行者提供直观理解，填补标准电子结构理论在处理涉及自旋的化学问题时的局限性，同时补充Bian等人发表的更技术性综述文章。

Method: 采用非技术性的直觉化叙述方式，少用方程，重点阐述物理直觉，追溯PSEST的历史发展脉络，并列举该快速发展的领域中的开放问题。

Result: 本文是一篇偏重直觉和历史的视角性文章，展示了PSEST如何从半经典表面跳跃动力学的起源演化为一套独立的电子结构理论框架，并指出了该领域面临的若干开放问题。

Conclusion: PSEST是理解涉及自旋的化学问题的必要理论发展方向，该领域仍有许多开放问题等待新研究者探索，本文作为一篇互补性文章有利于吸引新实践者进入该领域。
...
### [48] [Estimating the perturbed utility route choice model with trip-level data](https://arxiv.org/abs/2608.11464v1)
*Mogens Fosgerau,Nikolaj Nielsen,Thomas Rasmussen,Rui Yao*

Main category: stat.ME

TL;DR: 本文提出了一种针对扰动效用路径选择（PURC）模型的估计器，该估计器适用于个体出行级别数据，采用嵌套固定点算法结合上偏差校正线性回归与个体级扰动效用最大化问题，并验证了其统计性质及实际应用效果。


<details>
  <summary>Details</summary>
Motivation: 现有PURC模型的估计方法通常需要聚合级别的数据，无法充分利用个体出行级别的信息。作者旨在开发一种能够在个体出行数据上有效估计PURC模型的估计器，以提升模型估计的精度和实用性。

Method: 提出微PURC（microPURC）估计器，采用嵌套固定点算法：上层为偏差校正的线性回归问题，下层为个体级别的扰动效用最大化问题。通过这种嵌套结构同时估计模型的参数。

Result: 建立了microPURC估计器的统计性质，并通过模拟数据实验验证了理论结果。此外，在大规模真实世界数据集上展示了估计器的实际应用效果。

Conclusion: microPURC估计器能够有效地在个体出行数据上估计PURC模型，具备良好的统计性质，并通过模拟与真实数据验证了其可行性和有效性。
...
### [49] [Connecting the tensor-categorical formulation of anyon condensation with operator algebras and entropic order parameters](https://arxiv.org/abs/2608.12157v1)
*Hua-Chen Zhang*

Main category: cond-mat.str-el

TL;DR: 本文利用最近发展的准局部C*-代数上的Doplicher-Haag-Roberts双模形式化体系来研究任意子凝聚现象，建立了张量范畴与算子代数两种表述之间的联系，并自然地定义了表征凝聚的熵序参量，给出了其上限的极简证明。


<details>
  <summary>Details</summary>
Motivation: 任意子凝聚描述拓扑量子场论之间的转变，已有的两种表述——张量范畴和算子代数——之间的对应关系尚未被充分阐明。本文旨在通过DHR双模形式化体系，使这两种表述之间的概念联系变得直观明了，并为凝聚现象提供一个量子信息论意义上的序参量。

Method: 采用文献[1]中提出的准局部C*-代数上的Doplicher-Haag-Roberts双模形式化体系，将任意子凝聚与特定算子代数的扩张相关联，通过图示化方式使张量范畴与算子代数两种表述中的概念对应关系变得直观。在此基础上，定义了一个基于量子信息论的熵型序参量来表征凝聚。

Result: 给出了一个非常简洁的关于该熵序参量上限的证明。

Conclusion: DHR双模形式化体系为理解任意子凝聚提供了统一的框架，使得两种表述的概念联系更为直观，同时熵序参量的引入为凝聚提供了量子信息论层面的定量刻画。
...
### [50] [A Frequency-Space Terahertz Transceiver Chip for Multi-Agent Communications and Spatial Awareness](https://arxiv.org/abs/2608.11651v1)
...
TL;DR: 本文报道了一款208-258GHz、65nm CMOS工艺的全集成太赫兹收发芯片，在1.5mm×4.9mm面积内单片集成宽带前端与异构漏波超表面（HLM）孔径，仅用四个超原子即可实现75°频率控制的波束扫描，并通过频域与空域混频的协同设计实现频谱纯净的频率-空间映射，支持空分频分多址（SFDMA）通信，展示了多智能体同时收发、二维定位及感知增强通信能力，为未来太赫兹具身智能网络提供了可扩展硬件平台。


<details>
  <summary>Details</summary>
Motivation: 室内具身智能系统需要同时支持大容量多智能体连接和相互空间感知的可扩展硬件平台。太赫兹频谱虽然具有丰富的带宽和固有的空间选择性，但传统的相控阵和可编程超表面依赖密集波束赋形网络、单元级控制或外部太赫兹照明，使得可扩展多波束操作面临严峻挑战。

Method: 该芯片采用65nm CMOS工艺，单片集成宽带射频前端与异构漏波超表面（HLM）孔径。HLM产生强色散的漏波模式，仅用四个超原子即可实现75°的频率控制波束扫描。通过频域与空域混合的协同设计，实现频谱纯净的频率-空间映射，进而支持SFDMA通信。

Result: 芯片在208-258GHz频段工作，单片面积仅1.5mm×4.9mm。实现了75°频率控制波束扫描、多智能体同时收发、二维定位以及感知增强通信等多功能演示。

Conclusion: 该太赫兹芯片为未来太赫兹具身智能网络提供了一种可扩展的硬件平台，克服了传统波束成形方案在可扩展多波束操作上的瓶颈，融合了通信与感知能力。
...
### [51] [CoQui: A Coordinate-Conditioned Quantum Implicit Generative Adversarial Network for End-to-End Image Generation](https://arxiv.org/abs/2608.11884v1)
...
TL;DR: 本文提出了一种坐标条件隐式函数学习的量子图像生成方法，通过空间坐标和潜变量输入、经典嵌入网络生成电路参数、变分量子电路在每个坐标处评估来生成图像，解耦了图像分辨率与地址量子比特需求，避免了像素间共享概率归一化约束。


<details>
  <summary>Details</summary>
Motivation: 现有基于振幅的量子生成对抗网络(QGAN)在图像生成中存在两个关键缺陷：一是像素位置通常由计算基索引或地址量子比特编码，导致量子资源随图像分辨率增长；二是从归一化量子态联合解码多个像素会产生像素间概率竞争，限制了对每个像素的精确控制。

Method: 将量子图像生成重新表述为坐标条件隐式函数学习。以空间坐标和潜变量为输入，使用经典嵌入网络生成依赖于输入的电路参数，在每个坐标处评估变分量子电路。像素强度直接从专用颜色量子比特的期望值获得，通过查询所有空间坐标来生成完整图像。同时设计了专门化的变分量子电路以提供坐标条件生成的结构性归纳偏置。

Result: 在两个基准数据集上的模拟实验表明，该方法在视觉质量和定量评估上优于基于FRQI的生成方法和PQWGAN，同时使用更少的量子比特，并且生成质量优于相应的经典基线。

Conclusion: 通过坐标条件隐式函数学习框架，有效解决了振幅型QGAN中量子资源随分辨率增长和像素间概率竞争的问题，实现了更高效、更精确的量子图像生成。
...
### [52] [Reconfiguring Geovisualization in the Age of Generative AI: Insights from Domain Experts](https://arxiv.org/abs/2608.12059v1)
*Mengyi Wei,Chenyu Zuo,Jiaying Xue,Nianhua Liu,Dongsheng Chen,Shengkai Wang,Yu Feng,Liqiu Meng*

Main category: cs.CY

TL;DR: 本文通过对20位地理可视化专家的半结构化访谈，考察了生成式AI（GenAI）对地理可视化专业实践的影响。研究发现GenAI扩展了数据管理、创意探索和快速原型制作等能力，但并未消除现有约束，而是将瓶颈从生产环节转移到判断与验证环节。研究提出负责任地使用GenAI需要领域特定的空间验证、溯源、不确定性沟通、人工监督和问责机制。


<details>
  <summary>Details</summary>
Motivation: GenAI日益融入地理可视化，但其对专业实践更广泛的影响尚未得到充分理解。本研究旨在通过专家访谈来填补这一认知空白，揭示GenAI如何重塑地理可视化的专业实践。

Method: 采用半结构化访谈法，对20位地理可视化专家进行深入访谈。访谈围绕四个分析领域展开：数据（Data）、构思（Ideation）、原型制作（Prototyping）和迭代（Iteration），同时鼓励受访者反思超越这些活动的更广泛议题。

Result: GenAI扩展了地理可视化的能力（数据处理、创意探索、快速原型），但瓶颈从生产环节转向了判断与验证环节。专业价值越来越依赖于空间推理、情境解释、美学与伦理判断以及评估AI输出适当性的能力。同时引入了关于溯源、可解释性和问责性的新挑战，涉及模型、开发者、实践者、机构和用户之间责任的重新分配。

Conclusion: 负责任地在地理可视化中使用GenAI需要领域特定的方法，包括空间验证、溯源、不确定性沟通、人工监督和问责制。研究为GenAI如何重构地理可视化的空间知识生产实践提供了基于专家视角的洞见，并对专业实践、教育、系统设计和治理提出了启示。
...
### [53] [Benchmark-Based Comparative Assessment of Publicly Benchmarked Indian Foundation Models: A Capability and Evaluation-Maturity Framework](https://arxiv.org/abs/2608.11891v1)
*Avinash Agarwal,Vridhi Jain*

Main category: cs.CY

TL;DR: 该论文对印度公开基准测试的基础模型（foundation models）进行了结构化比较评估，覆盖八个能力域（通用推理、编程、智能体AI、网络安全、视觉、视频多模态、科学研究、印度语能力），并提出一个四维"基准成熟度指数"（BMI），用于评估各能力域的标准化、参与度、独立验证和国内覆盖程度。研究指出印度模型在饱和基准（如MMLU、MATH-500）上表现好，但参与新版/智能体/领域专项评测的频率明显较低，且不同机构参与度极不均衡。作者论证许多表面上的能力差距实际上是评测生态差距，这对国家AI项目的监测与资助标准设计有直接影响。


<details>
  <summary>Details</summary>
Motivation: 各国政府日益资助本土基础模型以强化国家AI能力、数字主权和多语言计算，但评估这类国家生态系统的进展面临基准报告不一致、专有评测方法和模型快速迭代等挑战，缺乏系统的评估框架来衡量国家层面的AI进展。

Method: 仅使用公开报告的基准测试结果，对印度基础模型与全球前沿模型及同等规模模型在八个能力域进行对比评估；提出一个四维"基准成熟度指数"（BMI），从标准化、参与度、独立验证、国家覆盖四个维度对每个能力域打分，并与纯描述性评估结果进行对照。

Result: 印度模型在MMLU、MATH-500等已被广泛认为饱和的基准上得分很高，但前沿开发者已不再报告这些基准。印度模型在新兴的智能体类和领域专项评测上的参与度明显较低；各印度组织间的基准参与度也极不均衡，其中Sarvam AI的基准覆盖范围远超其他机构。BMI显示其对成熟度的判断能细化甚至修正纯描述性评估的结论。

Conclusion: 公开记录中的许多"能力差距"在现有证据下无法与评测生态系统缺失造成的差距相区分。这意味着国家AI项目在设计和制定监测与资助标准时，必须区分真正的能力短板与评测覆盖不足，BMI可作为更可靠的国家级AI成熟度评估工具。
...
### [55] [RealisticTritonBench: A Benchmark for Triton-Kernel Generation in Real-World AI Frameworks](https://arxiv.org/abs/2608.12004v1)
*Jinjun Huang,Zhongzhen Wen,Tongtong Xu,Meng Yan,Xin Xia,Zhongxin Liu*

Main category: cs.SE

TL;DR: 本文提出RealisticTritonBench，第一个从真实AI框架的pull request中提取Triton内核生成任务的基准测试，克服了现有基准在任务多样性、端到端评估和验证可靠性方面的三大局限。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLM生成Triton内核的基准存在三个关键缺陷：(1)仅局限于PyTorch-to-Triton翻译，未反映真实Triton任务的多样性和复杂性；(2)只评估单个内核性能而非端到端性能，而后者才是AI框架实际部署的核心标准；(3)依赖可能含有缺陷的手写评估脚本，模型可利用这些缺陷绕过正确性检查并获取虚高分数。

Method: RealisticTritonBench从流行的开源AI框架中系统性地提取修改Triton内核的PR，并将其转化为带有具体工程上下文的生成任务。每个任务以自然语言需求为输入，要求生成对应的Triton内核实现，并提供完整可复现的评估环境。与以往只关注孤立内核性能的基准不同，该基准将生成的内核集成回原始框架，并使用端到端测试进行评估。

Result: 在RealisticTritonBench上评估了领先的LLM，发现它们在真实世界Triton内核生成任务上仍然表现困难。

Conclusion: RealisticTritonBench提供了更真实、更贴近生产环境的Triton内核生成评估，揭示了现有LLM在解决真实世界Triton内核生成任务方面的不足，为未来研究指明了方向。
...
### [56] [Dual-Model Sentiment Analysis of Consumer Reviews in the Retail Coffee Sector Using Machine Learning and Deep Learning Approaches](https://arxiv.org/abs/2608.12007v1)
*Muntasir Hasan Kanchan,Md. Alamgir Hossain,Md. Samiul Islam,Muhammad Masud Tarek*

Main category: cs.LG

TL;DR: 本研究提出一个针对星巴克客户评论的对比情感分析框架，使用经典机器学习和深度学习方法。基于ConsumerAffairs收集的700多条评论，评估了五种机器学习分类器（逻辑回归、SVM、决策树、随机森林、朴素贝叶斯）和五种深度学习模型（LSTM、RNN、BiLSTM、GRU、CNN）。结果显示SVM在机器学习模型中取得最高准确率91.0%，BiLSTM在深度学习中表现最佳，同时发现类别不平衡对正面情感召回率产生了负面影响。


<details>
  <summary>Details</summary>
Motivation: 消费者评论对品牌认知和商业策略具有重要影响，尤其在零售咖啡等服务驱动型行业。现有研究缺乏对经典机器学习和深度学习方法在真实世界消费者情感分析场景下的系统性比较评估，因此需要建立一个对比框架来指导实际应用中的模型选择。

Method: 从ConsumerAffairs收集超过700条星巴克评论，经过预处理和探索性数据分析识别时间和地理位置模式。将星级评分二值化生成情感标签（4-5分为正面，1-3分为负面），得到显著不平衡的数据集。评估五种机器学习分类器（Logistic Regression、SVM、Decision Tree、Random Forest、Naive Bayes）和五种深度学习模型（LSTM、RNN、Bidirectional LSTM、GRU、CNN），使用准确率、精确率、召回率和F1分数评估性能。

Result: SVM在机器学习模型中取得最高准确率91.0%；Bidirectional LSTM在深度学习模型中表现最强，并在未见数据上表现出良好的泛化能力。研究还发现类别不平衡问题对多个模型的正面情感召回率产生了负面影响。

Conclusion: 本研究为零售咖啡行业提供了经典机器学习和深度学习方法在真实世界消费者情感分析中的对比评估，强调了在客户体验分析中合理选择模型和进行预处理的重要性。
...
### [57] [Air Quality Station Simulation via LSTM and Attention-Based Modelling](https://arxiv.org/abs/2608.11839v1)
*Alexander Kostadinov,Petar O. Hristov,Dessislava Petrova-Antonova*

Main category: cs.LG

TL;DR: 本文提出了一个名为SATADL的深度学习模型，用于在空气质量监测站出现故障或断电期间，模拟其测量数据，输出多小时的空气质量预报（如PM10浓度），以充当"虚拟代理站"。该模型在全球四组空气质量站数据上进行了验证，最多模拟48小时的缺失数据，结果表明其在预测精度（决定系数和均方根误差）上优于多种基线和已发表的深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 城市空气质量监测依赖测量站的可靠运行，但硬件容易发生故障或断电，导致数据不可用，影响空气质量分析的完整性和质量。因此需要一种能够模拟故障站点测量数据的方法，以在站点恢复前提供连续的虚拟代理测量，保障分析工作的连续性。

Method: 提出名为SATADL的深度学习架构，从数据的不同维度提取信息以推断复杂关系，输出多步超前（multi-hour-ahead）的空气质量预测。模型通过假设施测站故障的场景，最长模拟48小时的PM10浓度，并与多种基线及已发表的深度学习模型进行对比基准测试。

Result: SATADL在全球四组空气质量站数据上，对于不同预测时间窗口，在决定系数（R²）和均方根误差（RMSE）两个指标上均优于基准模型和已发表的深度学习模型，证明了其作为虚拟代理站的适用性和优越性。

Conclusion: SATADL是一种有效且可靠的虚拟代理站方案，能够在空气质量监测站故障期间提供高质量的模拟测量数据，有助于维持城市空气质量分析的连续性和可靠性。
...
### [58] [High-dimensional Multi-objective Bayesian Optimization with Learned Variable Interactions](https://arxiv.org/abs/2608.11713v1)
*Hongyan Wang,Jiayu Huang,Haotian Zheng,Xin Gao,Chi Ding,Ying Liu,Xia Wang,Qing Xu,Keqiang Li*

Main category: cs.LG

TL;DR: 本文提出ViaMOBO，一个基于决策变量交互分析的多目标贝叶斯优化框架，用于解决高维昂贵黑箱多目标优化问题。该框架通过变量交互分析模型判断决策空间是否可完全或部分分解，并在分解后的子空间中进行局部贝叶斯优化，从而克服传统MOBO在高维空间中的指数级采样复杂度问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大多数多目标贝叶斯优化（MOBO）方法受限于低维决策空间，因为其采样复杂度随维度呈指数增长。高维昂贵黑箱多目标问题的优化需要一种能够处理高维决策空间的新方法。

Method: ViaMOBO的核心思想是：首先利用变量交互分析模型判断决策空间是否可以完全或部分分解；然后根据分析结果确定目标函数是可分离、部分可分离还是不可分离的；最后在分解后的决策子空间中执行局部贝叶斯优化。该方法不需要对问题做任何强假设，而是基于决策变量之间的潜在独立或相互依赖关系来自动推导目标函数的可分性结构。

Result: 在合成基准和真实世界基准上，将ViaMOBO与最先进的MOBO方法进行了比较。实验结果表明，ViaMOBO在逼近高维昂贵多目标问题的Pareto前沿方面优于其他相关MOBO基线方法。

Conclusion: ViaMOBO通过变量交互分析实现决策空间的分解，有效解决了高维场景下MOBO的维度灾难问题，在Pareto前沿逼近质量上优于现有方法，为高维昂贵多目标优化提供了通用且有效的解决方案。
...
### [59] [Is Per-Agent Policy Composition Safe? Rethinking Successor-Feature Transfer in Cooperative Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2608.11658v1)
*Zijian Zhao,Sen Li*

Main category: cs.LG

TL;DR: 本文针对多智能体强化学习中目标动态变化后的策略迁移问题，证明了独立逐智能体重组策略库会产生比库中所有策略都更差的联合行为，提出MA-USFA分层方法，通过下层通用后继特征逼近器与上层组合器实现安全且灵活的多目标部署。


<details>
  <summary>Details</summary>
Motivation: 许多强化学习系统（如车队管理、交通信号控制）在部署后目标会动态变化，而针对每个新目标重新训练策略成本过高。单智能体场景下，后继特征与广义策略改进（含通用扩展）已有成熟方案及性能保证；但多智能体迁移问题研究不足，常见的各智能体独立重组自有策略库的做法虽沿用了配方却丢失了保证。

Method: （1）理论层面：证明独立组合由于重组队友会改变每个智能体面对的环境、破坏其依赖的价值函数，可能产生严格劣于库中所有策略的联合行为（该失败在单智能体中没有对应情况）；并证明唯一无条件安全的固定规则是同步组合，但它无法服务给不同智能体分配不同目标的场景。（2）算法层面：提出MA-USFA分层方法，下层为通用后继特征逼近器，在条件于队友目标的条件下预测每个智能体的后继特征；上层为组合器，跨智能体选择每个智能体应遵循的库条目，并提供单智能体价值无法表示的跨智能体修正。训练一次后可直接部署，无需逐任务适配。

Result: 论文从理论上揭示了多智能体独立策略库重组的安全失效机理，论证了同步组合的唯一安全性及其灵活性局限，并提出MA-USFA方法兼顾安全性与灵活性，可一次训练、直接部署于动态目标场景。

Conclusion: 多智能体目标迁移需要在安全与灵活之间取得平衡：同步组合安全但只能服务统一目标，独立组合灵活但可能失效。MA-USFA通过分层架构（下层后继特征预测+上层跨智能体组合与修正）同时实现了安全性和对不同智能体目标的灵活支持，且无需部署后重新训练。
...
### [60] [Dynamics Models for Offline Hyperparameter Selection in Real-World RL](https://arxiv.org/abs/2608.11349v1)
*Jordan Coblin,Han Wang,Martha White,Adam White*

Main category: cs.LG

TL;DR: 本文首次将校准模型（calibration models）应用于真实工业场景——市政水处理厂，验证了离线动力学模型在高维非平稳传感器数据上能够为强化学习（RL）超参数选择提供支持。实验表明，包括带Laplacian距离度量的k近邻模型在内的多种校准方法，能生成逼真的长视距rollout并恢复有意义的超参数敏感性趋势，同时在年规模数据扩展、预训练智能体微调学习率选择和分布偏移鲁棒性方面提供了概念验证及其实践挑战。


<details>
  <summary>Details</summary>
Motivation: 强化学习在真实系统部署中的关键障碍是超参数选择，尤其在无仿真器且在线实验成本高昂的情况下。已有的校准模型方法仅在简单模拟环境中被评估，尚未在真实工业场景中验证其有效性。

Method: 在真实市政水处理厂中，对高维、非平稳的传感器数据进行nexting预测任务，评估多种校准模型方法（包括使用Laplacian距离度量的k近邻模型），并检验模型在年尺度数据集上的扩展性、对预训练智能体微调学习率选择的支撑能力，以及在分布偏移下的鲁棒性。

Result: 校准模型能够生成真实感强的长视距rollout，并恢复有意义的超参数敏感性趋势；在年规模数据上具备可扩展性，且能在分布偏移条件下保持一定鲁棒性。

Conclusion: 该工作为使用离线动力学模型支持RL在真实环境中的部署提供了概念验证，同时揭示了若干重要的实际挑战，为未来研究指明了方向。
...
### [61] [Spatially Resolving the Pre-Thermal Anatomy of a Driven Bosonic Fluid](https://arxiv.org/abs/2608.11561v1)
...
TL;DR: 利用NV磁强计对室温YIG薄膜中的磁振子动力学进行微米级空间成像，解析了从四磁振子相互作用到多代散射级联的参量散射层级，揭示了磁振子向热平衡演化的多阶段动力学过程。


<details>
  <summary>Details</summary>
Motivation: 相干驱动的量子多体系统在达到热平衡前如何重新分布能量仍然是多体物理中的核心挑战，需要直接的实验手段来可视化这一非平衡动力学过程。

Method: 采用氮空位(NV)磁强计对钇铁石榴石(YIG)薄膜进行微米尺度空间成像；通过双频混频协议隔离基本四磁振子相互作用并提取耦合强度；在铁磁共振附近施加强单频激发以触发自发多代散射级联；利用近场磁振子学框架定量捕获动力学，提取级联阶数和非线性系数。

Result: 解析了离散参量散射事件的层级结构；隔离了基本四磁振子相互作用并提取耦合强度；观测到多代自发放射级联，每代的主导散射通道涉及慢磁振子区的逸散射磁振子（类似慢光系统中光学非线性增强）；在慢磁振子区域观察到散射增强，并通过近场磁振子学框架定量捕获了动力学过程，提取了级联阶数和非线性系数。

Conclusion: 该工作确立了空间分辨磁振子学作为可视化非平衡多体动力学的强大平台，揭示了单色注入磁振子向平衡态演化的多级动力学过程。
...
### [62] [ATOM: Geometry-Aware Microgesture towards Object-Agnostic Tangible Interaction](https://arxiv.org/abs/2608.11871v1)
*Yinqiao Wang,Hao Xu,Qixuan Liu,Shengdong Zhao,Pheng-Ann Heng,Chi-Wing Fu*

Main category: cs.HC

TL;DR: ATOM是一个集成框架，通过指尖感知的检测流程和基于可用性的元素优先级排序方法，实现跨日常物品的微手势物物交互，在AR环境中将手持物品转化为0D、1D和2D的物性用户界面，并显著优于消融基线。


<details>
  <summary>Details</summary>
Motivation: 现有微手势交互通常局限于特定物体或固定界面，缺乏跨不同日常物体的通用性。作者旨在支持不同日常物体上的微手势交互，并自动利用每个物体的几何可供性，实现物体无关的物性交互。

Method: 提出了感知指尖的检测流程，利用生成式2D和3D模型进行几何增强与细化；引入基于可用性的方法，根据人体工效学适宜性对检测元素进行优先级排序；在此基础上开发AR系统，将日常手持物体转化为支持0D、1D和2D微手势交互的物性用户界面。

Result: 在跨不同形状和尺寸的日常烹饪物体的转换场景中，ATOM在任务完成率、可用性（SUS）和工作负荷（NASA-TLX）方面均优于消融基线。额外10个物体的研究验证了ATOM在物体和抓取方式上的泛化能力。

Conclusion: ATOM展示了在真实AR场景中实现流畅、物体无关的物性交互的巨大潜力，为日常物品向交互界面的转变提供了有效的技术途径。
...
### [63] [The SLT 2026 SmartGlasses Challenge: Benchmarking Egocentric Multi-Talker Speech Recognition and Understanding with Audio-Language Models](https://arxiv.org/abs/2608.12034v1)
...
TL;DR: 本文介绍了IEEE SLT 2026智能眼镜挑战赛，该挑战赛针对以自我为中心视角的多说话人语音处理问题，基于106小时四通道自我中心语音数据集，评估时间戳说话人属性自动语音识别（TSA-ASR）和口语语言理解（SLU）两个任务。结果表明说话人重叠是影响TSA-ASR性能的主要因素，而副语言声学理解在复杂SLU场景中仍是当前音频-语言模型的难点。


<details>
  <summary>Details</summary>
Motivation: 大语言模型和多模态大语言模型的进展推动了可穿戴语音接口的发展，智能眼镜提供了持续的音频感知平台。然而，由于动态声学条件、说话人重叠以及佩戴者为中心的录音几何带来的空间模糊性，该场景下的语音识别和理解仍具挑战性，需要系统化的评估框架来推动研究。

Method: 构建了IEEE SLT 2026智能眼镜挑战赛，包含两个赛道：双人对话理解和多方会议理解，联合评估TSA-ASR和SLU。基于106小时四通道自我中心语音数据集（含714个真实场景会话），描述了任务设计、数据构建、参赛提交及共享评估结果。

Result: 评估结果显示，严重说话人重叠是影响TSA-ASR性能的主要因素；在复杂的SLU场景中，副语言声学理解对当前音频-语言模型仍是难点。挑战的详细信息已发布于官方挑战网站。

Conclusion: 该挑战赛为智能眼镜自我中心多说话人语音处理提供了首个系统化基准，揭示了说话人重叠和副语言理解是当前技术面临的关键瓶颈，为未来可穿戴语音接口研究指明了方向。
...
### [64] [Domain-Aware Lightweight Spectral-Grouped Convolutions for Hyperspectral Fish Freshness Classification](https://arxiv.org/abs/2608.12227v1)
*Kazi Nabiul Alam,Pooneh Bagheri Zadeh,Akbar Sheikh-Akbari*

Main category: eess.IV

TL;DR: 提出SGNet（光谱分组网络），一个针对高光谱成像鱼类新鲜度检测的轻量级深度学习架构，通过分组卷积和深度可分离空间路径分离光谱与空间特征提取，并采用双注意力机制。在16天冷藏三文鱼数据集上达到97.8%分类准确率和0.64天MAE，仅需4.75M参数，相比ResNet-50和ViT实现5-18倍参数缩减。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习未能充分适配高光谱成像数据的特点：光谱信息主导空间纹理、标签具有序数结构、训练样本数量有限。现有方法无法高效处理这些特性，需要一种领域感知的轻量级网络架构以实现工业级的实时鱼类新鲜度预测。

Method: 提出SGNet架构，核心设计包括：(1) 使用分组卷积分离光谱维度特征提取；(2) 采用深度可分离空间卷积路径处理空间纹理；(3) 引入双注意力机制，将通道维度的squeeze-and-excitation与空间gating相结合，自适应地突出信息丰富的特征；(4) 针对序数标签结构进行优化设计。

Result: 在自建16天冰箱储存三文鱼片数据集上，SGNet达到97.8%分类准确率和0.64天平均绝对误差（MAE），仅需4.75M参数。与ResNet-50和Vision Transformers相比，参数减少5至18倍。消融实验验证了各组件的贡献。

Conclusion: 领域感知的架构设计（充分考虑HSI的光谱主导性、序数标签和小样本特性）能够在保持高精度的同时实现轻量化，适合工业实时新鲜度预测应用。
...
### [65] [ResPCC: A Loss-Resilient Neural Point Cloud Codec over Lossy Networks](https://arxiv.org/abs/2608.11845v1)
*Xueqin Niu,Mufan Liu,Yifan Wang,Le Yang,Jun Sun,Yiling Xu*

Main category: eess.IV

TL;DR: ResPCC是首个端到端神经点云编解码器，专为应对传输丢包场景设计。它通过编码端的Condition-Adaptive Latent Modulation (CALM)和Spatial-Channel Interleaving (SCI)机制，以及解码端的Mask-Aware Graph-based Latent Restoration (MGLR)和Dictionary-based Refinement (DBR)模块，在5%-30%丢包率下实现了优于基线方法的稳定性与率失真性能。


<details>
  <summary>Details</summary>
Motivation: 现有学习型点云压缩方法在理想传输条件下表现良好，但实际网络中普遍存在的丢包问题会严重破坏潜特征，导致坐标漂移和几何退化。现有方法缺乏对数据丢失的固有鲁棒性，因此需要设计一个能够感知丢包率并自适应恢复的点云编解码框架。

Method: ResPCC提出三个核心模块：1) CALM模块在编码端根据感知到的丢包率调制潜特征分布；2) SCI机制将通道维度的数据丢失转化为空间上分散的元素级缺失模式；3) MGLR模块在解码端通过掩码感知的图神经网络恢复损坏特征，随后用DBR阶段将恢复后的特征与典型先验对齐。

Result: 在ShapeNet和SemanticKITTI数据集上，在5%到30%的丢包率条件下，ResPCC相比基线方法在重建保真度和率失真性能上均表现出一致的优越性和稳定性。

Conclusion: ResPCC为实际网络场景下的3D数据传输提供了可靠解决方案，填补了神经点云编解码器在抗丢包鲁棒性方面的空白，展现了在恶劣传输条件下保持高重建保真度的能力。
...
### [66] [Boundary-Continuous Cross-Camera RGB Mapping via Hue-Split Model Trees](https://arxiv.org/abs/2608.11548v1)
*Yuma Kinoshita,Hitoshi Kiya*

Main category: eess.IV

TL;DR: 提出一种基于色调分割的模型树方法，用于跨相机的边界连续RGB映射。通过沿标量色调坐标递归划分源相机色彩空间，在每节点构建仿射颜色校正矩阵（CCM），并利用路径混合与边界原型正则化消除硬分割带来的轮廓伪影，在Canon相机对映射实验中显著降低log-RMSE并抑制色度跳变。


<details>
  <summary>Details</summary>
Motivation: 传统全局仿射CCM无法捕捉相机间因传感器光谱灵敏度差异和ISP流程导致的色调特异性颜色偏差，需要一种能按色调区分建模的颜色映射方法。

Method: 1) 沿源相机色彩空间的标量色调坐标递归划分，构建存储仿射CCM的模型树；2) 使用对数域误差目标拟合节点CCM；3) 提出边界连续公式，将根到叶路径上所有节点CCM的对数域输出进行混合，混合权重在单纯形约束下优化；4) 同时使用图卡对拟合损失和显式连续性正则化器（基于学习阈值两侧的确定性边界原型对）进行优化。

Result: 在Canon EOS-1Ds Mark II到Canon EOS 20D的映射实验中（使用Middlebury Registered Color Checker数据集），色调分割相比单一全局仿射CCM大幅降低log-RMSE；所提出的路径混合与边界原型正则化在两种光照和多种曝光条件下同时提升精度并抑制学习色调阈值处的色度间隙。

Conclusion: 色调分割的模型树方法能够有效捕捉相机间色调特异性颜色差异，而路径混合和边界原型正则化是消除硬分割伪影、实现边界连续映射的关键，兼顾了精度与视觉连续性。
...
### [67] [SinoDiff: Physics-Consistent Self-Supervised Diffusion for Unified Low-Dose to Standard-Dose PET Sinogram Recovery](https://arxiv.org/abs/2608.11514v1)
*Ghulam Nabi Ahmad Hassan Yar,Himashi Peiris,Sharna Jamadar,Alex Fornito,Zhaolin Chen*

Main category: eess.IV

TL;DR: SinoDiff是一个全新的自监督、物理一致的扩散框架，用于跨多个预定义剂量水平的PET正弦图恢复。通过将PET采集模型（泊松细化）集成到正向扩散过程中，实现物理一致的剂量依赖计数统计建模；在逆向过程中估计预定义剂量水平间的PET信号增量，从而无需跨剂量水平重新训练。在[18F]-FDG和[18F]-FDOPA数据集上达到了与监督和自监督基线相当的竞争性能。


<details>
  <summary>Details</summary>
Motivation: 低剂量PET减少辐射暴露但图像质量差，影响诊断信心。现有监督方法跨剂量泛化能力差且需要配对数据；自监督方法虽灵活但结果较差（丢失解剖细节、过度平滑病理特征）。这些局限性严重阻碍实际应用。

Method: SinoDiff提出一个自监督、物理一致的扩散框架：(1) 将PET采集模型通过泊松细化（Poisson thinning）集成到正向扩散过程，替代传统扩散中的噪声模拟，实现物理一致的剂量依赖计数统计建模；(2) 逆向扩散过程中估计预定义剂量水平间PET信号的增量变化，使得单一统一模型无需重新训练即可处理多个剂量水平；(3) 引入频率域卷积以捕获投影角度与探测器bin之间的长程依赖关系，适配PET正弦图的特性。

Result: 在[18F]-FDG和[18F]-FDOPA两个PET数据集上，SinoDiff在多个剂量水平下达到了与监督和自监督基线相当的竞争性能，同时保持了物理一致性和无需重训练的灵活性。

Conclusion: SinoDiff通过将PET采集物理模型融入扩散过程，成功克服了现有LD-PET恢复方法在跨剂量泛化、配对数据依赖和自监督性能不佳等方面的局限性，提供了一个统一、物理自洽且无需重训练的多剂量PET正弦图恢复方案。
...
### [68] [Uncertainty-Aware Compositional Localization and Placement Assessment of Catheters and Tubes in Chest X-Rays](https://arxiv.org/abs/2608.11288v1)
*Harshil Lodhiya*

Main category: eess.IV

TL;DR: UCompCXR是一个成分组合式框架，用于在胸部X光片上检测和评估导管与管路放置情况。它通过检测局部导管碎片、图聚类将其关联为设备实例、精度加权高斯估计融合尖端预测，并对每个设备进行分类放置评估。在RANZCR CLiP数据集上，相比共享相同MobileNetV3骨干的多任务基线，设备检测率提升26%，假阳性减少75%，尖端不确定性校准良好（95%覆盖率=0.948），且仅2.27M参数，可部署于资源受限的临床硬件。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法在评估胸部X光片导管放置时存在局限：全局分类方法无法定位具体设备位置，而单掩膜分割方法在导管重叠时无法进行逐设备评估。作者旨在解决这一安全关键但繁琐易错的临床任务中"设备级"评估能力缺失的问题。

Method: 提出UCompCXR成分组合式框架，包含四个步骤：(1) 检测局部导管碎片；(2) 通过基于图的聚类将碎片关联为设备实例；(3) 通过精度加权高斯估计融合每个碎片的尖端预测；(4) 对每个设备实例进行放置分类评估。

Result: 在RANZCR CLiP数据集（30,083张图像，5折患者级交叉验证并带bootstrap置信区间）上，UCompCXR相比共享MobileNetV3骨干的强多任务基线：检测出多26%的设备，假阳性减少75%，尖端不确定性校准良好（95%覆盖率=0.948）。聚合尖端误差有所上升——但仅因为模型发现了基线完全遗漏的设备（尤其是鼻胃管）。在匹配设备上，灾难性定位失败大幅减少。模型仅2.27M参数，单次前向传播即可部署。

Conclusion: UCompCXR通过成分组合的方式实现了逐设备级的导管放置评估，在检测能力和定位可靠性上显著优于多任务基线，同时保持轻量级架构适合临床硬件部署。尽管聚合误差看似升高，实则是检测更全面的副产品，具有实际临床应用价值。
...
### [69] [Uncertainty-Aware and Explainable Ensemble Deep Learning Framework for Multi-Class Skin Lesion Classification](https://arxiv.org/abs/2608.11280v1)
*Rofiqul Islam,Lilatul Ferdouse*

Main category: eess.IV

TL;DR: 本文提出一种结合不确定性感知与可解释性的深度学习框架用于多类皮肤病变分类，通过深度集成MaxViT-Tiny、ConvNeXt-Tiny和EfficientNetV2-B0三种模型，并利用MC Dropout估计预测不确定性、Grad-CAM++提供可视化解释。在HAM10000数据集上，在不确定性过滤条件下（熵<1.0，置信度>=0.7）达到96%准确率和99% ROC-AUC，宏平均精确率、召回率和F1分别为94%、95%和95%。


<details>
  <summary>Details</summary>
Motivation: 皮肤镜图像中的皮肤癌诊断面临类内变异大、类间相似性高、类别不平衡以及深度学习模型可解释性有限等挑战，因此需要更准确且可解释的诊断方法。

Method: 采用深度集成学习框架，组合视觉Transformer模型（MaxViT-Tiny）与CNN模型（ConvNeXt-Tiny、EfficientNetV2-B0）进行多类皮肤病变分类；使用MC Dropout估计预测不确定性以识别不可靠预测；采用Grad-CAM++ XAI技术突出影响模型决策的病灶区域，提供视觉解释。

Result: 在HAM10000数据集上，经过不确定性感知过滤（熵<1.0，置信度>=0.7）后，框架实现96%准确率和99% ROC-AUC，宏平均精确率、召回率、F1分别为94%、95%、95%，三个指标的加权平均均达96%。

Conclusion: 该框架实现了准确、可解释且具有不确定性感知能力的皮肤病变分类，为可信赖的计算机辅助诊断提供了有力支撑。
...
### [70] [Geometry-Based Compression of Plenoptic Point Clouds](https://arxiv.org/abs/2608.11273v1)
*Davi R. Freitas,Gustavo L. Sandri,Ricardo L. de Queiroz*

Main category: eess.IV

TL;DR: 本文提出一种基于Karhunen-Loève变换的多平面点云(PPC)属性压缩方法，将KLT应用于颜色属性后接多个带帧内预测能力的属性编码器，可集成到MPEG G-PCC标准中，实验结果显示其性能优于RAHT和基于视频的PCC方法，作者声称达到了新的最优水平。


<details>
  <summary>Details</summary>
Motivation: 多平面点云(PPC)作为一种新型数据结构，通过为每个点关联多个颜色来表示不同视角的光线，从而提供比普通点云更高的真实感。然而，这种多颜色属性带来了更大的数据量，需要高效的压缩方法来处理PPC的属性数据。

Method: 该方法首先对颜色属性进行Karhunen-Loève变换(KLT)以去除色彩间的相关性，然后使用多个具备帧内预测能力的属性编码器对变换后的系数进行编码。整个压缩方案可以嵌入MPEG的G-PCC标准框架中，并可复用G-PCC现有的任意属性编码方案。

Result: 在不同空间分辨率的PPC数据上进行压缩性能评估，结果表明该方法相比现有的RAHT和基于视频的PCC方案具有竞争力的性能表现。作者声称该编码器达到了当前最优水平。

Conclusion: 该工作表明KLT结合多个预测型属性编码器能够有效压缩PPC属性数据，并且可以与G-PCC标准兼容，为PPC压缩提供了新的最优方案。
...
### [71] [Hardware-Aware Deployment of Joint SAR Compression and Despeckling on FPGA](https://arxiv.org/abs/2608.11271v1)
*Cédric Léonard,Francescopaolo Sica,Martin Schulz*

Main category: eess.IV

TL;DR: 本文首次将联合SAR去斑与数据压缩（DDC）框架部署到基于ZCU102的嵌入式FPGA平台上，验证了其在星载系统严格功耗和计算约束下的可行性。研究发现ReLU替代GDN激活函数能提升SAR图像质量、残差块性价比低、FPGA是测试平台中能效最优的。


<details>
  <summary>Details</summary>
Motivation: 下一代SAR任务产生的数据远超下行链路传输能力，星载数据压缩对近实时对地观测至关重要。虽然已有工作表明联合去斑与压缩的LIC方法优于传统编解码器，但这些方法尚未经过星载系统严格功耗、计算和操作约束的验证，存在从算法到实际部署的鸿沟。

Method: 将联合SAR去斑和数据压缩（DDC）框架部署于基于ZCU102 FPGA的嵌入式平台，针对加速器的定点算术和有限支持操作集进行模型适配。评估四种模型拓扑结构，跨精度级别以及CPU、GPU、FPGA三种平台进行对比实验。

Result: 发现用ReLU替代传统GDN激活函数在SAR图像上提升了质量；残差块以十倍计算量仅带来极少的表示能力增益；FPGA在测试平台中能效最高。

Conclusion: 建立了可用的边缘部署工作流程，为星载SAR压缩提供了基于证据的设计起点，表明面向自然图像压缩的设计原则不一定能迁移到SAR图像场景。
...
### [72] [How to Spend Your Oracle Budget: Practical Guidance for Protein Structure Prediction Models](https://arxiv.org/abs/2608.12192v1)
*Aleksandra Kalisz,Jack Simons,Krisztina Sinkovics,Noam Ghenassia,Shikha Surana,Henry Moss,Paul Duckworth*

Main category: cs.AI

TL;DR: 本文首次系统性地对比了多种oracle预算感知的蛋白质结构预测指导方法（FK-steering、DPO、Best K-of-N和O3），在calmodulin和9EEH两个蛋白质靶标上评估发现：没有任何单一方法在所有预算和oracle条件下始终占优——O3在低预算下最有效，而FK-steering和DPO在预算增加时表现更好，并据此为实践者提供了可操作的建议。


<details>
  <summary>Details</summary>
Motivation: 蛋白质结构预测的基础模型在部分靶标上仍不可靠，而外部生物oracle可以标注和纠正这些失败，但生物oracle成本高昂，因此oracle预算成为关键约束。已有的多种指导方法（FK-steering、DPO、Best K-of-N）在预算消耗策略上各不相同，但缺乏系统性对比来指导方法选择。

Method: 将最近提出的Optimisation Over Outputs (O3)方法（在生成模型的潜在子空间内应用现成优化器）扩展到蛋白质结构预测模型，并与FK-steering、DPO和Best K-of-N三种现有方法一起进行基准测试，在两个蛋白质靶标（1CLL和9EEH）上使用不同oracle预算进行评估。

Result: 实验结果显示没有单一方法在所有oracle预算和oracle条件下始终占据主导地位。O3在低oracle预算下表现最优，而FK-steering和DPO在预算增加后展现出更好的性能。该工作是首个针对oracle预算感知指导的实用参考基准。

Conclusion: 研究结果被提炼为面向实践者的可操作建议，帮助他们在真实世界oracle预算约束下选择最合适的指导方法，填补了该领域缺乏系统性方法对比的空白。
...
### [73] [Mechanist: AI as a Scientific Instrument for Discovering the Mechanisms of Intelligence](https://arxiv.org/abs/2608.12036v1)
...
TL;DR: 本文介绍了Mechanist，一个利用AI作为科学仪器来自动发现AI模型内部机制的智能体系统。该系统整合了约13,000篇可解释性论文的知识图谱和覆盖26个领域、4,300万篇论文的多学科数据库，并配备了32种机理分析方法库。实验表明Mechanist在生成机制假说和执行实验方面优于Claude Code和现有AI科学家系统，并成功实现了从发现模型行为到解释和控制系统性AI的完整进展，包括发现跨模态安全风险迁移、建立信念的机制理论，以及将机制洞见转化为实际干预措施。


<details>
  <summary>Details</summary>
Motivation: AI模型的能力和风险机制仍未被充分理解，而随着AI开发速度加快和自动化程度提高，机制探索仍主要依赖人工，导致模型能力与人类理解和控制能力之间的差距不断扩大。

Method: Mechanist是一个智能体系统，构建了包含约13,000篇论文的可解释性知识图谱，并集成了涵盖26个领域、4,300万篇论文的多学科数据库。系统还配备了32种用于机制分析、因果干预和验证的基础方法库，以支持自主的机制发现过程。

Result: 与Claude Code和现有AI科学家系统相比，Mechanist生成了更有价值的机制假说并更可靠地执行实验。具体成果包括：发现不安全特征可通过看似安全的数据跨模态迁移的安全风险；建立了关于模型如何表示世界知识、形成信念、推断他人信念及其预训练期间涌现机制的信念理论；并将机制洞见转化为实际干预，提升模型在多种场景下的性能，引导科学基础模型生成具有指定属性的DNA序列。

Conclusion: Mechanist展示了从发现模型行为到解释和控制系统性AI的完整能力进展，证明了以AI作为科学仪器进行自主机制探索的可行性和潜力，为缩小AI能力与可理解性、可控性之间的差距提供了有效路径。
...
### [74] [HUGIN: Enhancing Vision-Language Planning for Autonomous Logistics Sorting](https://arxiv.org/abs/2608.11692v1)
*Xikai Sun,Cangtian Zhou,Kebin Liu,Ke Ma,Xu Wang,Zaishu Chen,Haotian Wang,Li Liu,Yunhao Liu*

Main category: cs.AI

TL;DR: 本文提出HUGIN训练框架以应对自主物流分拣系统(JMSU)中跨场景视觉理解与任务规划的挑战，通过内源数据增强和全局上下文排序两大组件提升VLM性能，并在自主构建的SortingBench基准上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 自主物流分拣系统(ALSS)是具身AI的重要工业应用，需要跨多个空间分离的摄像机视角进行联合规划。现有VLM直接应用于此面临两个核心挑战：跨场景监督信号稀缺，以及长视觉上下文导致的注意力分散。因此需要一个专门的训练框架来解决这些问题。

Method: 提出HUGIN训练框架，包含两个互补组件：(1) 内源数据增强(Endogenous Data Augmentation)——在操作约束下重组已验证的原子事实生成训练数据；(2) 全局上下文排序(Global Context Ranking)——使指令表征与完整视觉上下文的对齐强于与部分视觉上下文的对齐。此外构建了SortingBench数据集，覆盖四种自主物流分拣系统布局。

Result: 在五个开源VLM上，HUGIN一致优于匹配的基线。例如Qwen3-VL-8B在SortingBench上的准确率从63.6%提升至78.8%。消融实验验证了各组件的有效性，JMSU在具身任务中展现出溢出效益。超过15000个包裹的部署测试支持了基于VLM规划的实际可行性。

Conclusion: HUGIN框架有效解决了JMSU场景下VLM应用的两大难题，通过数据增强与上下文对齐机制显著提升了多场景联合理解与规划能力，并经大规模部署验证了其在工业自主物流分拣系统中的实用价值。
...
### [75] [AgenticTwin: An Agentic LLM Framework Integrated with Digital Twin for Anomaly Detection](https://arxiv.org/abs/2608.11679v1)
*Touseef Hasan,Mounika Ghanta,Souvika Sarkar,Ujjwal Guin*

Main category: cs.AI

TL;DR: 本文提出AgenticTwin，一个将LLM驱动的推理与基于数字孪生的异常检测流水线相结合的智能体框架，使操作员能够通过自然语言查询理解复杂系统中的异常。作者还在真实天气传感器数据上构建了基准评估流水线，并通过实验证明结构化智能体协作和知识接地推理能提升诊断质量、上下文检索和缓解质量。


<details>
  <summary>Details</summary>
Motivation: 数字孪生虽广泛用于监控信息物理系统，但即使由熟练操作员操作，解释数字孪生流水线中检测到的异常仍然极具挑战性，因为原始传感器数据的复杂性和体量使得深入分析困难。尽管LLM在推理和解释方面展现出潜力，但其在数字孪生驱动的异常分析中的集成尚未得到充分探索。

Method: (1) 提出AgenticTwin，一个集成LLM推理与数字孪生异常检测流水线的智能体框架，将LLM生成的解释锚定在数字孪生驱动异常分类器的输出上，并支持操作员以自然语言提问。(2) 构建基于基准的评估流水线，在真实世界天气传感器数据上注入合成异常，实现异常事件上操作员查询的受控生成。(3) 评估在实际信息物理环境中部署轻量级、开源LLM的可行性。

Result: 实验结果表明，结构化智能体协作与知识接地推理在不同可能异常场景下均能改善诊断质量、上下文检索效果以及缓解质量。

Conclusion: AgenticTwin证明了LLM驱动的智能体框架可以有效集成到数字孪生异常检测流程中，通过知识接地和智能体协作提升异常诊断与缓解能力，且轻量级开源LLM具备在该类实际环境中部署的可行性。
...
### [76] [CLAIM: Leading Open-domain Active Clarification of Large Language Models with Uncertainty Measurement](https://arxiv.org/abs/2608.11631v1)
*Kuangzhao Yang,Ziliang Zhao,Zhicheng Dou*

Main category: cs.AI

TL;DR: 提出了CLAIM框架，一种不确定性驱动的主动澄清学习框架，通过量化查询的不确定性（基于多模型答案分歧的熵）自动生成高质量合成数据，无需人工偏好标注，结合监督学习和强化学习训练统一的澄清决策模型，在开放域人机交互场景中实现低成本、稳健的澄清策略。


<details>
  <summary>Details</summary>
Motivation: 开放域人机交互中，LLM常遇到模糊或不完整的用户查询，直接回答会产生过度泛化、错误或低信息量的响应，而提出澄清问题能显著提升交互质量。但现有方法依赖人工标注数据或偏好对齐来解决两个核心挑战（何时需要澄清、澄清哪个方面），标注成本高且泛化能力受限。

Method: CLAIM通过多模型答案分歧计算熵来量化查询不确定性，消除对显式人工偏好标注的依赖；提出熵驱动的合成数据生成管线，结合熵不确定性估计、语义聚类和基于推理的判断实现自动标注；将澄清过程建模为结构化决策生成问题，采用SFT与GRPO相结合的混合训练范式。

Result: 实验结果表明，CLAIM无需依赖人工标注数据即可学习稳定且可泛化的澄清策略，为开放域LLM交互中的主动理解提供了低成本且鲁棒的解决方案。

Conclusion: CLAIM验证了不确定性驱动的方法可以替代人工标注来实现主动澄清学习，有效解决澄清的时机选择和方面选择两大挑战，在降低成本的同时保证策略的稳定性和泛化性，为真实开放域场景中的LLM交互提供了实用且可扩展的框架。
...
### [77] [MBA: Multimodal Benchmark and Agents for Real-World Business Ideation](https://arxiv.org/abs/2608.11616v1)
*Hojun Choi,Jaeyo Shin,Suin Lee,Hyunjung Shim*

Main category: cs.AI

TL;DR: 本文提出MBA-Bench，这是首个用于训练和评估商业创意智能体（business ideation agents）的多模态基准，包含30K样本、六个领域。作者还提出了两个经LoRA-SFT和GRPO训练的智能体MBA-b（盲评）和MBA-k（已知标准），分别通过创造性与可行性奖励优化，在评测中大幅超越纯文本和多模态基线。


<details>
  <summary>Details</summary>
Motivation: 现有的基于大语言模型的智能体系统在商业创意生成方面仍局限在纯文本范式，而真实世界场景本质上是多模态的（包含视觉线索），文本无法完整传达这些信息。因此需要一个多模态基准来更好地训练和评估商业创意智能体。

Method: （1）构建MBA-Bench：自动为图像生成标题，用GPT-4o通过检索查询生成、市场证据检索和证据增强合成三步生成每个商业问题的5个参考创意。（2）提出两个智能体：MBA-b（评判标准对智能体隐藏）和MBA-k（标准已知），分别用创造性和可行性两个奖励目标训练，MBA-k额外优化六个披露标准共八个目标。（3）训练流程：LoRA监督微调 + 分组相对策略优化（GRPO）及设定特定的奖励函数。（4）评估：使用MLLM-as-a-Judge按六个商业导向标准评测。

Result: MBA-b和MBA-k相比纯文本标题基线分别提升63.9%和77.1%，相比多模态基线分别提升25.6%和35.8%；多模态基线在某些指标上接近闭源模型表现。

Conclusion: 多模态信息对商业创意生成至关重要，通过针对性的奖励设计和策略优化，专用智能体在商业创意任务上显著超越通用基线，验证了多模态基准和任务定制训练的有效性。
...
### [78] [Foresight Without Seeing: Latent Futures for World Action Models](https://arxiv.org/abs/2608.11605v1)
*Jiakai Huang,Zhongbo Wu,Zheng Zhang,Zihan Wang,Shan You,Tao Huang*

Main category: cs.AI

TL;DR: 本文提出ForeWAM，一种动力学条件的直接策略世界行动模型（WAM），通过Future-KV机制在不解码未来视频的情况下，将预测性动力学上下文暴露给Action DiT，实现高效的动作生成，在LIBERO基准上达到96.7%/96.9%（标准/加速变体）的平均成功率。


<details>
  <summary>Details</summary>
Motivation: 现有世界行动模型在预测动力学暴露方式上存在权衡：显式未来WAM虽能提供直接的场景演变预测，但迭代视频去噪带来高昂推理成本；而直接策略WAM虽然动作预测高效，但缺乏显式推理时接口来将预测动力学暴露给Action DiT。作者旨在弥合这一差距，在保持动作生成效率的同时仍能为动作路径提供预测性动力学上下文。

Method: ForeWAM的核心是Future-KV机制：对当前视觉潜变量和随机未来槽执行单次Video DiT预填充，并在整个动作去噪过程中复用得到的逐层键值状态。同时引入由冻结的潜在动作教师监督的动力学寄存器，促使隐式未来状态捕获交互引发的转变（如物体运动、接触变化和任务进度）。真值未来观测和教师仅在训练阶段使用，部署时无需两者且不生成未来视频。

Result: 无需具身机器人数据预训练，ForeWAM标准变体和加速变体在LIBERO上分别达到96.7%和96.9%的平均成功率；标准变体在LIBERO-Plus上取得61.6%的成功率。

Conclusion: 直接策略WAM可以在保持高效动作预测的同时，将预测性动力学暴露给动作路径，而无需显式生成未来观测，从而弥合了高效推理与动力学可解释性之间的鸿沟。
...
### [79] [Learning from Online User Feedback for Shopping Agents](https://arxiv.org/abs/2608.11604v1)
*Haobo Zhang,Kelong Mao,Sulong Xu,Simiu Gu,Zhicheng Dou*

Main category: cs.AI

TL;DR: LOFA是一个从真实在线电商交互日志中学习购物代理的框架，结合基于可验证购买结果的强化学习与反馈感知的在线蒸馏，无需人工标注即可将用户对话反馈转化为密集的token级监督信号，显著提升推荐质量和用户满意度。


<details>
  <summary>Details</summary>
Motivation: 现有购物代理的训练主要依赖离线信号（如用户-商品交互或合成偏好数据），忽视了用户自然对话反馈中蕴含的丰富监督信息。同时在线反馈具有异质性、稀疏性和噪声大的特点，难以自动转化为可靠的学习信号。这两方面挑战促使了LOFA的提出。

Method: LOFA包含两个互补的训练目标：(1) 基于可验证购买结果的强化学习，捕获协作行为模式；(2) 反馈感知的在线蒸馏，识别用户对话中的指令并将其转换为密集的token级监督，捕捉用户特定偏好。

Result: 在真实电商日志上的大量实验表明，LOFA在推荐质量、响应帮助性和用户满意度对齐方面均一致性地优于强基线方法。

Conclusion: LOFA证实了从真实在线用户反馈中学习购物代理的有效性，为无需人工标注即可利用在线交互日志改进购物代理提供了可行的框架。
...
### [80] [From Prompting to Behavioral Alignment: Personalized LLM Judges for Recommendation Evaluation](https://arxiv.org/abs/2608.11493v1)
*Alireza S. Ziabari,Kat Ellis,Colleen Chan,Ding Tong*

Main category: cs.AI

TL;DR: 本文揭示了LLM在零样本场景下用于离线推荐评估时存在的"双向合理化"失效模式——即LLM对同一物品的相同证据既能论证正向也能论证负向用户参与结果。为此提出顺序行为对齐框架，结合微调与偏好优化，在真实首页交互日志上相比零样本基线提升32.19%的Macro-F1，且匹配生产级特征工程基线。


<details>
  <summary>Details</summary>
Motivation: 传统离线推荐评估依赖复杂且难以扩展的人工维护特征管线。LLM虽可通过原始文本日志直接预测用户参与度，但实证发现其存在"双向合理化"这一关键失效模式，即零样本LLM对相同物品和相同证据既能信服地论证正向结果也能论证负向结果，导致开箱即用的LLM在用户参与度预测上不可靠。

Method: 提出顺序行为对齐框架：将微调与偏好优化相结合，在成对的正确理由与反事实理由上进行训练。通过行为对齐使模型学习区分有效与无效的推理路径，从而抑制双向合理化。

Result: 在真实首页交互日志上评估，对齐推理方法相比零样本基线在Macro-F1上实现32.19%的提升，并匹配生产级特征工程基线的性能。

Conclusion: 行为对齐能够缓解双向合理化问题，同时交付人类可解释的推理轨迹，且无需人工管线开销，为LLM替代传统推荐评估管线提供了可行路径。
...
### [81] [Inverse Theory of Mind Modeling for Content Recommendation: From Web Browsing to Dynamic Intelligent Interfaces](https://arxiv.org/abs/2608.11354v1)
*Mengyu Chen,Feiyu Lu,Chun-Fu Chen,Lucas Vinh Tran,Jay Katukuri*

Main category: cs.AI

TL;DR: 本文提出一种逆心智理论（Inverse Theory of Mind, IToM）pipeline，通过从观察到的交互行为反向推断用户的信念、偏好和决策特质，生成结构化用户画像。在OPeRA数据集上，该方法在四项任务上匹配或超过真实用户画像，并展示了跨模态（VisionOS空间银行应用）的可迁移性。


<details>
  <summary>Details</summary>
Motivation: 现代推荐系统将观察到的行为视为用户偏好的可靠代理，但交互往往反映探索或比较而非稳定的偏好表达。随着界面从静态布局向生成式UI和沉浸式扩展现实（XR）演变，自适应环境不仅需要决定展示什么，还需要决定在哪里、何时、以何种显著性展示，以及最关键的是用户为何行动。因此需要更深层、跨模态的用户理解。

Method: 提出IToM pipeline，包含三个核心步骤：(1) 重建每个用户的决策上下文，包括被选项和可用备选项；(2) 应用LLM驱动的反事实推理，生成基于证据的自然语言信念陈述；(3) 通过多假设溯因推理综合这些信念，形成结构化用户画像。

Result: 在OPeRA数据集上，基于四类真实标注（人格评估、态度调查、访谈画像）进行四项任务评估（下一动作预测、购物态度对齐、大五人格推断、留出类别预测）。结果显示推断画像匹配或超过真实画像，且多假设推理对准确的人格预测至关重要。此外展示了在VisionOS上基于画像驱动的空间银行应用的跨模态可迁移性。

Conclusion: 从交互行为进行逆心智推理能够生成与真实用户画像相当甚至更好的用户理解，多假设推理机制是捕获人格复杂性的关键，且该方法具有跨模态可迁移性，适用于生成式UI和XR等新兴交互界面。
...
### [82] [Apodex Discovery: Reality Benchmarks and Environments for Evaluating and Building Discoverative Artificial Intelligence](https://arxiv.org/abs/2608.11341v1)
...
TL;DR: 本文提出Apodex Discovery框架，用于构建和评估"发现型AI"（discoverative AI）。该框架包含三个核心组件：问题侦察流程（调研561个行业、420余个高价值问题并筛选出20个）、统一的环境-任务-片段抽象（提供数据、工具、反馈、轨迹记录与验证），以及独立于最终任务成功率的HDS6评估体系（覆盖工具、修复、替代方案、一致性、证据和范围六个维度）。实验表明其在AAV衣壳设计和药物重定位/再配方任务上均超越当前最优水平。


<details>
  <summary>Details</summary>
Motivation: 与阿波罗登月计划相似，当今AI虽然能在问题、工具和成功标准被明确指定时解决困难任务，但现实世界中的重大挑战很少以可执行或可验证的形式呈现在AI面前。因此需要将AI评估从预定义基准测试扩展到面向真实发现的、可验证的调查性任务上。

Method: Apodex Discovery框架的核心是"重型求解器"（HDS6），由基础模型、操作台（harness）、工具和控制策略组成。三个核心组件分别是：(1) 问题侦察：系统化扫描561个行业/16个部门，汇编423个高价值现实问题并筛选20个用于初始发布；(2) 统一环境-任务-片段（EPISODE）抽象：提供数据、工具、约束、反馈、轨迹记录及中间产物和最终提交的验证；(3) HDS6评估体系：独立于最终任务成功，分别评估工具使用、修复能力、替代方案、一致性、证据和范围六个维度。

Result: 在AAV衣壳设计中，Apodex在存活性、趋向性、结构预测和生成设计方面比已发表的当前最优结果高出7%。在药物重定位与再配方任务中，专用的生物医学环境使GPT-5.5和GPT-5.6-sol的平均归一化预测得分比相同闭卷基础模型分别提升2.5和7.6个点。受控消融实验表明，固定的TRACES片段接口可以将性能差异归因于特定的求解器组件。

Conclusion: Apodex Discovery标志着AI评估范式从预定义基准测试（predefined benchmarks）向可验证调查（verifiable investigations）和真正科学发现的转变，为AI在实际科学与工业问题上的发现能力提供了系统化的构建与评估框架。
...
### [83] [Glance, Scrutinize, and Think: Advancing Video Anomaly Detection from Training-Free to Agentic Reasoning](https://arxiv.org/abs/2608.11260v1)
*Shibo Gao,Peipei Yang,Xu-Yao Zhang,Linlin Huang*

Main category: cs.AI

TL;DR: 本文提出了一种全局到局部的推理范式来解决视频异常检测(VAD)中"何时"与"什么"的割裂问题。作者先提出GtS（先概览后细查），一个无需训练的粗到细异常定位与理解框架；进一步提出工具增强的智能体VAD方法，让多模态大语言模型学会调用视频裁剪工具、检查密集重采样帧并进行自纠正。同时扩展了VAGU基准为VAGU-T（含7,567个真实视频、21类异常），并引入联合评估语义可解释性与时间精度的指标JeAUG。


<details>
  <summary>Details</summary>
Motivation: 现有VAD方法存在"when-what"割裂：传统DNN方法能定位异常发生的时间但缺乏语义理解，而LLM方法能解释异常内容但忽略精确的时间定位。作者将此归因于缺乏统一的推理范式，受人类审查监控视频的方式启发（全局浏览形成时间假设→细查可疑片段→迭代思考纠正错误），希望弥合时间定位与语义理解之间的鸿沟。

Method: 本文提出两条技术路线：(1) GtS——无需训练的框架，利用静态与动态文本引导实现粗到细的异常定位与理解，兼顾精度与速度；(2) 工具增强的智能体VAD方法——多模态大语言模型学习调用视频裁剪工具、检查密集重采样帧、自我纠正错位假设，通过冷启动监督微调加联合回答-定位奖励的强化学习训练。同时构建VAGU-T基准（含人工验证的grounding、解释、QA对及思维链工具调用轨迹）并提出JeAUG联合评估指标。

Result: 实验表明GtS大幅超越无需训练的基线方法；智能体模型在获得更高准确率的同时推理速度更快。VAGU-T基准和JeAUG指标为该领域提供了新的评测标准。

Conclusion: 本文通过模拟人类"先概览后细查"的监控视频检查方式，建立了统一的全局到局部推理范式，有效解决了VAD中时间定位与语义理解割裂的问题，并提供了更全面的基准和评估指标，为后续研究奠定了基础。
...
### [84] [Towards Terabit/$λ$/s Multidimensional Silicon Photonic Engine](https://arxiv.org/abs/2608.11639v1)
*Hao Chen,Zengqi Chen,Wu Zhou,Kaihang Lu,Mingyuan Zhang,Yuxiang Yin,Yiou Cui,Chaoran Huang,Pui-In Mak,Yeyu Tong*

Main category: physics.optics

TL;DR: 本文提出一种多维硅光子引擎，通过单片集成收发器、空间与偏振（解）复用器及光信号处理器，实现超过1.8 Tbit/s/λ/s的通信容量，免除了体积庞大的分立（解）复用器和功耗高的DSP，相对DSP在功耗和处理时延上实现超5000倍降低，并演示了300米光纤上的全双工、调制格式透明芯片间通信。


<details>
  <summary>Details</summary>
Motivation: AI工作负载激增推动共封装光学（CPO）发展，需要光学互连在超紧凑形态因子内实现更高的光并行度，同时保持严格的能效和时延约束。传统方案依赖庞大的分立（解）复用器和功耗高的DSP，难以满足AI工厂扩展需求。

Method: 提出一种多维硅光子引擎，在单片上集成收发器、空间与偏振（解）复用器和光信号处理器，替代分立（解）复用器与数字信号处理（DSP）。该引擎可自配置识别每根光纤上2、4或6个并发空间/偏振通道，并缓解动态通道串扰。实验在MIMO处理阶数为6时，与最先进DSP相比，实现功耗和处理时延均降低超过5000倍。

Result: 该光子引擎实现超过1.8 Tbit/s/λ/s的通信容量；相比DSP在MIMO阶数6时功耗和处理时延均降低超5000倍；演示了300米光纤上的全双工、调制格式透明芯片间通信。

Conclusion: 该工作代表了未来高性能计算和AI数据中心光学引擎的范式转变，展示了全光多维复用处理替代电子DSP的巨大潜力。
...
### [85] [CookVoice: Unified Framework for Style Controllable Multi-Modal Human Voice Generation](https://arxiv.org/abs/2608.11590v1)
*Haowei Lou,Hye-Young Paik,Dai Jia,Kai Li,Lina Yao*

Main category: cs.SD

TL;DR: CookVoice是一个统一的多模态、多风格、多任务人类语音生成框架，将人声分解为内容、韵律和风格三个关键因素，通过灵活的帧级对齐策略支持语音合成、歌声合成、风格可控生成、声音模仿、声音转换和声音编辑等多种任务，仅需4351万参数和最少4个ODE步即可实现与大规模基线相当的生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有语音生成系统大多针对特定任务设计，依赖任务特定的架构、控制信号或自回归解码，导致细粒度可控性和推理效率受限。作者希望构建一个统一的框架，同时支持语音和歌声生成，并提供更强的可控性与高效的推理能力。

Method: CookVoice将人声分解为内容、韵律和风格三个关键因素，在统一模型中支持语音与歌声生成。设计了一种灵活的对齐策略，将文本、风格和韵律控制信号映射到频谱图的帧级别，从而实现对生成过程的精确和灵活控制，支持包括TTS、歌声合成、风格可控生成、声音模仿、声音转换和声音编辑在内的多种任务。

Result: 实验表明CookVoice实现了与现有TTS和歌声合成基线相当的生成质量，同时提供更强的风格和韵律可控性。仅以4351万参数和最少4个ODE步数的高效推理，即可达到与大规模基线相当的性能。

Conclusion: CookVoice通过统一的分解式建模和帧级对齐策略，在多种语音生成任务上实现了质量与可控性的平衡，并以轻量化的参数规模和高效的推断步骤展现了良好的实际应用潜力。
...
### [86] [MVFM-3DAD: Multi-view Flow Matching for 3D Anomaly Detection via Density Proxy Estimation](https://arxiv.org/abs/2608.12148v1)
*Liangwei Li,Lin Liu,Jing Zhang,Xiaohui Du,Ruqian Hao,Xinwei Li,Hanzhe Liang,Juanxiu Liu*

Main category: cs.GR

TL;DR: 提出MVFM-3DAD，一种基于流的3D异常检测框架，通过双向几何投影器将不规则点云转换为结构化多视图表示，并由流引导密度代理估计器评估各视图特征的参考密度，再利用逆向过程映射回3D点，以终端正常度识别异常特征，无需输入重建或显式雅可比计算。


<details>
  <summary>Details</summary>
Motivation: 现有3D异常检测方法主要依赖记忆库检索或重建：记忆库方法受限于存储正常特征的覆盖范围，重建方法可能学习身份捷径从而也能重建异常输入。这促使研究者探索基于密度的方法，直接评估测试样本是否符合学习到的正常数据分布。

Method: 提出MVFM-3DAD流式框架，包含两个核心组件：1) 双向几何投影器(BGP)，通过前向过程将不规则点云转换为结构化多视图表示；2) 流引导密度代理估计器(FDPE)，估计每个视图特征的参考密度。随后BGP的逆向过程将这些多视图密度估计映射回对应的3D点，通过终端正常度识别异常特征。该方法无需输入重建或显式雅可比计算。

Result: 在Real3D-AD和MVTec3D-AD基准上，MVFM-3DAD的性能优于最强的竞争方法，且由于无需重建或雅可比计算，异常评分机制简单高效。

Conclusion: 将3D异常检测重新定义为正常数据分布上的密度代理估计是有效的替代范式，避免了记忆库覆盖限制和重建身份捷径问题，同时实现了SOTA性能与高效率。
...
### [87] [ESVR: 3D Ellipsoid-based Sparse Volume Rendering via Structure-aware Primitive Learning and Per-primitive Ray Sampling](https://arxiv.org/abs/2608.05564v1)
*Suemin Jeon,Youjin Kim,Jungwoo Park,Kyungryun Lee,Won-Ki Jeong*

Main category: cs.GR

TL;DR: ESVR提出了一种基于椭球体基元的稀疏体数据直接渲染框架，直接从原始体素数据学习而非从DVR渲染图像中学习，支持灵活的传递函数控制和交互式探索，在大型稀疏数据集上实现高达四个数量级的压缩和43-223 FPS的实时渲染。


<details>
  <summary>Details</summary>
Motivation: 大规模稀疏体数据的有效表示与渲染是科学可视化的挑战性难题。直接体渲染(DVR)虽提供高质量可视化，但其计算和内存开销随数据规模增长而急剧扩大。现有3DGS方法虽然通过紧凑几何基元实现了高效渲染，但它们是从DVR渲染图像而非原始体数据中学习，导致信息丢失，并限制了交互式探索中传递函数的灵活控制能力。

Method: ESVR框架包含以下核心组件：(1)具有有界支撑的可微椭球体基元，直接在3D空间学习体数据；(2)结构感知的基元学习与互补剪枝策略；(3)逐基元光线采样策略，实现快速准确的传递函数映射；(4)基于块(chunk)的优化方案与ghost椭球体，在训练期间提供边界上下文以支持大规模数据集。

Result: 在大型稀疏数据集上，ESVR实现了高达四个数量级的压缩比，并达到43-223 FPS的实时渲染性能，同时保持具有竞争力的重建质量。

Conclusion: ESVR通过直接从原始体数据学习椭球体表示，克服了现有3DGS方法依赖DVR渲染图像而导致信息丢失和传递函数控制受限的问题，为大规模稀疏体数据的高效表示与交互式可视化提供了一种可行方案。
...
### [88] [Learning Loco-Manipulation From SMPC Demonstrations With Sparse Offline-to-Online RL](https://arxiv.org/abs/2608.12063v1)
*Martin Schuck,Maks Sorokin,Simone Manni,Duy Ta,Angela P. Schoellig,Marco Hutter,Simon Le Cleac'H,Jan Brüdigam*

Main category: cs.RO

TL;DR: 本文提出一种利用基于采样的模型预测控制（SMPC）在仿真中作为自动化专家生成大规模离线数据的方法，仅用稀疏任务奖励训练离策略强化学习智能体，从而绕过密集奖励塑形的瓶颈。该框架结合高层智能体与低层动态稳定性控制器，使学到的策略超越原始最优控制教师，并在不同形态机器人（Spot四足和G1人形）上验证了sim-to-real的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 机器人自主性要求移动和操作能力的整合，但标准强化学习（RL）在复杂任务上的扩展受限于密集奖励塑形的缓慢人工过程。作者旨在通过消除人工奖励设计的需求，大幅缩短新技能的学习时间。

Method: 1) 在仿真中使用基于采样的模型预测控制（SMPC）作为自动化、可快速调整的专家，生成大规模离线数据集；2) 利用该数据解决探索问题，用纯稀疏任务奖励训练离策略RL智能体；3) 将高层RL智能体与低层动态稳定性控制器集成；4) 在仿真到现实的框架中部署。

Result: 学到的策略能够超越原最优控制教师，产生严格符合真实任务目标的更优行为，并在不同形态的机器人（配备机械臂的Spot四足机器人和G1人形机器人）上成功部署复杂的移动-操作技能，验证了框架的鲁棒性。

Conclusion: 该框架通过SMPC生成数据替代人工奖励设计，显著降低强化学习训练时间，同时保持乃至超越最优控制器的性能，为跨形态的复杂移动-操作任务提供了可扩展的sim-to-real解决方案。
...
### [89] [Scalable Multi-Agent Maze Traversal with Local Communication](https://arxiv.org/abs/2608.11895v1)
*Julian Rau,Jahir Argote-Gerald,Grace McFassel,Genki Miyauchi,Paul Trodden,Roderich Groß*

Main category: cs.RO

TL;DR: 本文提出一种分布式多智能体导航算法，使智能体在未知、可能含环的图环境中集体遍历并定位目标，通过领导者-跟随者关系和领导者切换协调，仅单个智能体执行单智能体迷宫求解器进行探索。算法被证明完备，其makespan在智能体数量上渐近等价于全知最优策略，并给出了时间和空间复杂度。


<details>
  <summary>Details</summary>
Motivation: 洞穴网络、管道系统等迷宫式环境对未知环境下的多智能体导航构成重大挑战，尤其是在通信受限情况下。现有方法难以同时兼顾未知环境探索、多智能体协调和避碰，需要一种在通信受限条件下仍能高效完成集体导航的分布式算法。

Method: 提出一种分布式算法：智能体在指定起始节点顺序进入未知可能含环的图，通过局部通信建立领导者-跟随者关系并支持领导者切换。任意时刻仅有一个智能体执行单智能体迷宫求解器进行探索，其余智能体通过跟随关系协同移动，避免碰撞并最终到达未公开的目标节点。

Result: 证明算法完备性，其makespan在智能体数量上渐近等价于具有全知信息的最优策略；推导了时间和空间复杂度。仿真实验最多支持625个智能体，结果显示随着智能体数量增加，平均总燃料消耗（sum-of-fuels）下降，且该方法优于所有智能体独立执行单智能体求解器的朴素基线方案。

Conclusion: 该分布式算法在未知迷宫环境中实现了多智能体的高效集体导航，理论上保证完备性和渐近最优性，仿真验证了其扩展性和相对于朴素基线方法的优势，适用于洞穴、管道系统等通信受限的迷宫式场景。
...
### [90] [D3D-GEN: Robot-Aware Domain-Grounded Interactive 3D World Generation for Social Robotics](https://arxiv.org/abs/2608.11876v1)
*Anh Duc Do,Volodymyr Scherbyna,Tai Duc Nguyen,Spaarsh Thakkar,Zhengcheng Shen,Teham Buiyan,Archan Misra,Linh Kästner*

Main category: cs.RO

TL;DR: 提出D3D-GEN，一个结合领域智能体与检索增强生成（RAG）管道的世界生成系统，能够快速生成领域基础、完全交互式的3D仿真世界，不依赖固定3D模型数据库，支持Isaac Sim和Gazebo等仿真器。


<details>
  <summary>Details</summary>
Motivation: 具身智能（Embodied AI）社交导航的训练和验证高度依赖逼真的仿真环境，但现有方法难以在真实性与可仿真性之间取得平衡，且多依赖固定模型数据库，限制了生成世界的多样性和泛化能力。

Method: D3D-GEN包含两部分：一是领域智能体，自动收集公开可访问的领域特定数据并构建持久化的领域数据库；二是基于该领域数据库的RAG管道，通过动态查询用户提供的可扩展语义数据库来生成合理的楼层平面图和物体布局。系统集成于本地Web前端，输出可直接加载到Isaac Sim和Gazebo的交互式3D世界。

Result: 已在多个常见领域（室内住宅、医院、办公室）构建了数据库，并为每个领域生成数十个不同的、合理的仿真环境。

Conclusion: D3D-GEN通过自动化的领域知识收集与RAG驱动的世界合成，实现了快速、灵活且无需固定3D模型数据库的交互式3D世界生成，为具身智能的社会导航仿真提供了高效解决方案。
...
### [91] [Enhancing Visual Domain Robustness in Behaviour Cloning via Saliency-Guided Augmentation](https://arxiv.org/abs/2608.11870v1)
*Zheyu Zhuang,Ruiyu Wang,Nils Ingelhag,Ville Kyrki,Danica Kragic*

Main category: cs.RO

TL;DR: 本文提出RoboSaGA，一种基于显著性引导的叠加式数据增强方法，用于视觉行为克隆。它通过策略驱动的显著性在像素级别动态调整增强强度，在任务无关区域进行激进增强，同时保留任务关键信息，以提升模型对视觉域偏移（阴影、干扰物、背景变化）的鲁棒性，且无需修改网络结构或增加额外学习目标。


<details>
  <summary>Details</summary>
Motivation: 视觉行为克隆中，传统图像增强（如随机裁剪、颜色抖动）在显著视觉域偏移下泛化能力不足；叠加式增强虽在通用视觉任务中有前景，但在行为克隆场景下需保留任务关键语义、时空关系及智能体-目标交互，直接套用存在风险，因此需要一种能区分任务相关与无关区域的增强方法。

Method: RoboSaGA属于叠加式增强家族，利用策略驱动的显著性图在像素级别动态调节增强强度：对任务无关区域施加强烈增强，对任务关键区域保持弱增强，从而在丰富视觉多样性的同时不破坏关键信息。该方法可无缝集成到现有行为克隆架构中，无需结构改动或额外学习目标。

Result: 在仿真和真实世界实验中，RoboSaGA在保持域内性能的同时，显著提升了对视觉域偏移的鲁棒性，包括干扰物、背景、光照与阴影变化等场景。

Conclusion: RoboSaGA验证了显著性引导的像素级自适应增强对视觉行为克隆域泛化的有效性，兼顾域内保真与域外鲁棒性，且具有良好的即插即用特性。
...
### [92] [G0.5: One Autoregressive Stream for Robot Reasoning and Action](https://arxiv.org/abs/2608.11739v1)
...
TL;DR: G0.5是一种预训练的回归式视觉-语言-动作（VLA）模型，通过单一Transformer解码器在统一目标下同时生成推理与动作token，替代了传统的"预训练VLM+独立流匹配动作专家"架构，在7项独立基准上超越SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通常将预训练VLM作为上下文编码器，动作生成由单独训练的动作专家完成，导致VLM并未真正参与决策。作者希望让VLM本身成为决策者，使预训练的语言能力直接迁移到物理行为控制。

Method: 提出三个关键组件：1）可学习的跨具身动作分词器（cross-embodiment action tokenizer），将异构机器人动作映射到共享词表；2）原生思维链流（native chain-of-thought stream），在动作token之间交织任务分解、目标接地与动作提示；3）视觉记忆模块（visual memory module），通过视觉编码器注入多秒历史信息。推理与动作共享同一套权重，在机器人数据集与VQA样本上联合预训练。

Result: 在7个独立任务上全面超越SOTA：R1lite/R1pro真实微调76.7%（对比π0.5的53.3%与GR00T-N1.7的24.4%）、2025 BEHAVIOR挑战31.4%（对比26.3%/26.1%）、DROID后训练零样本迁移82.5%、LIBERO 98.9%、RoboTwin 2.0 93.3%、SimplerEnv-Bridge 87.3%。提示可直接控制动作粒度、任务时域与OOD场景处理，无需额外训练。

Conclusion: G0.5证明了让VLM在单一目标下同时承担推理与动作生成的可行性，预训练VLM的语言推理能力可以有效迁移到物理决策中，且通过提示即可适应不同任务需求，为构建真正的端到端具身智能决策模型提供了新范式。
...
### [93] [Video2Track: From Real-World Interaction Videos to Steerable Adversarial Closed-Track Testing for Automated Driving Systems](https://arxiv.org/abs/2608.11592v1)
*Mengjie Tian,Xinrui Zhang,Tianyu Li,Peizhi Zhang,Guirong Zhou,Haojie Feng,Junpeng Huang,Qixiang Zhang,Lu Xiong*

Main category: cs.RO

TL;DR: 本文提出Video2Track框架，将从真实驾驶视频中提取的交互场景转移到可操控的对峙式封闭测试道测试中，通过视觉语言模型进行场景语义映射，并结合条件扩散模型和Stackelberg博弈生成可控风险与交互风格的多智能体轨迹，实现真实且可引导的自动驾驶系统验证。


<details>
  <summary>Details</summary>
Motivation: 现有封闭测试道测试大多依赖标准化协议或预定义轨迹，交互过于脚本化，难以复现真实公共道路交通的自然复杂性，缺乏将真实驾驶场景转移到封闭测试道进行可复现、可操控验证的有效方法。

Method: Video2Track框架包含两个紧密耦合的模块：（1）场景语义映射模块，利用视觉语言模型从驾驶视频中提取结构化语义，并通过检索增强生成将其映射到封闭测试道拓扑库中，识别兼容的地图片段和交互锚点；（2）动态交互测试模块，基于映射的拓扑和锚点，通过条件扩散模型生成多样化的多智能体轨迹，并利用带参数化对抗目标的Stackelberg博弈调控交互强度。

Result: 封闭测试道实验表明，该框架能够忠实复现具有代表性的真实世界交互场景，并生成具有可控风险等级和交互风格的可执行场景变体，为真实且可引导的自动驾驶系统验证提供了可扩展的方法。

Conclusion: Video2Track为自动驾驶系统验证提供了一种从真实视频到封闭测试道的可扩展场景转移框架，在保持场景真实性的同时实现了交互风险与风格的可操控性，填补了现有封闭测试方法在复杂交互场景再现方面的空白。
...
### [94] [RoadWeaver: Large-Scale Lane-Level HD Map Generation from Scratch for Autonomous Driving Simulation](https://arxiv.org/abs/2608.11580v1)
*Yueyuan Li,Zexi Chen,Weijie Xi,Mingyang Jiang,Songan Zhang,Hanyang Zhuang,Ming Yang*

Main category: cs.RO

TL;DR: 本文提出RoadWeaver，一个从零生成多样化、大规模车道级HD地图的粗到细框架。它先合成全局道路布局，扩展为连通道路网络，再构建具有拓扑一致性的车道级几何结构。实验表明其可达率99.8%、死端比例10.7%、端点对齐误差0.24 m，相比SOTA方法将端点对齐误差降低94.4%，且生成完整HD地图仅需1.39-3.50秒。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶仿真需要多样且可扩展的车道级HD地图以支持复杂路网上的长时程评估，但现有方法要么依赖手工或重建的真实地图（可扩展性受限），要么只能生成局部道路结构而非完整的HD地图。

Method: RoadWeaver采用粗到细（coarse-to-fine）的三阶段流程：首先合成全局道路布局，然后将其扩展为连通的道路网络，最后构建具有拓扑一致车道连通性的车道级几何结构。

Result: 可达率99.8%，死端比例10.7%，端点对齐误差0.24 m；相比SOTA生成方法，端点对齐误差降低94.4%，生成完整HD地图耗时仅1.39-3.50秒，可直接部署于驾驶仿真器。

Conclusion: RoadWeaver能够高效生成完整、大规模且拓扑一致的HD地图，为自动驾驶系统的闭环评估提供可扩展的仿真环境，代码与实现将随论文公开发布。
...
### [95] [Keep the Future, Drop the Rollout: RIFT for World Action Models](https://arxiv.org/abs/2608.11521v1)
*Chushan Zhang,Jinguang Tong,Xuesong Li,Yikai Wang,Hongdong Li*

Main category: cs.RO

TL;DR: 本文提出RIFT（Rollout-free Imagination via Future Tokens），通过学习的预测标记在一次主干网络前向传播中构造完整的未来K/V缓存，在不进行迭代视频生成的情况下实现未来条件化动作生成，在LIBERO和RoboTwin 2.0基准上达到接近或超越基于rollout基线方法的效果，同时将动作块延迟降低68.2%至89.1%。


<details>
  <summary>Details</summary>
Motivation: 世界动作模型（WAMs）依赖预测未来进行动作条件化，但迭代式视频rollout会引入显著的部署延迟。作者质疑动作生成是否必须依赖逐步展开的轨迹，还是仅需其未来表示即可。这一问题的答案将决定能否在设计部署阶段去除昂贵的迭代视频生成流程。

Method: 首先通过成对的闭环干预实验（掩码或重新指定future-cache值）验证未来缓存值及其位置对动作执行的影响。发现对于Joint和Cosmos-2模型，重放固定的最终干净K/V缓存几乎不改变执行（成功率达97.9%-98.2%），从而将缓存消费与缓存生产解耦。基于此洞察，提出RIFT框架，使用学习到的anticipation tokens在一次主干网络前向传播中构造完整未来K/V缓存，同时保留原始的future-read接口。

Result: 在全部40个LIBERO任务上，RIFT达到98.8%成功率，接近基于rollout的Joint、IDM和LingBot-VA（98.4%-98.6%），同时将动作块延迟降低68.2%-89.1%。在RoboTwin 2.0上，RIFT在干净/随机化场景分别达到92.9%/92.6%，为评估方法中的最高观测值。此外，干预实验表明Joint和Cosmos-2可以复用固定缓存但构造仍需rollout。

Conclusion: 未来条件化动作生成并不需要在部署时进行迭代视频rollout；通过将缓存消费与缓存生产分离，并利用学习到的anticipation tokens一次性构造完整的未来表示，可以在保持高成功率的同时大幅降低延迟。这为无rollout的未来条件化机器人控制提供了可行的新范式。
...
### [96] [Herding End-to-End Autonomous Driving via Neuro-Symbolic Safety Guards](https://arxiv.org/abs/2608.11451v1)
*Simón Patiño Idarraga,Erick Silva,Rehana Yasmin,Ali Shoker*

Main category: cs.RO

TL;DR: 本文提出一种神经符号安全护栏（neuro-symbolic safety guard），作为轻量级模块附加在已训练好的端到端驾驶代理的命令接口上。该模块在任何指令到达车辆前，用显式安全规则检查指令，必要时替换为最近的安全替代方案。无需重训练、无新增学习组件，每个干预都可执行且可追溯到触发它的规则。在Fail2Drive和Bench2Drive长尾基准上以TransFuser v6为案例，成功率达15%提升，安全关键碰撞减少最高53%，同时保持原有驾驶分数。


<details>
  <summary>Details</summary>
Motivation: 现代端到端驾驶代理虽然平均性能高，但仍会违反人类驾驶员绝不会忽略的基本交通规则。根本原因在于结构性缺陷：它们学习的是统计模式而非保证安全驾驶的物理条件，导致决策过程不透明且安全约束无法强制执行。

Method: 引入神经符号安全护栏：一个轻量级模块，附加到已训练代理的最终指令接口上。在指令到达车辆前，基于显式安全规则检查指令，仅在必要时用最近的安全替代方案替换。护栏无需重训练，不添加任何学习组件，每次干预都可执行且可追溯到触发规则。

Result: 在Fail2Drive和Bench2Drive长尾基准上以TransFuser v6（TFv6）作为案例验证：成功率提升15%，安全关键碰撞减少最多53%，同时保留原有驾驶分数。

Conclusion: 神经符号安全护栏通过将显式安全规则与学习型驾驶代理相结合，有效弥补了端到端模型在安全约束执行方面的结构性缺陷，在不牺牲驾驶性能的前提下显著提升了安全性和决策可解释性。
...
### [97] [Top-down Traffic Scenario Generation via Joint Initial-Goal Diffusion and Trajectory Infilling](https://arxiv.org/abs/2608.11407v1)
*Da Saem Lee,Yash Vardhan Pant,Sebastian Fischmeister*

Main category: cs.RO

TL;DR: 本文提出TrafficDiffuser，一个自顶向下的交通场景生成框架，通过联合建模初始状态和目标状态对来生成高层交通场景，使初始状态更具可解释性，并将轨迹生成简化为插值问题。在Argoverse 2数据集上的实验表明，该方法在速度分布距离上比次优方法降低55.3%，离路率降低2.8%。


<details>
  <summary>Details</summary>
Motivation: 现有的交通模拟器需要预定义的智能体初始状态才能生成轨迹，这限制了可扩展性和多样性；虽然已有数据中心化的智能体初始化方法，但生成的初始状态缺乏可解释性（即无法解释为何智能体在特定位置初始化）。此外，给定初始状态后的轨迹生成也面临目标可变性和到达方式学习的挑战。

Method: 提出TrafficDiffuser框架，采用自顶向下的方式生成高层交通场景，即联合建模初始状态和目标状态对。该方法使初始状态更具可解释性，并将轨迹生成简化为插值填充问题。框架支持基于不同轨迹模式进行约束，并可集成到现有轨迹生成模型中。

Result: 在Argoverse 2运动预测数据集上进行大量实验验证生成输出对真实世界分布的捕捉效果。在生成目标状态的同时，TrafficDiffuser在智能体初始化方面优于次优方法，速度分布距离降低55.3%，离路率降低2.8%。

Conclusion: TrafficDiffuser通过联合建模初始状态和目标状态对，成功实现了可解释的、更贴近真实分布的交通场景生成，既能提升初始化质量，又能与现有轨迹生成模型无缝集成。
