...
中文摘要：视频生成模型的最新进展已使高保真、时间连贯的视频生成成为可能。然而，这些模型往往难以满足需要专门知识、特定身份、物理一致性或有序事件的提示。在本文中，我们提出了 VideoGen-Agent，一个通过多任务智能体强化学习训练的多模态智能体，用于使用外部工具进行视频生成。该智能体通过多轮交互协调增强、生成与验证工具，利用提示与中间观察来指导其决策。我们在一个涵盖六类任务的类别平衡数据集上训练一个共享策略。对教师生成的轨迹进行监督微调以建立工具使用行为，随后通过强化学习对其进行改进。一种类别感知的混合奖励评估工具调用的有效性、任务适配的工具使用以及生成视频的质量。我们进一步提出了 VABench，一个包含 600 条提示的留出基准，涵盖程序性知识、单实体与多实体身份保持、物理一致性、场景构图以及多镜头时间结构。在 VABench 上，VideoGen-Agent 相较于其基础文本到视频生成器提升了 19.1 分，从 56.5 提升至 75.6。升级生成工具后在无需额外智能体训练的情况下进一步将分数提升至 86.1。在 84.3% 的对比中，人类评分者更偏好升级后的配置，而非最强的独立基线。这些结果支持跨视频生成任务学习工具使用，并表明所训练的智能体能够受益于生成工具的后续进展。
...
中文摘要：视频世界模型使得对动态环境的交互式探索成为可能，但难以在长时程和跨视角条件下遵循先前的观测。我们提出 WorldCrafter，一种视频世界模型，为此学习一种可进行相机查询的隐式3D感知记忆。其核心洞见在于：让所请求的视角决定多视角证据如何被压缩进视频生成器有限的token预算中。一个记忆编码器与姿态条件化的读出模块与视频生成器联合训练，在去噪之前将历史观测整合为一组固定的、特定于目标视角的token，且无需显式的基于深度的对应关系。通过将该记忆与近期时间上下文以及少步蒸馏相结合，WorldCrafter 能够从单张输入图像或文本提示出发实现流式场景探索。在静态与动态场景上的实验表明，其在长时程一致性和相机控制精度方面取得显著提升，同时在分钟级探索过程中保持了视觉质量。
...
### [3] [GAE: Learning a Geometry-Native Latent Space for 3D-Consistent World Generation](https://arxiv.org/abs/2609.24981v1)
GAE：学习面向三维一致世界生成的几何原生潜在空间

*Jiahao Lu,Minghao Yin,Wenbo Hu,Hengyu Liu,Wang Zhao,Sai-Kit Yeung,Ying Shan,Yuan Liu*

Main category: cs.CV

中文摘要：我们提出了一个紧凑的几何原生潜在空间，作为感知与生成的共享基础。视觉生成器能够生成照片级真实的帧，却无法保持一致的三维场景。我们认为这不仅是建模问题，也是表示问题：生成器通常演化以外观为中心的潜在表示，而感知模型在编码跨视图结构的语义丰富空间中恢复几何。与其将几何作为另一种输出添加，我们将几何基础模型的特征重参数化为用于生成的紧凑潜在空间。我们通过几何原生自编码器（geometry-native autoencoder，GAE）实现了这一转变，其潜在表示可联合解码为外观、深度、相机和点图。有了这一状态，标准的条件流即可支持多样化的生成任务。在保持生成器和训练协议固定的受控比较中，用 GAE 替换潜在表示同时提升了视觉质量和独立测量的三维一致性：在 RealEstate10K 和 DL3DV 上 FVD 分别下降 $12.7\%$ 和 $23.1\%$，在 RealEstate10K 上相机轨迹误差减半。这些结果共同表明，潜在空间对于几何一致的生成至关重要，并可作为感知与生成之间的共享接口。
...
### [4] [INTCORT: Training-Free Spatial Reasoning Enhancement for Vision-Language Models via Input Transformations and Confidence Routing](https://arxiv.org/abs/2609.24813v1)
INTCORT：通过输入变换与置信度路由为视觉语言模型实现免训练的空间推理增强

*Haoran Sun,Jingqi Xu,Yanhui Li,Enci Liu,Kaidi Xu,Yanwei Liu*

Main category: cs.CV

中文摘要：视觉语言模型（Vision-Language Models, VLMs）在多模态任务中已展现出卓越的能力，但在空间推理方面仍表现不佳。现有的依赖训练与免训练增强方法分别面临高计算成本并伴随灾难性遗忘，以及内部机制干扰从而损害通用能力的问题。在本工作中，我们首先验证了两个关键假设：适当的几何图像变换与查询反转变换能够恢复错误的空间预测，且正确预测比错误预测表现出更高的关系词元置信度（relation-token confidence）。基于这些发现，我们提出 INTCORT，一个免训练的空间推理增强框架，它通过输入变换构建多个推理视图，并通过关系词元置信度路由聚合其预测，而无需修改 VLM 的内部机制。在若干常用基准上的实验结果表明，INTCORT 在多种 VLM 上大幅提升了空间推理准确率，在所有模型和基准上平均提升 10.01%。与先前工作相比，INTCORT 取得了更优的性能，提升幅度最高达 25.01%。
...
中文摘要：在本文中，我们提出了 SVEET，一个仅需在预训练的双向视频扩散模型上进行训练，却能够以自回归方式支持高质量流式视频编辑的框架。为解决这一问题，我们首先系统地重新审视了现有的视频到视频扩散方法，并识别出实现此类流式适配的两条关键原则：主干特征解耦（backbone feature disentanglement）与条件帧独立性（conditional frame independence）。基于这些洞见，我们提出了一种用于可控视频生成的新范式。其核心是，一个辅助模型分支通过时间上独立的自注意力对源视频输入进行编码，并将中间特征注入到相应的主干模块中，以实现与流式兼容的控制。此外，为弥合双向模型与流式模型特征空间之间的差异，我们提出了一种解耦训练方案，该方案显式地强制视频可控性与模型因果性这两个优化方向之间保持正交。这种解耦确保了推理时两个目标之间的兼容性，并促进了跨异构主干架构的平滑零样本知识迁移。大量实验表明，SVEET 在保持实时性能的同时实现了更优的编辑质量，在单张 H100 GPU 17 上达到 15 FPS，且未使用任何辅助加速技术。代码可在 https://github.com/YujiaHu1109/SVEET 获取。
...
### [6] [Relationally Grounded Latent World Models for Autonomous Driving](https://arxiv.org/abs/2609.24626v1)
面向自动驾驶的关系基础潜在世界模型
...
中文摘要：潜在世界模型（latent world models）为自动驾驶学习预测性表示，但这些状态所保留的关系语义往往仍是隐式的。我们研究交通场景图（traffic scene graphs）能否作为潜在世界表示的特权语义监督（privileged semantic supervision）。在 LAW 的基础上，我们从 nuScenes 3D 标注构建以行动者为中心的场景图，使用冻结的文本嵌入模型对其序列化的关系结构进行编码，并在训练期间将视觉潜在表示与该语义目标对齐。我们在推理时移除监督分支，因此它既不需要场景图也不需要 3D 标注，且不增加任何测试时计算。在 nuScenes 上，相对于我们重新训练的 LAW 基线，我们的方法将平均轨迹 L2 误差从 0.661 降至 0.622（5.9%），并将碰撞率从 0.456 降至 0.217（52.4%）。它同样优于一种非结构化的字幕式语义目标，这支持了显式关系结构对于潜在世界模型表示学习的益处。
...
### [7] [ME-VLM:A Unified VLM for Embodied Cognition and Agent Coordination](https://arxiv.org/abs/2609.24526v1)
...
中文摘要：物理AI（Physical AI）要求模型将视觉与语言理解建立在真实世界环境中，同时考虑环境约束与执行反馈。我们提出 MachEmbodied-VLM（ME-VLM），这是一个统一的视觉语言模型，包含 4B 和 35B-A3B 两个版本，将具身认知与多模态智能体能力结合在一起。我们的工作强调物理感知与时空推理，以及在数字环境和物理环境中的规划、交互与结果评估。我们构建了涵盖具身任务与多模态智能体任务的训练数据，其中包含执行观测与反馈，以支持结果评估和决策优化。训练流程包括具身能力注入、对具身专家与多模态智能体专家分别进行强化学习，以及多教师同策略蒸馏（multi-teacher on-policy distillation），将二者互补的能力整合到单一模型中。实验表明，该模型在具身基准与智能体基准上，以及在自动驾驶和具身导航任务上，均取得了具有竞争力的性能。面向边缘部署，视觉 token 压缩、W4A8 量化以及软硬件协同优化使 4B 版本能够在 M100 上实现端侧推理，将预填充延迟从 400 ms 降至 188 ms。项目页面：https://machembodied.com/ME-Brain/ME-VLM.html 代码仓库：https://github.com/MachEmbodied/ME-VLM
...
### [8] [DeCo: Efficient Decouple-to-Couple Learning for Multi-Task Visual Grounding](https://arxiv.org/abs/2609.24409v1)
...
中文摘要：多任务视觉定位要求模型联合理解语言语义，并执行准确的视觉定位与分割。尽管多模态大语言模型取得了成功，但有效地将其适配到多个定位目标仍然具有挑战性。现有方法通常通过共享表征来强制任务协作，却忽视了任务导向的特征关注点之间的内在冲突。在本文中，我们提出了 $\textbf{DeCo}$，一个高效的 $\textbf{De}$couple-to-$\textbf{Co}$uple（解耦到耦合）学习框架，通过两阶段范式解决这一困境：任务特定表征解耦，随后进行互补先验耦合。具体而言，我们首先提出任务感知语义解耦（Task-aware Semantic Decoupling, TSD），在显著的词级指导下将共享视觉线索路由到各自的特征中，从而缓解定位与分割之间的表征干扰。此外，我们观察到，由于密集监督，分割自然提供了信息丰富的定位先验。基于这一洞察，我们引入混合先验耦合（Hybrid Prior Coupling, HPC），它将句子级语义先验与由掩码导出的空间先验相结合，以增强定位。DeCo 构建于一个冻结的多模态编码器之上，仅需轻量级可训练参数，同时在多个定位目标上实现了强泛化能力。在 RefCOCO/+、G-Ref、ReferIt、Flickr、DIOR-RSVG、SARVG1.0、RRSIS-D、RIS-LAD 和 RefDIOR 上的大量实验表明，DeCo 在自然与遥感基准上均达到了最先进的性能。代码与模型可在 https://github.com/xiaoqiang-lu/DeCo 获取。
...
### [9] [NeuIDO: Neural Intrinsic Dynamics Operator for Physics-Informed 4D World Models](https://arxiv.org/abs/2609.24313v1)
NeuIDO：面向物理信息4D世界模型的神经内禀动力学算子

*Jiajing Lin,Xin Zhang,Jianhua Sun*

Main category: cs.CV

中文摘要：世界模型旨在捕捉环境动力学并预测未来轨迹，在具身智能方面展现出日益增长的潜力。物理信息4D生成（Physics-informed 4D generation）通过整合物理仿真来预测3D物体交互，为通向世界模型提供了一条有前景的路径。然而，这一范式依赖于人为施加的动力学假设，而非将世界动力学内化，因此距离真正的世界模型仍有差距。为弥合这一差距，我们提出NeuIDO，一种新颖的世界动力学建模框架，它从视觉观测中学习统一的内禀动力学表征，推动物理信息4D生成向世界模型迈进。具体而言，我们将世界建模形式化为一个神经算子学习问题，并引入两阶段训练策略，以学习从视觉观测分布到内禀动力学分布的可泛化映射。基于这一观测-动力学映射，NeuIDO能够直接从视频中进行零样本动力学推理，并可通过少样本自适应进一步与复杂的真实世界动力学对齐。大量实验表明，NeuIDO有效地将多种视觉观测背后蕴含的内禀动力学统一为共享表征，并能在新场景中快速推理动力学。
...
### [10] [HappyWorld-Bench](https://arxiv.org/abs/2609.24308v1)
HappyWorld-Bench
...
中文摘要：评估世界模型需要同时考察其生成世界的质量，以及其在探索、交互和修改下的一致性与响应性。我们提出 HappyWorld-Bench，一个综合性基准，用于评估生成的世界在智能体与其交互时是否保持可靠。我们的设计建立在一个包含六种世界能力（W1-W6）的分层能力框架之上，涵盖从生成式构建到统一世界建模，并在三条相互独立的评估轨道上实例化：视频世界模型、空间世界模型和具身世界模型。HappyWorld-Bench 包含 1,138 条视频提示、300 个空间场景和 254 个具身测试用例。在全部三条轨道上，我们构建并运行 HappyWorld-Arena，以组织人类 A/B 比较并得出模型级别的 Elo 评分，其与新设计的、用于刻画行为正确性的自动化指标互为补充。我们在这一统一框架下评估了 14 个视频世界模型、9 个空间系统和 8 个具身候选模型。结果揭示了全部三条轨道上仍然存在的可靠性差距：视频模型在扩展推演和重访过程中表现出一致性下降，空间模型最多达到 70.14% 的放置准确率和 73.33% 的编辑执行率，具身模型难以在多步动作中保持状态，也难以对改变后的动作条件和物理规则做出精确响应。这些发现凸显出，评估世界模型不仅应依据视觉质量，还应依据状态一致性及其对动作和干预响应的正确性。
...
### [11] [CoaG: Cylinders on a Grid: Coarse 3D Layout Control for Video Generation](https://arxiv.org/abs/2609.24208v1)
CoaG：网格上的圆柱体：面向视频生成的粗粒度3D布局控制

*Zhangsihao Yang,Mengyi Shan*

Main category: cs.CV

中文摘要：我们探究：为了控制生成视频中人物站立的位置以及相机移动的方式，一个人需要绘制多少几何信息。我们的答案是：一个地平面，以及每个人一个圆柱体。用户在ground上绘制一个网格，在每个人应站立的位置放置一个圆柱体，在81帧内移动这些圆柱体和相机，模型便会渲染出一段逼真的视频，其中人物占据圆柱体的位置、随圆柱体的移动而移动，并从所绘制的相机视角被观察。外观来自文本提示词和背景参考图像；布局与运动则来自几何信息。由于没有数据集将这样的信号与视频配对，我们自行构建配对数据：一个自动引擎从组合式种子出发撰写2000条caption，用文本到视频模型为每条caption生成一个片段，并通过人物跟踪、背景inpainting、agentic ground-mask循环、前馈多视图重建和平面拟合，将每个片段反推回其几何表示，全程不使用真实拍摄素材，也不使用人工标注。在1935个此类元组上训练的Wan2.2-Fun-Control上的LoRA，在留出片段上能够遵循所绘制的布局和相机路径：生成的人物与圆柱体的数量、顺序、位置和高度相匹配，文本改变他们是谁，参考图像改变他们在哪里，并且dolly-in、orbit、pan和crane路径均被遵循，dolly-out仅被弱遵循。
...
### [12] [A paired synthetic construction-site image dataset for robust computer vision under adverse conditions](https://arxiv.org/abs/2609.24075v1)
一个用于恶劣条件下鲁棒计算机视觉的配对合成建筑工地图像数据集

*Viet Huy Duong,Ruoxin Xiong,Md Abdullah Al Forhad,Weishi Shi*

Main category: cs.CV

中文摘要：用于建筑监测的计算机视觉系统在不利的环境与视觉条件下可能出现性能退化，然而此类条件在现有建筑图像数据集中仍代表性不足。我们提出 ConSynth-X，一个配对的合成建筑工地图像数据集，包含 34,199 张图像，源自 3,109 个真实世界源场景。该数据集包含 11 个特定条件子集，涵盖降水、雾、夜间照明、夜间不利天气，以及小目标或远距离视图。每张合成图像均与其对应的源场景相关联，从而支持跨环境与视觉条件的受控比较。ConSynth-X 包含源自源数据的标注、生成元数据、来源信息与图像质量指标，支持目标检测、图像描述、视觉定位与视觉问答。技术验证通过基于嵌入的相似性与分布分析，评估源图像—合成图像的保真度以及与真实不利条件影像的一致性。该数据集为在具有挑战性的现场条件下评估并提升建筑视觉与视觉—语言模型的鲁棒性提供了一个结构化资源。
...
### [13] [MotionJEPA: Preventing Temporal Feature Collapse by Capturing Visual Changes in Latent Space](https://arxiv.org/abs/2609.23881v1)
MotionJEPA：通过在潜在空间中捕捉视觉变化来防止时序特征坍缩
...
中文摘要：联合嵌入预测架构（Joint Embedding Predictive Architectures，JEPAs）是一种颇具前景的范式，可在不进行视觉重建的情况下学习与任务无关的潜在世界模型。然而，标准 JEPA 训练表现出强烈的慢特征（slow features）归纳偏置，导致特征抑制与潜在表示坍缩。尽管逆动力学（inverse dynamics）提供了时序上的抗坍缩能力，但它依赖动作标签，并且几乎不提供嵌入通用的、无标签动态信息的激励。我们提出差分图像与单图像嵌入正则化（Difference Image and Single image embedding Regularization，DISReg），这是一种新型正则化器，它构建于一个逆动力学风格的模块之上，该模块预测时序差分图像嵌入且不使用任何像素重建损失，从而鼓励静态与动态特征的平衡学习。DISReg 由一个静态项和一个动态项组成：静态项塑造图像嵌入的分布并鼓励慢特征；动态项与对嵌入的直接正则化不同，它不对图像嵌入的形状或分布施加约束，而仅激励动态特征的存在。通过将该正则化器集成到标准 JEPA 中，我们建立了新的架构 MotionJEPA。潜在探测（latent probing）表明，MotionJEPA 产生的表示比其他方法更完整，而我们的轨迹分析显示，它保持了曲率较低的几何简单的潜在嵌入。我们进一步表明，MotionJEPA 在四个环境中提升了静态背景干扰物下的下游规划成功率。
...
### [14] [Mira-Scene: Pixel-Aligned Layouts for Generative 3D Scene](https://arxiv.org/abs/2609.23796v1)
...
中文摘要：单图像3D物体生成如今已能产生高保真资产，但将其准确地放置到连贯的场景布局中仍是一项未解决的挑战。核心困难在于物体布局的表示方式。整体式方法将放置吸收进场景级生成过程，牺牲了物体级细节。组合式方法通过将几何与布局解耦来保持物体保真度，但通常将布局参数化为稀疏、无界的姿态变量，这类变量难以学习，且在场景级监督稀缺时泛化性差。我们提出Mira-Scene，一个组合式3D场景重建框架，用稠密、有界的对应关系恢复取代稀疏姿态回归。其核心是规范坐标图（Canonical Coordinate Map, CCM），这是一个像素对齐场，将每个可见物体像素映射到该物体有界规范空间中的表面坐标。当与来自单目几何估计的场景空间点云图（Point Cloud Map, PCM）配对时，CCM可导出稠密的规范到场景对应关系，并由此通过稳健的几何对齐恢复物体变换。由于CCM在有界规范空间中运作，它提供了一个稳定的预测目标，可在无需场景级布局标注的情况下，从可扩展的物体级3D数据中训练。Mira-Scene进一步引入了一个多模态扩散Transformer，联合生成物体几何与CCM，其使用具有共享注意力与位置编码的模态专属专家流，以促进几何—布局一致性。在室内、室外、合成以及真实场景（in-the-wild）上的实验表明，Mira-Scene在布局精度上大幅优于强基线，在使用有限开源训练数据的情况下，相较SAM3D在3D-IoU上取得39.8%的相对提升，在2D-IoU上取得16.5%的相对提升。
...
### [15] [OnlineWM: Causality-Aware Active Online Learning for Effective World Modeling](https://arxiv.org/abs/2609.23753v1)
...
中文摘要：生成式世界模型旨在以动作为条件预测未来状态，其中动作可控性对于可靠的动力学建模至关重要。尽管近期工作利用模拟器生成的数据来增强这一能力，但现有训练流程面临两个根本性局限。第一，静态的离线数据收集导致训练集与模型不断演变的误差模式之间出现分布错位，无法解决动力学预测仍不可靠的关键长尾场景。第二，最小化观测差异的标准目标往往促使模型利用虚假相关，而非捕捉潜在的动作—效果因果关系。为解决这些局限，我们提出 OnlineWM，一个通过主动模拟器交互与因果感知优化来持续改进世界建模的在线训练框架。OnlineWM 引入两项关键创新：（1）主动在线学习：不再是使用固定数据集，OnlineWM 自适应地向模拟器查询针对模型当前预测弱项的新交互序列，从而确保高效用数据的获取。（2）因果感知微调：我们提出一种反事实学习策略，对相同状态下不同动作的结果进行对比，迫使模型将状态转移归因于特定动作，而非环境自身的演化，从而使其预测建立在可靠的因果机制之上。通过将主动数据获取与因果优化相结合，OnlineWM 建立了一个闭环精炼过程，确保模型既对多样场景具有鲁棒性，又在其因果归因上保持精确。大量实验表明，OnlineWM 显著增强了动作可控性，并能有效泛化到未见领域。
...
### [16] [Why Do Video Diffusion Models Violate Physics? Unveiling the Flaws in Attention Mechanisms](https://arxiv.org/abs/2609.23658v1)
...
中文摘要：尽管视觉质量令人印象深刻，最先进的视频扩散模型常常生成违反现实世界物理规律的内容。现有解决方案依赖外部先验或专门数据，而我们则通过探究这些模型的内部机制来研究其根本原因。具体而言，我们首次对文本到视频扩散模型的“运动规划”过程进行了可解释性研究，揭示了运动轨迹在早期去噪阶段是如何形成的。基于“先形状、后细节”这一发现，我们将交叉注意力轨迹模式与因果头贡献相结合，识别出驱动运动规划的一小部分特定注意力头。进一步地，我们的自注意力分析表明，旋转位置嵌入（RoPE）会引发过度的空间注意力衰减。这导致早期候选区域过早锁定在物理上不合理的位置，抑制了相邻帧中的合理轨迹，并触发生成失败模式。为解决这一根本缺陷，我们提出了一种轻量级的架构修改，在不同去噪步长上对RoPE的频率进行缩放。该策略减少了过度的注意力衰减，帮助模型探索更好的候选区域，从而建立连贯的物理运动。最后，免训练和基于训练的实验均证实了我们的方法在增强生成视频的物理常识方面的有效性。
...
### [17] [An Efficient and Effective Watermarking Scheme for the Protection of the Intellectual Property Rights of Video Generative Models](https://arxiv.org/abs/2609.23586v1)
...
中文摘要：视频生成模型（VGMs）的快速发展使得生成高度逼真的合成视频成为可能，这引发了人们对这些模型知识产权（IPR）的担忧。特别是，两个密切相关的取证任务在很大程度上仍未得到解决：合成视频验证（判断一段视频是否由受保护的 VGM 生成）以及模型所有权验证（判断一个可疑的 VGM 是否为受保护 VGM 的未经授权副本）。在本文中，我们提出了一种新的生成内水印方案，可以解决这两个验证任务。首先，我们提出了一种名为 VidMark 的新型视频水印网络，它融合了两尺度离散小波变换（DWT）分解和全局时间注意力模块（GTAB），以增强水印的鲁棒性和不可感知性。其次，我们提出了一种解码器引导的微调流程。通过利用冻结的 VidMark 解码器，该流程使 VGM 能够合成携带不可感知、鲁棒且模型特定水印的视频。最后，我们建立了两个验证框架，以执行合成视频验证和模型所有权验证。在代表性 VGM 上的大量实验表明，所提出的方案在两个任务上均实现了超过 99% 的水印提取准确率和 100% 的验证准确率，同时对视频生成质量的影响可忽略不计。此外，这些水印对全面的视频级和模型级攻击表现出很强的鲁棒性。
...
### [18] [Transferring Visual Explanations: How Cross-Architecture Knowledge Distillation Affects Model Interpretability](https://arxiv.org/abs/2609.23561v1)
...
中文摘要：在资源受限的环境中，部署高效的神经网络至关重要，然而紧凑模型往往会牺牲可解释性——这在自动驾驶和医学等安全关键领域中是一个关键问题。本研究考察知识蒸馏（Knowledge Distillation, KD）是否会将大型教师网络的空间特征归因迁移到紧凑的学生网络中。为评估KD方案对可解释性的影响，我们在ImageNet-1K上将ResNet-152教师网络蒸馏为ResNet-34学生网络，通过系统性地改变蒸馏温度和软标签损失权重，考察五种配置。模型在top-1准确率以及两个可解释性指标上进行评估：相关性质量准确率（Relevance Mass Accuracy, RMA）和相关性排序准确率（Relevance Rank Accuracy, RRA）。这些指标通过以真值物体掩码为基准的Grad-CAM热力图计算得出。我们的结果表明，top-1准确率范围为71.6%至74.0%。对于Grad-CAM，RMA范围为7.7%至9.7%，RRA范围为7.3%至10.1%；对于Guided Grad-CAM，RMA范围为16.1%至18.6%，RRA范围为15.9%至21.5%。可解释性对软标签权重的敏感程度远高于对温度的敏感程度：使学生网络锚定于硬标签可同时保持准确率和粗粒度定位，而对学生网络施以较重的教师权重则会同时降低二者。然而，细粒度归因在测试的每一种配置中均低于未蒸馏基线，这表明logit蒸馏传递模型关注“何处”比传递该关注的像素级结构更为容易。我们评估了卷积模型与基于transformer的模型之间的12种跨架构组合，揭示了细粒度空间推理的继承从根本上受限于学生网络固有的结构偏置。据我们所知，这是该可解释性感知评估框架——此前曾用于神经网络剪枝——首次应用于KD。
...
### [19] [The Right Future for Action: Learning Action-Relevant Predictive States in World Action Models](https://arxiv.org/abs/2609.23369v1)
行动的正确未来：在世界行动模型中学习动作相关的预测状态

*Qiwen Gu,Jifan Li,Bingjie Gao,Rui Chen,Jing Tang,Xiangxiang Chu,Junqiao Zhao*

Main category: cs.CV

中文摘要：免生成的世界行动模型（WAMs）在训练时保留未来视频预测，但在推理时依据内部视频特征行动，这使得这些特征应当为控制保留什么变得不明确。我们的表征诊断表明，具有更可预测的未来变化的表征未必能使线性动作解码更容易。观测到的未来变化提供了超出当前时刻的额外动作信息，且线性可读的动作信息在空间上是集中的。这些发现促成了动作相关预测状态（ARPS），即在视频专家与动作专家之间的一个紧凑预测接口。ARPS 使用一个以时间跨度（horizon）为条件的状态预测器，将中间视频特征聚合为一个紧凑状态，该状态为动作专家提供全部视觉上下文。未来表征监督训练该状态的不同部分去预测不同未来时刻的视觉表征，以及它们相对于当前时刻的变化。在推理时，监督分支被移除，动作专家仅使用由当前观测计算得到的已学习预测状态。受控消融实验表明，未来监督显著提升了分布偏移下的泛化能力。ARPS 在 LIBERO 上达到 99.2% 的成功率，并无需适配即可迁移到 LIBERO-Plus，达到 87.3%，超过 Fast-WAM 39.2 个百分点。
...
### [20] [CausalWM: Causal Chain-of-Thought Reasoning for Embodied World Model](https://arxiv.org/abs/2609.23184v1)
...
中文摘要：具身世界模型（Embodied world models）学习从视觉观测与控制信号中预测未来的物理动态，其中物理知识以隐式方式纠缠于潜在表示之中。我们提出 CausalWM，一个 16B 的具身世界模型，它在未来视频预测之前执行显式的因果思维链（causal chain-of-thought）推理。CausalWM 将有用变量组织为一条推理轨迹，使模型能够逐步捕捉物理演化背后的因果依赖关系。为训练 CausalWM，我们收集了 31K 小时的具身数据，并开发了一个三阶段范式，由大规模视频预训练、因果 CoT 中期训练和多目标 RL 后训练组成。尽管仅使用有限的一组监督 CoT 变量，CausalWM 仍展现出涌现的上下文学习（in-context learning）能力，从而实现上下文视觉特征引导与高效的少步生成。CausalWM 在语言条件、动作条件、单视角与多视角基准上均取得了最先进的性能，包括在 TriWorldBench 排行榜上取得 Top-1 性能。
...
### [21] [SparkDiffusion: Mitigating the High-Sparsity Trap --- A Unified Framework for up to $265\times$ Single-GPU Acceleration of Visual Generation](https://arxiv.org/abs/2609.23153v1)
SparkDiffusion：缓解高稀疏陷阱——一个实现视觉生成最高达 $265\times$ 单 GPU 加速的统一框架
...
中文摘要：视频扩散 Transformer 的计算开销很高，因为注意力机制主导了长时空 token 序列。我们识别出\emph{高稀疏陷阱}：在极端注意力稀疏度下，步局部训练损失持续下降，而最终生成质量却停滞或退化。该陷阱本质上是一个监督问题：主要的最终误差源自高噪声的结构生成阶段，而面向最终目标的训练能够纠正那些即使大幅延长步局部训练也无法纠正的最终误差。由此得到一个简单的分阶段原则：\emph{首先将稀疏架构适配为粗先验，然后校正最终分布}。我们将该原则实例化为 \method，这是一个用于视觉生成的统一加速框架，结合了短暂的稀疏预热、少步轨迹混合蒸馏，以及带融合内核的 FP8 量化。\method 在 Wan2.1/Wan2.2 骨干网络以及 T2V/I2V 任务上，于长序列 720P 生成中保持 $97\%$ 的注意力稀疏度并具备强视觉质量，并在 Wan2.1-T2V-1.3B-480P 上达到 $90\%$ 稀疏度。在 3 步无 CFG 推理下，\method 在单张 RTX~5090 上对 Wan2.1-T2V-14B-720P 相较 50 步 CFG 稠密基线实现了 $265\times$ 的端到端加速（在 H100 上为 $220\times$），并在 $1.3$s 内完成对 Wan2.1-T2V-1.3B-480P 视频的去噪。
...
### [22] [RewardVerse: Rubric-Guided Policy Optimization for Video Reward Modeling](https://arxiv.org/abs/2609.22947v1)
RewardVerse：面向视频奖励建模的评分量表引导策略优化

*Zhenchen Tang,Yang Li,Songlin Yang,Bo Peng,Xiaotong Zhao,Shuai Li,Haotian Fan,Alan Zhao,Jing Dong*

Main category: cs.CV

中文摘要：强化学习（RL）对于优化视频生成模型至关重要，而一个稳健的奖励模型（RM）则是其基石。然而，现有的视频奖励模型往往产生不稳定的标量分数，因为它们将复杂、主观的视频质量直接映射为单一分数，而缺乏明确的评估标准。这导致标量漂移（scalar drift），即评分尺度在不同提示之间崩溃或偏移，使得奖励对RL而言不可靠。受专业人工标注工程的启发，我们以RewardVerse来应对这一问题。RewardVerse是一个基于评分量表（rubric）的视频奖励框架，它在评估查询与评分器之间引入动态评分量表作为中间表示。RewardVerse并不进行无约束的直接评分，而是首先生成明确的评估标准，然后执行评分量表引导的评分，从而提供一个稳定的语义锚点，缓解标量漂移。为高效优化这一协同流程，我们提出评分量表引导策略优化（Rubric-Guided Policy Optimization, RGPO），一种两阶段训练算法。RGPO首先使用自演化的种子评分量表对评分器进行预热，然后联合优化评分量表生成器以产生适应查询的评估标准，同时持续使评分器与人类评分对齐。在16维EvalVerse基准和外部数据集上的大量实验表明，RewardVerse缓解了标量漂移，在逐点（pointwise）和成对（pairwise）评估上均达到最先进性能，并为视频生成中的RL提供了稳健且可解释的奖励信号。
...
### [23] [Human-Level Accuracy, Non-Human Strategies: Revealing Model-Human Divergence in Video Physical Reasoning](https://arxiv.org/abs/2609.22788v1)
...
中文摘要：视频基础模型如今在物理推理基准上已达到人类水平的准确率，然而此类任务要求预测未被观测到的物理结果。这些模型究竟是在进行类人的前向模拟，还是在利用可见场景中的统计规律性？仅凭准确率无法区分这些策略。我们引入一个分布式的评估框架，将模型随机种子与人类评分者视为群体，从而能够比较共识、不确定性与策略。在 Physion 基准上，我们评估了三种 ViT-L 架构（V-JEPA2、VideoMAEv2、DINOv2）。V-JEPA2 将准确率差距缩小至约 1 个百分点（73.2% 对 74.2%），然而模型与人类的分歧达到 26.4%，远超人类与人类之间的分歧（4.8%），一致性也显著更低（kappa 约 0.48 对 0.91）。这种分歧与前向模拟的需求相对应：模型在几何推理（连接，+11.8 个百分点）上优于人类，但在引力动力学（滚动，-11.8 个百分点）和因果链（多米诺骨牌，-10.5 个百分点）上表现不及人类。策略指纹分析确认，三种架构都共享非人类的策略，而没有一种与人类一致。归因分析表明，预测这种分歧的是不可观测的结果特征，而非可见场景属性，这与模型更多依赖场景层面的统计规律性、而非显式前向模拟相一致，这是一种仅凭准确率无法揭示的系统性分歧。代码可在 https://github.com/fanhong-li/model-human-divergence 获取。
...
### [24] [ConsistWorld: Evidence Routing for Consistent Multi-Agent World Models](https://arxiv.org/abs/2609.22641v1)
...
中文摘要：自回归视频世界模型能够为单个观察者实现时间上连贯的生成。将其扩展到多个智能体，则需要在因果流式生成下，实现由独立控制的视角与时间间隙之间的一致性。我们提出 ConsistWorld，一种多智能体世界模型，它从单张共享图像生成静态场景的相机控制视频流。我们将一致性形式化为：从已提交的多智能体历史以及并发生成的同伴视角中，向正在生成的 token 路由证据。姿态条件记忆检索（Pose Conditioned Memory Retrieval）从所有智能体中选取相关的历史观测，从而恢复超出近期上下文窗口的证据。可见性门控同伴共享（Visibility-Gated Peer Sharing）根据估计的历史覆盖度与当前视角重叠度来调节当前同伴信息。二者共同决定哪些历史观测进入上下文，以及并发同伴信息在何处产生贡献，从而支持长期回忆与协同探索。这两种机制都使用相机几何，并在固定智能体数量与检索预算下维持有界的活跃上下文。在证据共享案例以及视频长度与智能体数量泛化性上的实验表明，ConsistWorld 在保持有竞争力的生成质量的同时，实现了较强的跨时间与跨智能体一致性。
...
### [25] [Hierarchical Aggregation of Semantic Uncertainty in 3D Scene Graphs](https://arxiv.org/abs/2609.22351v1)
三维场景图中语义不确定性的层次化聚合
...
中文摘要：开放词汇三维场景图（3DSGs）将每个物体节点锚定在一个视觉-语言嵌入中，然而它们把每个条目都记录为同等确定，因此查询该地图的机器人无法分辨其中哪些条目是不可靠的。语义不确定性的估计器本可以提供这种区分，但它们需要对模型进行重复采样、训练或使用留出标签，而这些在查询时对已部署系统来说都无法获得。我们提出一个框架，利用检测器置信度以及3DSG已经存储的嵌入，将它们转换为某个条目正确的概率，并将该概率沿包含层级传播为某个房间包含所查询类别的信念。四个信号——每个信号都与其所指示的物体级误差配对——在视觉-语言模型所学习的logit尺度上被转换为概率，并以闭式形式组合，无需额外的感知或训练。共享同一检测器和同一词汇表的物体是共同失败的，因此该框架在完全相关极限下对它们进行聚合，而在独立性假设下的聚合会把一个重复出现的误差当作重复的证据。在HM3DSem上针对一个最先进的3DSG系统进行评估，该框架改进了物体检索，并降低了它所读取的图的房间级断言的误差。
...
### [26] [Rethinking Streaming Video Diffusion Model: Context, Execution, and Training](https://arxiv.org/abs/2609.22283v1)
...
中文摘要：理解流式视频扩散的设计空间对于探索其在生成质量与计算效率方面的潜力至关重要。我们构建了一个统一的分析框架，将模型与采样器的选择、历史条件化、执行调度以及训练策略联系起来。该框架容纳了一大类因果上下文选择策略，并明确揭示了它们的计算依赖关系以及训练—推理对齐。在这一设计空间内，我们研究了三种具有代表性的策略：干净历史、同层历史和渐进历史。在完整的 VBench 提示集上，同层历史和渐进历史分别取得了 85.24 和 85.60 的总分，而干净历史参考为 84.45。长视频对比进一步表明，渐进历史在主体一致性和运动连贯性方面有所提升。通过允许多个去噪节点一起处理，渐进历史流水线在我们所评估的条件下实现了 $1.57$-$2.83\times$ 的 DiT 稳态加速。我们另外发现，对 DMD 假分数网络进行 LoRA 适配即可提升生成质量，其可训练的假分数参数量仅为全参数适配的 2.15%。这些发现共同表明，完全去噪的历史并非高质量流式生成的先决条件，并推动了历史条件化、执行与训练的联合设计。
...
### [27] [DexTacWAM: A Visuo-Tactile World-Action Model for Dexterous Manipulation](https://arxiv.org/abs/2609.24976v1)
DexTacWAM：面向灵巧操作的可视-触觉世界-动作模型

*Haoran Yuan,Zekai Wang,Boning Shao,Haoran Lu,Trevor Darrell,Ismini Lourentzou,Wei Zhan*

Main category: cs.RO

中文摘要：灵巧操作依赖于接触动力学，而这类动力学通常只能从视觉中被部分观测。近期的世界-动作模型（World-Action Models, WAMs）将预测性视频世界建模与动作生成相结合，但仍在很大程度上以视觉为中心，因此无法直接对这类接触动力学进行建模。我们提出 DexTacWAM，一种可视-触觉 WAM，它独立编码每个指尖，通过一个感知手指与位姿的触觉压缩器聚合所得特征，并将触觉潜变量注入视频扩散世界模型，以实现联合的可视-触觉世界建模。在 22 自由度双臂平台上的六项接触密集型灵巧操作任务中，DexTacWAM 在每项任务上均取得最高分数，平均为 70.6，而最强基线为 38.0。消融实验将这一提升归因于将接触演化建模为所预测世界状态的一部分，而非仅靠触觉条件化：在保持相同触觉特征与动作专家的情况下，移除触觉世界建模使四项任务均值从 74.7 降至 26.6。在使用冻结的预训练视觉 VAE 进行四小时触觉编码器适配后，我们的持续视觉到触觉学习将预训练视频模型扩展到触觉，每项任务仅需约 100 条演示且无需触觉中期训练，同时将视觉预测质量保持在仅视觉对应方法 0.5 dB 以内。该压缩器在实现训练速度提升 2.26 倍、推理速度提升 1.29 倍的同时，保留了融合前 89.4% 的接触召回率。这些结果共同表明，预训练视频先验能够以数据和计算高效的方式扩展到分布式多指接触动力学。
...
### [28] [D-JEPA: A Decision-Aligned Latent World Model](https://arxiv.org/abs/2609.24749v1)
...
中文摘要：潜在世界模型预测动作的后果，但准确的预测并不能保证潜在距离能够反映哪个候选动作将被成功执行。我们识别出一种决策局部的预测差距（decision-local prediction gap）：在少数竞争执行的未来中，一个被预测为更接近目标的候选动作，其实际结果可能比某个可用的替代方案更差。我们提出 D-JEPA，一种决策对齐的潜在世界模型，它从已执行的结果中学习候选未来之间与决策相关的关系。一个有界的、置换等变（permutation-equivariant）的算子联合推理目标相对的预测特征与序数证据，在动作选择最关键之处对预训练的预测几何进行细化。受限的预测器适配与共享的序数接口将这种对齐扩展到互补的预测几何之上。D-JEPA 进一步在 JEPA 兼容的未来表示中实现所学习到的决策结构，从而能够通过原生的潜在距离规划进行部署。在潜在控制、操作、预训练动作生成模型、物理机器人以及自动驾驶上的评估表明动作选择得到改善，包括在 PushT 上 87.89% 的成功率、在 RoboTwin 上平均 15.04 个百分点的提升，以及在物理机器人任务上 17 个百分点的提升。这些结果确立了与决策相关的关系结构作为连接预测性世界建模与有效控制的直接桥梁。
...
### [29] [Think Like a World Model, Act Like a VLA: Distilling World-Model Representations into Compact Robot Policies](https://arxiv.org/abs/2609.24682v1)
像世界模型一样思考，像VLA一样行动：将世界模型表征蒸馏进紧凑机器人策略

*Trung Dao,Sankalp Yamsani,Jaden Park,Joohyung Kim,Yong Jae Lee*

Main category: cs.RO

中文摘要：视觉-语言-动作（Vision-Language-Action，VLA）模型将观测映射为动作，但没有任何考虑世界如何响应的目标，因此其鲁棒性主要受数据覆盖范围的限制。世界模型恰恰承载了那个缺失的目标，并因此具有更好的物理根基，然而将未来向前推演每条决策需要数秒，这使其无法进入控制回路。我们表明，二者可以被分离。世界模型关于物理场景所知道的一切存在于其\emph{内部特征}之中；生成未来只是产生这些特征的目标，因此可以继承其物理根基，而将生成机制抛在身后。我们在普通的VLA训练中加入一个特征对齐项：将冻结的世界模型在训练帧上运行一次并缓存，学生模型学习与该缓存保持一致。训练期间不加载教师模型，投影器在使用后被丢弃，部署的策略与未蒸馏的基线完全相同，在消费级RTX~5090上以$32$~ms和$1.86$~GB运行，因此所有收益都可归因于表征，而非增加的容量或测试时计算。一个$0.8$B的学生模型在LIBERO上达到$97.9\%$，在RoboCasa-GR1人形机器人操作上从$48.2\%$提升到$50.5\%$，且同一目标可迁移到真实硬件，在单臂和双臂平台上均是如此。该收益在学生模型规模、骨干网络、对齐层和教师模型发生变化后依然存在，表明这是一种广泛的表征先验，而非两个特定网络之间脆弱的对齐。项目页面：https://thaw-vla.trung-dt.com/。
...
### [30] [LIBERO-VPro: Benchmarking Closed-Loop Visual Robustness of Robotic Foundation Models](https://arxiv.org/abs/2609.24350v1)
...
中文摘要：机器人基础模型在标准操作基准上取得了令人瞩目的性能，然而这些评测通常假设在整个执行过程中视觉观测是干净、及时且一致的。我们提出 LIBERO-VPro，一个通过扰动执行过程中可用的视觉证据，来系统评测机器人基础模型闭环视觉鲁棒性的基准。LIBERO-VPro 涵盖四个互补维度，包括视觉证据退化（Visual Evidence Degradation）、相机陈旧（Camera Staleness）、视觉来源一致性（Visual Source Consistency）和任务相关场景变化（Task-Relevant Scene Variation），跨 12 个挑战类别、96 种实验设置和 3,296 个任务条件案例。我们在约 196,000 个仿真回合上评测了三个视觉-语言-动作模型和三个世界-动作模型，并辅以在 Franka Research 3 上的 200 次真实世界部署测试。我们的结果揭示，强劲的标称性能可能掩盖视觉基础（visual grounding）与适应能力方面的重大缺陷。模型往往在严重的物体级遮挡下仍能成功，但当局部交互线索被打断或熟悉的空间先验被违反时，性能会急剧下降。它们还对陈旧或缺失的观测高度敏感，并且当变化后的任务前提条件要求行为适应时表现吃力。最后，VLA 与 WAM 表现出不同的鲁棒性特征，表明视觉鲁棒性是多维的且依赖于架构。LIBERO-VPro 提供了一个系统性诊断框架，用于开发能够在具有挑战性的视觉条件下更可靠地对动作进行基础和适应的机器人基础模型。
...
### [31] [Imagine-RL: Residual-Confidence-Guided Cross-Attention for World-Model-Augmented VLA Reinforcement Learning](https://arxiv.org/abs/2609.24033v1)
Imagine-RL：残差置信度引导的交叉注意力用于世界模型增强的VLA强化学习

*Kejia Hu,Wentong Zhai,Bo Zhao,Shuai Liang*

Main category: cs.RO

中文摘要：在接触密集型操作中，可靠的动作评估需要超越当前观测，关注未来的视觉与接触后果。现有的噪声空间强化学习能够高效地引导一个冻结的视觉-语言-动作（VLA）策略，但其评论家（critic）在很大程度上忽略了这些后果。我们提出了Imagine-RL，它通过动作条件化的视觉-力矩想象（action-conditioned visual-torque imagination）来增强噪声空间VLA后训练。对于每个候选动作块，一个冻结的视觉-力矩潜在世界模型（VTLWM）自回归地预测紧凑的未来表示，而无需像素重建。一个当前图像-状态-动作查询（query）对观测到的历史和预测的未来进行注意力计算，而先前窗口的预测残差提供逐令牌（token-wise）的置信度先验，以抑制不可靠的未来令牌。通过将当前证据与预测的后果相结合，动作评论家能够更好地评估候选动作并监督actor，同时VLA和VTLWM保持冻结。在四个真实机器人任务上、每个任务进行50次评估试验，Imagine-RL仅使用100条RL轨迹，平均成功率相比DSRL提高了（23.6%），相比VLA基线提高了（60%）。
...
### [32] [Grounded Action Model: 3D Grounding as a Foundation for Robotics](https://arxiv.org/abs/2609.23863v1)
具身动作模型：以三维定位作为机器人学的基础

*Gehao Zhang,Weikai Huang,Shailesh Shailesh,Yiyan Peng,Jiafei Duan,Ranjay Krishna*

Main category: cs.RO

中文摘要：操作策略必须知道哪些物体重要以及它们在哪里，然而当前机器人基础模型所依赖的预训练骨干网络——从视觉-语言-动作模型（VLAs）中的语言，到世界-动作模型（WAMs）中的视频生成——并不直接要求这种度量定位（metric grounding），而是将其留待从机器人示范中隐式学习。我们提出具身动作模型（Grounded Action Models, GAMs），这是一种以三维定位构建的机器人基础模型新范式。GAM 可以以语言、点或框提示为条件，这些提示首先被转换为所选物体的共享的以物体为中心的表征。该表征捕捉以目标为中心的视觉特征与度量物体几何，并通过多流 transformer 与机器人状态历史混合，以预测动作块（action chunks）。尽管 GAM 可以自主运行，它们也可以作为低层控制器，由高层规划器使用其多种输入模态进行控制，从而实现长时程和依赖记忆的操作。在 RoboTwin 2.0 上，GAM 在 50 项任务上取得 55.3% 的平均成功率（相较于 Spatial Forcing 的 52.0%），其中在场景随机化下为 47.6%（相较于 Abot-M0 的 30.4%），而其动作策略仅在干净场景示范上训练。在 LIBERO-PRO 上，它在 16 种扰动设置下取得 61% 的最先进平均成功率（相较于 $π_{0.5}$ 的 53%），在目标被重新放置或新指定时增益最大。在两台真实机器人上，GAM 在双臂 YAM 上于视觉偏移下保持 17/20 的成功次数，而 $π_{0.5}$ 为 4/20；同时，其在 Franka 上与 Molmo2 规划器组合后，在长时程和依赖记忆的任务上取得 64.7% 的 ID 和 49.8% 的 OOD 步骤完成率。
...
### [33] [WOLF: World Model Guided LiDAR Exploration with Predictive Frontiers](https://arxiv.org/abs/2609.23656v1)
WOLF：世界模型引导的预测性前沿LiDAR探索

*Yuyang Tian,Penghui Yang,Pengyuan Wu,Haoran Yang,Chenhui Li,Pengfei Han,Dong Wang,Zhigang Wang,Bin Zhao,Xuelong Li*

Main category: cs.RO

中文摘要：基于LiDAR的无人机（UAV）探索通过持续选择下一个观测位置来构建地图。然而，基于已测地图的决策对遮挡背后的空间延续具有有限的预见性，使得潜在的信息丰富方向无法被识别。我们提出WOLF，一个由世界模型引导的框架，通过预测未来观测来增强自主探索。在训练阶段，一个循环世界模型从探索轨迹中学习观测动态，其循环记忆保留了解读连续视角下部分观测所需的空域上下文。基于该上下文，模型在探索过程中将观测历史与候选运动相结合，以预测局部占据与可见性。为进一步引导感知，一种预测性前沿生成机制随后利用置信度、分支一致性（branch agreement）和观测质量对这些预测进行对齐与融合，以识别有前景的区域。由此产生的预测性前沿与已测前沿相结合，以引导几何视点选择与轨迹生成，同时新的扫描更新后续预测。在仿真中，我们的方法在Garage环境中以相当的覆盖率将平均终止时间相对EPIC减少10.9%，并在Tunnel环境中将平均覆盖率从42.12%提升至98.35%。真实世界实验进一步展示了所学模型的机载部署，以在物理飞行期间进行在线推理。
...
### [34] [PRIMO: Prior-Informed Odometry from Human-Motion Tracking for Humanoid Robots](https://arxiv.org/abs/2609.23610v1)
PRIMO：面向人形机器人的基于人体运动跟踪的先验知情里程计

*Xu Han,Angsong Li,Shaopeng Zhang,Enyu Li,Peiwen Lin,Chuang Wang,Yuan Zhuang,Haiyu Lan*

Main category: cs.RO

中文摘要：经仿真训练的人形机器人本体感知里程计面临两个迁移挑战：由面向部署的特定机器人控制策略生成的训练轨迹仅覆盖有限范围的运动，而仿真到现实（sim-to-real）的差异会使无约束预测不可靠。我们通过基于人体运动跟踪的先验知情里程计（Prior-Informed Odometry from Human-Motion Tracking，PRIMO）同时解决这两方面问题。在数据方面，我们让仿人机器人在仿真中跟踪多样化的重定向人体运动，以此生成里程计监督信号，从而将监督信号与部署策略解耦，并拓宽训练运动分布。在模型方面，一种先验知情估计器使用物理知情与对称性知情先验来构建速度与旋转预测，并使用一条粗糙原始上下文通路在保留编码特征的同时保留传感器上下文，从而增强仿真到现实的泛化能力。在统一的真实机器人协议下，PRIMO 在每个域-指标对比中相较评估的最强外部基线将平均误差降低 31.6%-61.7%。在两个运动策略修订版本之间，策略专用模型表现出对称交叉，而跟踪-运动（Tracking-Locomotion）训练将相反策略的平均仿真误差降低 86.8%-94.6%。在真实动态运动上，相对于在两个部署策略的并集上训练，跟踪-运动训练将平均误差降低 69.2%-81.7%。在所测试的运动组合中，先验知情估计器在仿真与真实机器人评估中均持续降低平均轨迹误差，优于其无约束对照版本。代码见 https://github.com/Agibot-Spatial-Intelligence/PRIMO。
...
### [35] [AR-WAM: A Visual-Conditioned Agent-Ready World Action Model for Robotic Manipulation](https://arxiv.org/abs/2609.23578v1)
AR-WAM：一种面向机器人操作的视觉条件化、可供智能体直接使用的世界行动模型
...
中文摘要：随着 AI 智能体能力日益增强，由智能体驱动的机器人控制正成为一种颇具吸引力的范式。然而，当前主流的视觉-语言-行动（VLA）模型与世界行动模型（WAM）仍依赖自然语言指令来指定操作任务，而这对智能体驱动的控制而言是一种不合适的接口：存在指代歧义、空间上不精确、与智能体自身固有的语言理解能力冗余，并且将意图与执行纠缠在一起。我们提出 AR-WAM，一种视觉条件化、可供智能体直接使用的世界行动模型，它以两个互补条件取代语言：一个视觉定位提示（目标物体的边界框），用于指明交互对象与位置；以及一个可学习的操作 token，用于指定要执行的原子技能。我们的紧凑型 0.5B 参数模型采用冻结的预训练视觉编码器且不含语言编码器，在紧凑的隐状态中预测场景演化，同时解码动作，并通过显式、可监督的推理信号暴露策略的意图。一个与模型无关的兼容层提供三种原语（detect、execute 和 query），使本地 VLM 或在线智能体 API 能够直接驱动该策略，而长时程记忆与闭环错误恢复则交由智能体一侧负责。在 RoboTwin 2.0、RMBench 以及真实的 Astribot S1 双臂平台上，AR-WAM 在标准操作任务上达到与最强基线相当的水平（平均成功率 87.2%），并在依赖记忆的任务和真实机器人长时程任务上优于这些基线，成功率分别提升 5.9% 和 36.7%，同时保持最低的推理延迟（14.1 ms）。
...
### [36] [RopeFormer: Cross-Trial Adaptation from Interaction History for Dynamic Rope Manipulation](https://arxiv.org/abs/2609.23432v1)
RopeFormer：面向动态绳索操作的基于交互历史的跨试验适应

*Menglin Wu,Kaixiang Yao,Shangbo Luan,Masayoshi Tomizuka,Yuxin Chen*

Main category: cs.RO

中文摘要：动态绳索操作对未知的物体动力学高度敏感：相同的机器人运动在不同绳索上可能产生显著不同的响应，而显式辨识相关物理属性则十分困难。我们提出 RopeFormer，一个以历史为条件的框架，利用先前的任务交互作为后续控制的上下文。该策略保留跨试验的动作-响应历史，同时保持其权重固定，且不需要显式的在线绳索参数估计。在持续单臂旋转、双臂旋转与瞬态挥鞭的匹配仿真评估中，相较于重置同一个检查点，保留上下文能够改善后续控制，且其收益随绳索动力学与观测设置的不同而变化。我们进一步将冻结的策略部署到 Unitree H1-2 上，使用此前未见过的实体绳索。从 T1 到 T3，Rope Swing 的目标获取时间减少 30.9%，Rope Twirl 减少 33.9%，而 Rope Whip 的平均目标命中次数从 0.2 增加到三次中的 2.3 次。这些结果表明，先前的交互能够为动态可变形物体操作提供有效的控制上下文。机器人视频、代码和数据可在 https://ropeformer.github.io/ 获取。
...
### [37] [Robot World Models Are Not Invariant to How the Actions Are Written](https://arxiv.org/abs/2609.23252v1)
机器人世界模型对动作书写方式并非不变
...
中文摘要：机器人策略使用两种动作参数化之一进行训练：绝对关节目标，或相对于当前状态的增量。这一选择是机器人学习中一项现实存在的工程决策，而以动作为条件的世界模型会悄然继承它。我们表明这种继承是灾难性的。一个在一种参数化上训练的潜在动力学模型，若被输入以另一种参数化书写的完全相同的指令轨迹，便会崩溃：在三个机器人数据集和两种形态上，检索性能下降 2.6–13.4 倍，目标条件动作选择从 53% 降至 15%，而在 PushT 上，关于同一未来的两种信念近乎正交（cos = 0.067，最坏情况 -0.377），因此该预测器并非优雅退化，而是在回答另一个问题。这并非通常意义上的分布偏移伪影：在给定关节输入的情况下，两种编码可以以 R^2 = 0.996 相互重建，因此没有信息丢失；我们给出了区分有效重参数化与有损摘要或传感器替换的检验。该检验拒绝了我们所提出的四个轴中的三个。缺陷存在于动作通道中，而针对视觉模型的不变性文献并未考察这一通道：那里的工作关注裁剪、抖动和相机位姿，而指令的参数化却未经审计。修复方法是对两种编码取平均，而平均的位置至关重要。对目标函数取平均本身即可恢复任务性能；对输出取平均虽因凹性而对概率是安全的，却无法用于方向值预测，因为归一化均值可能比轨道中任一成员的得分都低。目标函数平均所留下的是尾部：最坏情况一致性停留在 0.78，加入一项不一致惩罚将其提升至 0.995，而在一次潜在展开中，这正是一个逐渐侵蚀的最坏情况与一个得以保持的最坏情况之间的差别。在 PushT 上，仅靠平均并不能修复该轴。
...
### [38] [Probabilistic Scene Graphs: Hierarchical Representation and Real-time System](https://arxiv.org/abs/2609.23144v1)
概率场景图：层次化表示与实时系统
...
中文摘要：3D 场景图为机器人感知提供了语义丰富且层次化的表示。然而，现有系统并未将不确定性作为显式的信念来维护，也未在构建与细化图的操作中传播该不确定性。我们提出概率场景图（Probabilistic Scene Graph，PSG），它是对传统场景图的推广，用于表示可能图上的后验分布，该后验被分解为实体、关系和语义属性的离散图结构，以及在空间上对其加以定位的连续状态，并对这两个部分均维护不确定性。几何信息由节点直接承载，而非从另行构建的度量地图中选取，因此度量地图（在需要时）是由图推导而来，而不是先于图存在。我们用高斯层次图（hierarchical graphs of Gaussians，HGG）来实例化 PSG 的概率空间定位：每个物体基元由 Normal-Inverse-Wishart 信念下的全协方差高斯表示，而在一个节点内部递归地应用同一参数化，则产生一个几何图，以更细的分辨率解析其表面。随后，我们构建了一条在建图与细化过程中始终保持这些信念的建图流水线：一种纯基于图的由粗到精对齐通过比较节点信念来配准观测，而嵌套的期望最大化与因子图优化则联合细化位姿、物体参数和内部几何。在跨室内 RGB-D、室外 LiDAR 以及跨模态部署的六个数据集上，HGG 以传感器速率运行，内存近似恒定，并实现了最先进的物体精度与零样本图对齐。
...
### [39] [Verti-WM: A Physics-Aided Exteroceptive World Model for Off-Road Reinforcement Learning](https://arxiv.org/abs/2609.23118v1)
Verti-WM：一种用于越野强化学习的物理辅助外部感知世界模型

*Chenhui Pan,Tong Xu,Xuesu Xiao*

Main category: cs.RO

中文摘要：越野导航的强化学习需要大量车辆-地形交互数据，而在高保真仿真中采集这些数据成本高昂。世界模型提供了一种有前景的替代方案，可在策略优化过程中替代模拟器推演。然而，越野世界模型必须将状态转移动基于外部感知的地形信息，而仅凭本体感知无法提供此类信息。这一挑战因需要同时建模刚性地形与可变形地形而进一步加剧，而在这一方面，数据驱动方法与基于物理的方法各自具有互补优势。我们提出 Verti-WM，一种物理辅助的外部感知世界模型，它循环融合用于刚性地形的冻结 Transformer 与用于可变形地形的神经符号地形力学模型。在每个预测位姿处，从给定地图查询得到的高程与语义观测为融合提供条件，从而支持六自由度推演以进行策略优化，而无需进一步访问模拟器。Verti-WM 相较数据驱动基线和基于物理的基线，分别将预测误差降低 34.6% 和 21.7%。完全在 Verti-WM 内训练的策略达到了可比的任务成功率，同时相较在高保真模拟器中直接训练，计算时间减少 23.6 倍。我们进一步使用真实世界数据验证 Verti-WM，使其能够在学习得到的真实世界运动动力学内进行策略优化，并在 Verti-4-Wheeler 平台上达到 80% 的成功率，而直接进行仿真到现实迁移的成功率为 40%。
...
### [40] [PileBelief: Persistent Physical State for Interaction-Driven World Modeling](https://arxiv.org/abs/2609.22858v1)
...
中文摘要：世界模型使机器人能够在执行动作之前预判动作的后果。这一能力在挖掘作业中尤为重要，因为每一次铲挖都会重塑地形并影响后续动作。然而，局部观测无法完全揭示底层的支撑条件与物料状况。我们提出 PileBelief，一种面向部分可观测挖掘场景的交互驱动持久世界模型，它能够保留可见表面之外的物理证据。该模型将观测条件化的物理先验与世界地址化形变记忆和物理响应记忆相结合。动作对齐读取与门控残差修正可改进地形变化与结果的预测。在部署权重固定的情况下，已完成的交互会更新实测信念，而假设性动作则推进一个单独的想象状态。与仅使用当前观测的基线相比，PileBelief 将五步联合预测误差降低 10.8%，将离线动作选择遗憾降低 65.5%。在 Newton/MPM 以及真实挖掘数据集上的实验进一步表明，其在地形变化与铲斗体积预测方面均有改善。我们的方法能够从局部观测出发实现多步预测与候选动作排序，即便底层土壤状态未知。这些结果说明，持久物理信念是机器人为持续重塑的环境构建世界模型时一种有用的表示形式。
...
### [41] [Expert-Play Contouring Control: Faster-than-Demonstration Planning from Slow Expert and Fast Play](https://arxiv.org/abs/2609.22798v1)
专家-玩耍轮廓控制：从慢速专家与快速玩耍实现快于示范的规划

*Seunghoon Cho,Wonsuhk Jung,Sundhar Vinodh Sangeetha,Shreyas Kousik*

Main category: cs.RO

中文摘要：专家示范通常规定了机器人应当做什么，但没有规定它能以多快的速度完成。模仿学习（IL）继承了示范的时间节奏，而当更快的执行改变了机器人-物体动力学时，直接加速所学运动可能会失败。我们将快于示范的执行作为一个动力学感知的控制问题来研究，并提出专家-玩耍轮廓控制（EPCC），它结合了慢速专家示范与快速的非专家玩耍。专家示范训练一个潜在轨迹生成器，其预测被重参数化为一个与时间无关的、表示成功任务推进的轮廓，而玩耍则训练一个快速动作结果的世界模型（WM）。在部署时，我们提出的规划器使用该 WM 来优化动作，使沿专家导出的轮廓的推进最大化，同时对偏离预期任务演化的行为进行惩罚。在三个视觉运动操作任务上平均而言，EPCC 达到了 IL 基线 $2.0\times$ 的吞吐量，其中在一个具有交互敏感物体动力学的任务上达到了最强加速基线吞吐量的 $2.2\times$。我们的分析表明，收益集中在更快的执行改变机器人-物体演化的地方。总之，我们的结果凸显了一个简单而有效的原则：示范提供任务意图，而玩耍数据提供更快执行该意图所需的动力学覆盖。
...
### [42] [Latent Policy Steering: An Efficient and Flexible Framework for Cross-Embodiment Transfer](https://arxiv.org/abs/2609.22521v1)
...
中文摘要：学习得到的机器人视觉运动策略的性能在很大程度上取决于其训练数据的规模与质量，然而在真实世界中为机器人采集高质量演示数据仍然成本高昂。尽管大规模机器人数据集与人类数据集日益可得，但本体差异（embodiment gaps）与动作空间不匹配使其难以被直接利用。因此，跨本体迁移（cross-embodiment transfer），即复用来自其他本体的经验以改进目标本体上的学习，对于将机器人学习扩展到超越逐机器人数据采集的规模至关重要。在本工作中，我们发现，通过从各本体之间共享的内容——即世界如何响应运动的视觉动力学——中学习，并在测试时有效利用稀缺的目标本体数据，可以实现高效迁移。所提出的框架名为潜在策略引导（Latent Policy Steering, LPS），它实现了一个与本体无关的预训练阶段，即在多种本体上训练一个带有光流的基于图像的世界模型（World Model, WM）。随后，所得的世界模型在目标本体上使用机器人动作进行微调。接着，它通过在世界模型的潜在空间中搜索与微调数据保持接近的计划，将基础策略引导向更好的动作。LPS 是一个与策略无关的框架：它能够灵活适配不同的策略，而无需重新训练它们。在 Robomimic 与真实世界评估中，仅在一个未见过的目标本体上使用 50 条演示数据，LPS 便将 Diffusion Policy 的平均性能相对提升 16% 和 62%，将 Pi0.5 相对提升 8% 和 14%。
...
### [43] [AffordanceWAM: Affordance-Aware Joint World-Action Modeling for Robot Manipulation](https://arxiv.org/abs/2609.22332v1)
AffordanceWAM：面向机器人操作的具身可供性感知联合世界-动作建模
...
中文摘要：可泛化的机器人操作需要预测场景将如何演化、识别在何处交互是可行的，以及确定如何行动。带有动作标注的机器人视频可直接监督控制，但成本高昂且多样性有限；而第一人称人类视频捕获了多样的交互，却缺乏机器人动作，并在具身形态与外观上存在差异。我们提出 AffordanceWAM，一种具身可供性感知的生成式世界动作模型（World Action Model），它在生成的未来世界（World）中通过标量可供性（Scalar Affordance）和可供性热力图（Affordance Heatmap）来表示以物体为中心的时空可供性。该表示将视觉预测锚定到与任务相关的物体和交互区域上以生成动作，并在人类视频与机器人视频之间提供共享的交互目标。AffordanceWAM 构建于预训练视频扩散 Transformer 之上，使用分别参数化的世界专家（World Expert）与动作专家（Action Expert），通过掩码联合自注意力（Masked Joint Self-Attention）耦合，在统一流匹配（flow-matching）目标下联合预测未来 RGB 观测、标量可供性场、可供性热力图以及连续机器人动作。人类视频监督全部三个未来世界流，而机器人轨迹还额外提供动作监督，从而无需人类动作标签或动作重定向即可实现迁移。在 RoboCasa、CALVIN ABC$\rightarrow$D 以及真实世界操作上的实验表明，相比仅使用 RGB 和仅使用机器人数据的基线，本方法取得了持续一致的提升。在固定机器人监督下，RoboCasa 的性能随着具身可供性标注人类视频规模的增大而单调提升。这些结果支持将可供性作为视觉-语言-动作学习与人类到机器人迁移的有效接口。
...
### [44] [EditWM: Event-Decomposed World Modeling with Incremental Correction for End-to-End Autonomous Driving](https://arxiv.org/abs/2609.22317v1)
EditWM：面向端到端自动驾驶的事件分解世界建模与增量修正

*Junjie Yang,Qingwei Zeng,Youyou Li,Zicheng Ding,Ziyi Shi,Shuqi Shen,Hongliang Lu,Hai Yang*

Main category: cs.RO

中文摘要：世界模型通过预测与候选轨迹相关联的场景演化来支持自动驾驶。驾驶动力学在可预测性上存在差异，这促使人们区分常规演化与事件引发的偏差，后者需要选择性修正。我们提出 EditWM，一种在紧凑视觉特征空间中将未来预测分解为正常演化与事件驱动增量修正的世界模型。一个以轨迹为条件的正常预测器提供基础预测，随后被冻结以用于修正学习。一个修正解码器将该预测与观测历史和规划动作进行比较，产生一个有界的特征更新，其贡献由一个学习到的门控进行调节。修正后的未来特征通过候选特定的交叉注意力为轨迹评分提供条件，从而将世界建模与规划选择相连接。在推理时，EditWM 仅使用过去和当前的观测、自车状态以及候选轨迹。在全部 12,146 个 NAVSIM navtest 场景中，以专家轨迹为条件的评估显示，未来特征 MSE 相较 Normal 降低 5.35\%，并在 83.54\% 的场景中取得改进。使用官方 EPDMS 评估器，该系统在百分制上达到 91.05 EPDMS。这些结果表明，当修正后的未来表征被集成到规划中时，未来特征预测得到改善，且轨迹选择具有竞争力。
...
### [45] [ORDER: A Fictitious-World Benchmark for Domain-Adaptive Embodied AI](https://arxiv.org/abs/2609.22285v1)
ORDER：面向领域自适应具身智能的虚构世界基准

*Sai Krishna Reddy Sathi,Anuj Tiwari*

Main category: cs.RO

中文摘要：通过持续预训练使语言模型适应新领域，会引出一个基本的评估问题：如果训练语料与模型已知的内容存在重叠，那么性能提升就无法被清晰地归因于新学习到的知识，而非已有知识。这一点对于知识密集、任务轻量（KHTL）的机器人部署最为重要——例如药品配发、危险材料处理、特定设施专属规程，在这些场景中物理任务很简单，但支配性规则是专有的且关乎安全，而且进行大规模现场测试成本高昂或不安全。我们提出 ORDER（Ontology-driven Decision-making for Embodied Reasoning，本体驱动的具身推理决策），这是一个构建于虚构世界之上的基准：一个 342,069 token 的合成语料库，定义了一套自洽的物理规则，其不可能出现在任何模型的预训练数据中。ORDER 将一个 500 题的知识测试（ORDER-BENCH）与一项更难的组合任务 ORDER-SPATIAL 配对：在熟悉场景和完全新颖场景中对物体进行排序以实现安全操作。未经适应的 GPT-4.1 在 ORDER-SPATIAL 上得分低于随机水平（Kendall's tau = 0.441），表明其先验与所发明的物理规则主动冲突。经过持续预训练后，小模型在熟悉场景和新颖场景上均取得大幅提升——这证明是真正的世界模型归纳，而非记忆。随后我们将这一流程推进到机器人流水线：在知识测试中回答良好的模型，往往在没有进一步技能适应阶段的情况下无法生成有效、可执行的计划；而在加入该阶段后，小型、完全离线的模型即便在 GPT-4.1 被赋予对同一套规则的检索访问能力时也仍优于 GPT-4.1（Kendall's tau = 0.848 对 0.606），并在一个完整的从感知到执行的闭环上得到验证，该闭环在带有人工在环纠正的仿真 iiwa7 机械臂上进行了演示。贯穿始终的是，预测真实计划质量的是 ORDER-SPATIAL 的表现，而非知识测试的准确率。
...
### [46] [Monet: Measuring the Ecosystem of Open-Source Text-to-Image Models Tailored for Harmful Services](https://arxiv.org/abs/2609.24134v1)
Monet：测量面向有害服务的开源文本到图像模型生态系统

*Zihao Wang,Jiacen Xu,Zilong Lin*

Main category: cs.CR

中文摘要：开源文本到图像（T2I）生态系统促进了模型的快速开发与共享，但也托管着有意为有害服务量身定制的模型，我们将其称为 Monet。既有工作已在单一平台上考察了特定类型的有害 T2I 模型，但 Monet 并非孤立存在。更广泛的 Monet 生态系统——涵盖模型特征、跨平台传播、治理规避、变现以及下游部署——仍缺乏充分理解。在本研究中，我们首次对 Monet 进行系统性的生态系统层面测量。以真实世界模型中心的政策为依据，我们构建了包含十类有害服务的分类体系，并在八个主要 T2I 模型中心识别出 23,947 个 Monet，其中下载量最高的超过 1900 万次。尽管部分开发者采用防盗机制以应对未经授权的再上传，Monet 仍在平台上大规模传播，其中 40.76% 被镜像到多个模型中心。这种传播进一步通过跨平台存档实现治理规避，使 11.99% 的 Monet 在其原始平台被封禁后仍可访问，此外还存在关键词混淆与模型级防护绕过等其他规避策略。Monet 还支撑起协同的商业活动——一项涉及 668 个模型、完成 914 笔委托，另一项则宣传灰市账号养殖服务——并通过 GitHub 项目与推理 API 触达用户，引发下游儿童安全担忧。这些发现揭示了平台孤立式防御的局限性，并凸显了对跨平台威胁情报、协同治理与技术防护的需求。
...
### [47] [A Patient World Model for Early Forecasting of Digital Health Campaign Outcomes: Capabilities and Limits](https://arxiv.org/abs/2609.23333v1)
面向数字健康活动结果早期预测的患者世界模型：能力与局限

*Yunlong Wang*

Main category: cs.LG

中文摘要：数字直接面向消费者（DTC）健康活动通常在事后才进行衡量。进行中的预测通常依赖于为每个截断点和预测期分别构建的独立分类器。我们将该任务视为一个动态系统问题，并构建了一个紧凑的患者世界模型。该架构为每位患者维护一个潜在状态，联合学习以暴露为条件的状态动态与每周转化风险，并向前推演至未来转化曲线。我们在一个包含147{,}173名患者和520万个处于风险的人周（person-weeks）的美国活动数据集上对其进行评估。在以记录的未来暴露为条件的回顾性评估中，该模型从第4周截断点预测截至第52周的剩余新品牌（new-to-brand）处方量的相对误差为2.9\%，从第8—26周截断点预测的相对误差为0.8—2.6\%。最强的非循环基线——在给定相同生存推演和信息的条件下，采用合并风险梯度提升模型——相对误差为13.6—33.1\%。按预测期分别训练的每预测期分类器表现则显著更差。一项Fisher信息分析表明，当转化事件稀少时，密集的下一暴露监督是必要的。移除该辅助目标会使处方量误差增加约$2$—$14\times$，而在更常见的专科医生就诊结果上并未带来一致的不利影响。我们还评估了情景模拟。将所有未来暴露关闭会使预测转化率从0.31升至0.89，这一模式与观察性暴露数据中的选择效应相一致。该结果凸显了将以暴露为条件的推演作因果解读的局限。
...
### [48] [Prioritized Rollouts for Efficient World Model-based Vision-Language-Action Policy Optimization](https://arxiv.org/abs/2609.22879v1)
...
中文摘要：视觉-语言-动作（Vision-Language-Action, VLA）模型已成为具身智能的一种强大范式，但用强化学习（RL）对其进行微调仍受限于真实世界机器人交互的成本。基于模型的强化学习（MBRL）通过使用学习到的世界模型生成推演（rollouts）以进行策略优化，从而降低这一成本。然而，随着 VLA 策略与世界模型规模的扩大，其计算开销变得高昂。现有方法通常同等对待各个状态，忽视了它们在策略改进效用上的显著差异。在本文中，我们表明策略不确定性有助于识别具有更大策略改进潜力的状态。策略仅在少数状态子集上表现出高不确定性，这些状态往往出现在决策敏感阶段，此时微小的动作差异即可改变任务结果，这表明在这些状态上的策略改进可能尤其有价值。基于这些发现，我们提出 U-GROW，一种轻量级、即插即用的采样层，它将更多的模型推演导向这些信息量丰富的状态。通过仅修改分支起始分布（branched-start distribution），U-GROW 可以集成到现有 MBRL 流程中，而无需改变策略优化目标。在仿真与真实世界操作任务中的实验证明了 U-GROW 的高效性与有效性，支持使用策略不确定性来引导经验生成。
...
### [49] [FIRM-WM: State-factorized factual-interventional recurrent modeling for reward-free visual planning](https://arxiv.org/abs/2609.22816v1)
FIRM-WM：用于无奖励视觉规划的状态因子化事实-干预循环建模

*Yilun Wu,Yunjian Zhang,Aobo Li,Mujiangshan Wang,Haitao Wu,Aqiang Zhang*

Main category: cs.LG

中文摘要：无奖励的潜在世界模型可以从离线视频中学习，并通过在预测的潜在未来中优化动作来解决新的图像-目标任务。这一设定对规划状态提出了两项要求：其坐标必须能够与目标图像进行比较。此外，其动力学必须保留速度、运动趋势、接触以及其他依赖历史的信息，而不局限于这些目标坐标。离线训练还造成第二处不匹配：每条记录轨迹只揭示一个事实未来，而基于采样的规划器会比较许多并非从同一状态出发所采取的动作。我们提出 FIRM-WM（Factual–Interventional Recurrent World Model，事实-干预循环世界模型），这是一个围绕这两处缺口设计的紧凑像素世界模型。其循环状态将一个带类型的、可与目标比较的配置与一个 128 维动态纤维（dynamic fiber）分离，后者用于预测，但被排除在终止目标代价之外。广泛的事实轨迹提供状态覆盖，而共同重置（common-reset）干预分支为替代动作序列提供被观测到的结果。在执行每个分支之前，我们重置环境，并恢复由环境的状态设置接口所暴露的相同记录值。在匹配的 CEM 规划和三个独立的完整流程随机种子下，FIRM-WM 在 TwoRoom 上达到 99.0$\pm$1.0%，在 Reacher 上达到 92.7$\pm$2.1%，在 OGBench-Cube 上达到 88.0$\pm$3.0%，而 LeWM 分别为 89.0%、88.0% 和 70.0%。所部署的模型使用 2.98--3.42M 参数，并在这些任务上记录到 2.13--11.60$\times$ 更低的规划时间。
...
### [50] [Contrastive World Models](https://arxiv.org/abs/2609.22175v1)
对比式世界模型
...
中文摘要：通过像素重建训练的世界模型在视觉复杂的环境中可能表现不佳，在这些环境中无关信息主导了目标函数，并使模型偏离与规划和控制相关的信息。我们提出对比式世界模型（Contrastive World Models），一种无需像素重建来学习潜在动力学模型的方法。在 Dreamer 的基础上，我们将标准世界模型目标中的观测重建替换为类似 Deep InfoMax 的下界，该下界最大化状态-动作序列与未来观测的局部块特征之间的互信息，从而鼓励状态表示保留对未来具有预测性的信息，而不要求模型重建视觉上无关的细节。我们在三个视觉复杂度递增的设置中通过小规模实验评估了我们的方法。在默认设置中，我们的方法与 Dreamer 以及一个动量预测基线相当；而一旦引入干扰物或自然视频背景，我们的方法便显著优于二者，同时由于完全移除了像素解码器，训练也更加高效。我们的方法具有通用性，除可获取状态-动作序列和未来观测之外，几乎不作任何假设。这些结果表明，基于对比式信息最大化（infomax）的目标是构建对视觉干扰因素具有鲁棒性的世界模型的一个有原则且富有前景的方向，这一特性对于将基于模型的 RL 智能体迁移到真实世界尤为重要。
...
### [51] [UniK: Universal Knowledge Perception for Digital and Physical AI](https://arxiv.org/abs/2609.23971v1)
...
中文摘要：两类具有变革性的人工智能系统正在重塑组织的运作方式：\textit{数字AI}（digital AI），它对企业知识进行推理以驱动聊天机器人和智能体工作流；以及\textit{物理AI}（physical AI），它从视频、游戏过程与传感器遥测中学习以控制机器人和自主系统。二者都面临同一个基础性瓶颈：大规模原始知识，横跨异构模态，被锁定在私有语料库中，而现有AI基础设施无法可靠或高效地访问这些知识。我们提出\textit{通用知识感知（Universal Knowledge Perception, UniK）}作为面向这两类系统的通用平台，覆盖从富文本、视频到分子数据和传感器遥测等多模态的完整知识生命周期（摄取、增强、索引、检索与持续评估）。我们提出UniK，它构建于Polymath Retrieval（在自动增强索引之上的多索引融合）之上，且无需任务特定的微调。在五个数字AI领域（医学文献、开放域问答、化学、法律视频程序以及政府开放数据）中，UniK与一个开源的700亿参数模型相结合，持续达到或超越规模大出数个数量级的前沿专有LLM：在政府数据上RAG准确率为76\%，而GPT-5为47\%；在医学问答上无需微调即达77.9\%；在所有开源化学流程中位居首位。我们表明，同一套基础设施可直接应对物理AI世界模型训练所面临的数据整理、索引与检索挑战，在其中知识问题更为困难，但在结构上是相同的。
...
### [52] [PhysAI-Bench: A Benchmark for LLM-Based Agentic Decision-Making in Autonomous UAV-Centric Physical AI](https://arxiv.org/abs/2609.23695v1)
PhysAI-Bench：面向以无人机为中心的自主物理人工智能中基于LLM的智能体决策基准

*Mohamed Amine Ferrag,Merouane Debbah,Abderrahmane Lakas,Manu Perumkunnil,Norbert Tihanyi*

Main category: cs.AI

中文摘要：物理人工智能（Physical AI）的最新进展加速了基础模型在无人机（UAV）等自主系统中的应用，这些系统必须在动态环境中感知、推理、规划与行动。现有基准评测物理感知、直觉物理、具身导航与协同推理，但很少评估实现可靠自主性所需的智能体决策能力。我们提出 \textit{PhysAI-Bench}，一个用于评估该能力的基准。它包含 10,178 个标准化决策实例，这些实例自动提取自自主无人机任务的对话轨迹。每个实例保留任务上下文、时间依赖关系、物理约束、模型上下文协议（Model Context Protocol, MCP）工具调用、智能体到智能体（Agent-to-Agent, A2A）交互、传感器观测以及人工智能原生的 6G 网络条件，包括延迟、丢包、吞吐量、边缘负载和网络切片。我们仅暴露每个决策之前的信息，防止未来事件泄漏，并近似在线决策。我们采用两阶段协议评估 29 个基础模型。我们在一个包含 35 个实例、经人工验证的开发集上，从零样本、三样本和五样本提示与四个温度的 12 种组合中，通过三次运行选出模型特定的配置。随后，我们冻结每个选定的配置，并在一个固定的、按回合不相交的 500 个实例集合上通过三次运行进行评估。GPT-5.3 取得最高准确率（52.00%），其次是 GPT-5.2（49.40%）和 Grok~4.5（49.07%）。少样本提示通常能提升性能，而温度的影响有限。结果表明，物理人工智能中可靠的智能体决策仍是一个未解决的挑战。数据集可在 https://github.com/maferrag/physai-bench 获取
...
### [53] [Spatial-Interactor: Learning Spatial Reasoning through Interaction with the Observable Physical World](https://arxiv.org/abs/2609.23038v1)
...
中文摘要：空间推理对于视觉语言模型（VLMs）理解和在物理世界中行动至关重要。在动态环境中进行推理要求 VLMs 感知由物体运动与视角变化引起的局部状态转移，并在长轨迹上对其进行整合，以维持一个不断更新的空间状态，然而现有 VLMs 在这两种能力上仍然有限。当前的空间训练主要聚焦于关于物体属性和空间关系的静态问题，对状态转移提供的直接监督有限；相比之下，交互轨迹天然地将先前的观测、动作和随后的观测连接起来，为局部状态转移提供了直接监督，而完整轨迹则揭示了连续转移之间的依赖关系。因此，我们提出了 Spatial-Interactor，一个训练 VLMs 通过交互对物理世界状态转移进行建模的框架，并将这一学习过程组织为三级课程，涵盖 L1 被动世界状态转移、L2 主动自身状态转移和 L3 长时程交互轨迹。相应地，我们从模拟和真实交互轨迹构建了从空间交互中学习数据集（LSI-108K），其任务与每一级别的目标相一致。我们的两阶段训练策略对 L1 和 L2 应用监督微调（SFT）以进行局部转移建模，随后使用策略上蒸馏（OPD），其采用特权自蒸馏：一个被给予片段级转移描述的教师分支监督学生的策略上 CoT，帮助学生学会在 L3 长轨迹上整合连续的转移。在多个 VLMs 和空间基准上的实验表明，在局部转移建模和长时程整合方面均取得了一致的增益。
...
### [54] [Generative Embodied Multiple Behavior Control Systems for Human-like Agents](https://arxiv.org/abs/2609.22691v1)
面向类人智能体的生成式具身多重行为控制系统

*Chongyu Bao,Haokai Yang,Yuhan Wang,Zhaochong An,Kunpeng Liu,Xiaolan Liu*

Main category: cs.AI

中文摘要：认知神经科学中一个长期存在且内涵丰富的二分法是关于人类行为控制机制的划分，即习惯性行为与目标导向行为。现有类人智能体框架主要关注对目标导向行为的建模，而习惯性行为在很大程度上被忽视，尽管它在人类日常生活中起着至关重要的作用。在本文中，我们通过研究联合建模目标导向行为与习惯性行为的多重行为控制系统来填补这一空白。我们提出一个受人类行为控制机制启发的框架，其中习惯控制器（Habitual Controller）从个性化习惯记忆中检索线索触发的行为，而目标导向控制器（Goal-directed Controller）采用情境感知的世界模型来预测动作后果并估计其价值。仲裁器（Arbiter）根据个体差异与瞬时内部状态动态平衡两个系统的影响。为在3D环境中重建多样化的人类水平行为指令，我们进一步开发了一个关键帧引导的3D动作生成模块。通过广泛的评估方法、人类研究和消融研究，实验结果表明我们的方法显著提升了类人性（human-likeness）表现。我们方法的有效性表明，利用习惯性行为与多重行为控制系统协调对于构建可信的具身类人智能体具有益处。
...
### [55] [Generative Tutorial: Towards Live Contextualized Visual Instructions for Physical Tasks](https://arxiv.org/abs/2609.24955v1)
...
中文摘要：物理任务的视觉指令通常在一个情境中编写，而在另一个情境中被遵循，这要求用户将所演示的工具、材料和空间关系转换到自己的环境中。我们提出“生成式教程”（Generative Tutorial），这是一个用于实时视觉指令的概念框架，它在用户的环境和任务流程中描绘预期结果和动作。针对最先进的图像和视频生成的形成性评估，在15项物理任务中识别出了失败之处和潜在益处。基于这些发现，我们构建了一个增强现实原型系统，该系统利用观察到的工作区情境以及前置动作的预测视觉结果，主动生成目标图像和演示视频。一项有24名参与者参与的实验室研究发现，与预先编写的指导相比，使用该系统时任务执行质量更高、感知到的工作区对应性更强，且步骤确认间隔更短。定性研究结果凸显了情境相似性如何塑造信任、生成错误如何影响解读，以及指导的呈现方式应如何适应用户需求，从而为未来设计提供启示。
...
### [56] [Mask-Aware Execution for Efficient JEPA Training](https://arxiv.org/abs/2609.22674v1)
面向高效 JEPA 训练的掩码感知执行
...
中文摘要：联合嵌入预测架构（Joint Embedding Predictive Architectures，JEPAs）正成为核心的表征学习原语，并成为视觉、视频、音频、脑动力学和时间序列等领域潜在世界模型的构建模块。尽管（具有）广泛部署的（潜力），当前的 JEPA 训练流水线效率低下：每个输入都要经过多个特定于掩码的分支执行，存在冗余的目标侧工作，以及受内存限制的 token 路由。这些开销随掩码数量的增加而增长，并限制了 GPU 效率。我们提出 M-JEPA，一种掩码感知的执行架构，在不改变学习目标的情况下重构 JEPA 训练。M-JEPA 将掩码无关的计算与掩码相关的路由分离，从而实现共享上下文编码器执行、带反向支持的融合 token 路由与切片、在目标 token 并集上的稀疏目标编码器执行，以及针对稀疏输入的掩码 patch 嵌入。由此得到的流水线在保持训练语义的同时，减少了计算、内存流量和同步开销。我们为五种 JEPA 变体实现了 M-JEPA，并在 NVIDIA A100 GPU 上对其进行评估。与最先进的基线相比，M-JEPA 在 2-10 个掩码时实现了最高 1.7 倍的端到端训练加速。另外，借助掩码 patch 嵌入，在高稀疏度下实现了 4.75 倍的 patch 嵌入加速。这些结果表明，执行层面的重构、而非对 JEPA 目标的改变，是高效 JEPA 训练的关键杠杆。
