...
FixAnything：基于视频生成先验的三维一致性渲染精炼

*Khiem Vuong,Deva Ramanan,Srinivasa Narasimhan*

Main category: cs.CV

中文摘要：使用三维场景表示（如高斯泼溅（3DGS）、神经辐射场（NeRF）、网格甚至点云）进行渲染视图时，当输入视图稀疏或目标视图远离输入时，会产生伪影。近期工作利用基于扩散的生成先验来缓解这些伪影，但这些方法针对单一表示专门设计，需要定制架构或大量重新训练。我们提出FixAnything，一个用于修复多种渲染伪影的统一模型。它通过改造预训练的视频生成模型来实现，仅需极少的修改和轻量级微调即可利用其隐式多视图先验。我们的关键洞见在于，即使是有噪声的渲染序列也保留了相机运动和粗略的场景结构，从而可以将清理任务表述为视频到视频的翻译。为了控制应保留哪些场景结构，我们引入一个表示干净像素的二进制掩码，使模型能够将输出锚定到高质量输入（例如训练视图），同时精炼其余部分。为了促使FixAnything生成支持下游重建的三维一致性渲染，我们使用相机位姿精度（通过运动恢复结构获取）作为直接偏好优化（DPO）的奖励信号。在四种不同的三维表示上，FixAnything通过轻量级微调持续提升渲染质量，证明单一的通用视频先验可以取代多个专用精炼流水线。该框架的简洁性使其无需架构重新设计即可直接采用未来更强大的视频模型。
...
### [2] [GeoWAM: Visual Geometry World Action Models for Autonomous Driving](https://arxiv.org/abs/2608.23486v1)
...
中文摘要：世界动作模型（WAMs）近来作为自动驾驶中联合建模场景演化与自车动作的框架而受到越来越多的关注。大多数现有的WAM在像素空间中学习场景动力学，将用于未来观测预测的视频生成主干网络与用于自车轨迹预测的动作头相结合。然而，像素仅提供了这些动力学的间接表示：它们将几何与运动同外观、纹理和光照纠缠在一起，迫使模型从二维观测中推断三维变换。我们认为，以点云表示的几何为驾驶提供了更自然的状空间，因为它显式地捕获了空间结构以及支配场景演化的刚性与非刚性变换，同时直接与执行驾驶动作的空间相对齐。基于这一见解，我们提出了 \textbf{GeoWAM}，一种面向自动驾驶的视觉几何世界动作模型。GeoWAM并非预测未来图像，而是经过预训练以预测未来场景几何，从而产生同时编码空间结构与时间演化的表示。一个几何条件化的动作头随后利用这些学习到的几何动力学来预测未来的自车轨迹。广泛的开环与闭环评估表明，视觉几何世界建模所生成的驾驶策略显著优于基于图像的方法，确立了未来几何预测作为自动驾驶有效预训练目标的有效性。
...
### [3] [Long-Horizon Audio-Visual Generation for Persistent Stories and Interactive Worlds](https://arxiv.org/abs/2608.23383v1)
面向持久故事与交互世界的长时程音视频生成
...
中文摘要：视频生成正从孤立的短片向长篇叙事与交互世界发展，这要求模型能够保持身份一致性、遵循用户控制，并在长时间展开中保持稳定。我们提出了JoyAI-Echo-1.5，一个具有两种专用变体的统一音视频生成系统。长视频变体引入了可组合的跨镜头记忆，该记忆聚合了多个先前镜头中的视觉证据以及从经过语音过滤的全镜头音频中提取的说话人线索，从而在文本、图像和记忆条件的灵活组合下实现持久的角色外观与语音身份一致性。世界模型变体将异构的导航输入转换为经过标定的公制6自由度相机轨迹，并通过几何感知的条件注入通路将其注入模型，从而实现在灵活视点下的控制器无关交互。为了支持高效的长时程生成，我们将双向音视频骨干网络转换为因果式少步生成器，采用渐进式教师驱动（progressive teacher forcing）以及在自生成展开上的短时程和长时程自梯度驱动（Self-Gradient Forcing）。实验表明该系统在两种设定下均表现出较强的性能。JoyAI-Echo-1.5在跨镜头一致性、视觉质量、文本对齐和语音保真度方面相较现有长视频基线均有所提升。其世界模型变体在WBench上排名第一，平均得分为81.7，并在SANA-WM-Bench上取得了领先的视觉质量和长时程持久性。综合来看，这些结果表明记忆、几何控制和基于展开的训练为生成连贯故事和持续演化的交互世界提供了实用的基础。项目页面：https://echo-team-joy-future-academy-jd.github.io/Echo-1.5-Page/。
...
### [4] [EchoWM: Open and Enterable Omnimodal World Models](https://arxiv.org/abs/2608.23189v1)
...
中文摘要：我们提出EchoWM，一种用于可进入的生成式媒体的全模态世界模型，它响应连续导航，同时联合生成720p视频、环境声音、音乐和语音。我们围绕相机意图组织交互：在第一人称场景中，相机意图指定观察者的运动，而在第三人称场景中，相机-角色动态从数据中学习，无需特定视图的控制器。离散命令和连续姿态被映射到共享的度量尺度相对6-DoF轨迹，通过数据集级校准在异构数据上保持运动幅度。为了联合学习音频-视觉生成和轨迹控制，我们构建了一个互补的数据引擎，并采用渐进式训练，随后进行自回归后训练以实现长时程生成。大量评估表明，\model在公开的世界模型基准上实现了强大的轨迹跟随和高视觉质量，支持跨不同主体的第一人称和第三人称交互，并在长时程生成中保持同步的环境声音和语音。
...
### [5] [BenthicFlow: Generating Extensible Underwater Environments via Flow Matching](https://arxiv.org/abs/2608.23173v1)
...
中文摘要：水下环境中三维场景理解的计算机视觉应用仍然具有挑战性，原因在于缺乏高质量的三维数据，以及基于地表训练的模型无法泛化到水下场景。为应对这一挑战，一种新兴趋势是利用生成模型来弥合数据域差距。然而，现有方法通过分别训练的模型事后拼接独立生成的瓦片来组装大规模场景，同时仅在单个勘测站点内展示异质景观。我们引入BenthicFlow，一个基于单一条件流匹配模型的统一框架，该模型联合生成对齐的纹理和深度图。一种受MultiDiffusion启发的采样过程在生成轨迹全程协调重叠窗口，从而无需单独的拼接模型即可生成空间可扩展的RGBD马赛克。生成的马赛克随后通过表面对齐的高斯曲面元（Gaussian surfels）被提升为显式的三维底栖环境。跨地理上不同的勘测站点的实验表明，BenthicFlow在保留站点特有外观的同时，生成与目标分布紧密匹配的一致、大规模三维场景。代码和训练模型可在 https://github.com/jacomof/BenthicFlow 获取。
...
### [6] [Loopy: Seamless Video Loop Generation via Anchored Looping Shift of Positional Embedding](https://arxiv.org/abs/2608.23090v1)
Loopy：通过位置嵌入的锚定循环移位实现无缝视频循环生成

*Haotian Dong,Wenjing Wang,Chen Li,Jing Lyu,Xin Wang,Di Lin*

Main category: cs.CV

中文摘要：循环视频对于网页图像、游戏开发和社交媒体等实际应用至关重要。然而，现有方法通常因忽略了视频生成模型如何感知时间顺序以及这与循环行为的关系，而无法生成高质量的循环视频。在本工作中，我们首次揭示了DiT内不同注意力层中的位置嵌入表现出不同程度的位控制，其中最显著的层起到锚定作用。我们将这一锚定层定义为循环视频的参考点，为其余层次提供强大的上下文先验，以促进无缝且连贯的视频内容的生成。基于这一发现，我们提出了一种锚定位置嵌入移位策略，根据各层的时间控制效果应用特定于层的移位长度，有效将DiT的时间感知从直线转变为圆形。利用该策略，我们开发了一个通用框架Loopy，用于高质量循环视频生成，支持RGB和RGBA视频，同时还支持身份控制、风格迁移等高级AIGC功能。实验表明，我们的方法在生成的循环视频中显著提升了时间一致性和视觉保真度。已发布的模型可在我们的网站上获取：https://donghaotian123.github.io/Loopy。
...
### [7] [Grounding Isn't Knowing: Do VLMs Need Object Localization for Spatial Reasoning?](https://arxiv.org/abs/2608.23074v1)
定位不等于知晓：视觉语言模型进行空间推理是否需要对目标的定位？

*Xiwei Liu,Yulong Li,Xinlin Zhuang,Xuhui Li,Zhixiang Lu,Haolin Yang,Imran Razzak,Yutong Xie*

Main category: cs.CV

中文摘要：视觉语言模型（VLM）能够回答空间问题，但将目标定位与空间推理联系起来的机制仍鲜为人知。空间推理在内部是否需要精确的目标定位，还是可以通过全局布局线索绕过显式定位，这一问题尚未得到充分探索。在本工作中，我们使用一套机制性可解释性工具，包括词元消融、分层探测、注意力剔除和因果中介分析，研究了两个具有代表性的模型系列，即LLaVA-1.5和Qwen2.5-VL。我们发现，空间关系预测遵循一个分阶段的从定位到推理的过程，其中目标对齐的词元建立了粗略的目标-参考锚点，而精确的边界框边界并非必需。位置信息在关系决策出现之前即可被解码，并且一小部分注意力头中介了定位和空间推理二者的因果效应。这两个任务共享早期与定位相关的处理过程，但最终依赖于部分不同的专门路径。通过严格的实验，我们从词元、层和头三个层面提供了VLM如何将目标定位转化为空间关系的说明，表明知道目标在哪里并不等同于知道它们如何关联。
...
### [8] [FOVEA: Focused On-Demand Visual Evidence Adaptation for Cache-Friendly Multimodal Speculative Decoding](https://arxiv.org/abs/2608.22883v1)
FOVEA：面向缓存友好的多模态投机解码的按需聚焦视觉证据自适应

*Hengjie Zhu,Dayan Wu,Zihao Zhang,Xinze Liu,Jingxuan Yu,Peng Fu,Zheng Lin,Weiping Wang,Ding Wang*

Main category: cs.CV

中文摘要：多模态投机解码通过允许轻量级草稿模型提出候选令牌，供更大的目标模型进行并行验证，从而加速视觉-语言模型。现有方法通常将草稿模型条件化在固定的视觉接口上，例如预定义的视觉令牌预算或静态压缩表示。然而，我们的受控视觉预算分析表明，视觉需求在不同任务和解码阶段之间存在显著差异，这意味着更多的视觉输入并不总是有益的。实际上，证据不足可能削弱视觉锚定，而过多的上下文则会增加开销并可能干扰草稿生成。我们提出FOVEA（按需聚焦视觉证据自适应），一种面向缓存友好的方法，它构建可复用的视觉记忆，并针对草稿状态动态检索有界子集。累积质量规则确定选择哪些条目以及选择多少个条目。所选条目被聚合成视觉读出，并通过轻量级门控残差校正与当前草稿隐藏状态融合。该校正不将视觉令牌插入自回归上下文，而仅修改传递给语言模型头的表示。在多个视觉-语言主干网络和多模态基准上的实验表明，FOVEA提高了草稿接受率和端到端解码速度，相较于自回归解码实现了高达$2.13\times$的加速。这些结果表明，状态条件证据检索是替代在多模态生成过程中使用固定视觉表示的有效方案。
...
### [9] [Direct, Parallel, or Sequential? A Comparative Study of Training-Free Multi-Subject Image-to-Video Generation](https://arxiv.org/abs/2608.22819v1)
直接、并行还是串行？免训练多主体图像到视频生成的比较研究

*Yanliang Qi,Kexi Chen,Muchao Ye,Haomiao Ni*

Main category: cs.CV

中文摘要：基于文本条件的图像到视频（I2V）生成技术发展迅速，但生成包含多个主体的视频仍然具有挑战性。模型必须同时保持每个主体的外观、分配不同的运动，并维持连贯的空间和时间交互。本文对免训练多主体I2V生成的三种代表性范式进行了系统研究：直接生成、并行生成和串行生成。直接生成将预训练的I2V模型应用于完整的参考图像和提示词，要求所有主体和运动联合合成。并行生成和串行生成则将参考图像和提示词分解为主题特定的视觉和文本条件。并行生成独立合成每个主体，随后将生成的视频组合起来，降低了每个生成步骤的复杂度，但代价是主体间上下文较弱。串行生成首先合成背景视频，然后逐步引入各个主体。这种方式保留了累积的场景上下文，但对主体排序敏感并存在误差传播问题。我们通过多种多主体场景对这三种范式进行了实证评估，比较了外观保持、运动保真度、时间一致性以及主体间连贯性，同时刻画了它们各自不同的失败模式。我们的研究结果揭示了每种范式的优势和局限性，并为设计可控的多主体视频生成系统提供了实用见解。
...
### [10] [Object-Uni: A Unified Model for Object-Centric Spatial Understanding and Controllable Generation](https://arxiv.org/abs/2608.22757v1)
Object-Uni：面向对象中心空间理解与可控生成的统一模型

*Mining Tan,Yinuo Wang,Ziqi Zhou,Weize Quan,Sifei Li,Jingdong Chen,DanDan Zheng,Libin Wang,Weiming Dong*

Main category: cs.CV

中文摘要：用于视觉理解与生成的统一模型取得了快速发展，但它们仍然缺乏理解和操作对象实例空间状态的能力。现有模型可以用自然语言描述对象，但在精确表示连续的对象位姿以及在目标视角下生成几何一致的图像方面仍存在困难。为缓解这一问题，我们提出了\textit{Object-Uni}，一个面向对象中心空间理解与可控生成的统一模型。具体而言，我们将对象中心空间智能表述为一个统一问题，连接位姿感知、空间推理、位姿条件生成和对象中心新视角合成。我们将对象位姿视为理解和生成共享的显式几何变量，而非仅仅是预测标签或控制信号。为了使位姿能够被多模态大语言模型使用，我们提出了一种基于视角的朝向抽象方法，将朝向映射为结构化的视角描述，同时保留连续的几何监督。我们进一步构建了一个对象中心空间基准（UniSpatial-80K），并训练了一个统一模型，采用对象令牌锚定的位姿锚点将每个实例与其位姿状态关联起来。实验表明，我们的模型提升了对象级位姿理解和位姿可控生成能力，推动统一模型从描述对象走向操控空间状态。
...
### [11] [OmniCAD: A Large-Scale Benchmark for 3D Spatial Reasoning in Robotics Assemblies](https://arxiv.org/abs/2608.22637v1)
...
中文摘要：最近的视觉-语言模型（VLM）在机器人感知和空间推理方面表现出强大的能力，但其对复杂机械装配体进行推理的能力仍未得到充分探索。我们提出OmniCAD，这是一个面向装配感知的三维空间推理的大规模基准，涵盖多种工业系统，包括机器人机构、汽车零部件、航空航天结构和农业机械。OmniCAD包含25,000个机械装配体，每个装配体平均有12个零件和21种配合关系类型。每个装配体都包含经人工验证的真实三维模型以及来自20个视角的渲染图。该基准评估三种能力：（1）组件级三维空间推理，要求预测零件的位置和姿态；（2）零件间关系推理，要求识别配合关系和装配约束；（3）工具增强的智能体推理，模型需迭代选择视角、检查视觉证据并完善预测。实验表明，当前的VLM在工业装配推理方面表现困难，常常产生不准确的姿态、无效的配合关系、零件相互穿透，并且随着装配复杂度增加性能下降。我们将开源该基准、评估代码和工具接口，以支持准确、物理有效且可扩展的三维装配推理研究。
...
### [12] [UR$^{2}$-MLLM: Uncertainty-aware Revisit Reasoning in Multimodal Large Language Models for Radiology Report Generation](https://arxiv.org/abs/2608.22217v1)
UR$^{2}$-MLLM：放射学报告生成中多模态大语言模型的不确定性感知复查推理

*Yucheng Chen,Yang Yu,Jiazhou Zhou,Yufei Shi,Yongying Lan,Yichi Zhang,Liyi Li,Si Yong Yeo*

Main category: cs.CV

中文摘要：放射科医生通过迭代且有选择性地复查可疑区域来完善其诊断结论，从而生成诊断报告。近年来用于放射学报告生成（RRG）的多模态大语言模型（MLLM）已从纯文本推理转向“图像伴随思考”（Thinking-with-Images）范式，将视觉证据融入推理过程。然而，现有方法在推理过程中提供静态视觉证据，缺乏动态复查机制，未能考虑放射科医生如何重新审视不确定的观察结果。为此，我们提出了一种不确定性感知的复查推理多模态大语言模型（UR$^{2}$-MLLM）框架，该框架在RRG推理过程中动态复查不确定区域。UR$^{2}$-MLLM首先通过在不确定性感知数据集上训练来具备不确定性感知能力。随后，我们构建了一个多模态推理轨迹数据集，并配以检测-复制（detect-and-copy）机制，用以指导何时何地进行复查。最后，视觉定位奖励通过强化学习优化该行为，使复查区域与相应的解剖结构对齐。在MIMIC-CXR和IU-Xray上的实验表明，UR$^{2}$-MLLM取得了最先进的性能，凸显了不确定性感知的视觉复查推理对于生成可靠且临床对齐的报告所具有的价值。
...
### [13] [GaussVid: Sparse-View Gaussian Splatting with 3D-Aware Video Diffusion Priors](https://arxiv.org/abs/2608.21849v1)
GaussVid：基于三维感知视频扩散先验的稀疏视角高斯泼溅

*Xinhui Liu,Can Wang,Wei Jiang,Wei Wang,Dong Xu*

Main category: cs.CV

中文摘要：三维高斯泼溅（3D Gaussian Splatting，3DGS）在新视角合成方面取得了显著成功；然而，稀疏视角下的重建往往表现出明显的伪影。虽然最近的视频扩散模型为3DGS修复提供了强大的时空先验，但直接对其进行微调用于修复并非最优，因为这些模型缺乏对底层多相机几何结构的感知，导致多视角不一致。在本工作中，我们提出了一种新颖的三维感知视频修复框架，旨在提升稀疏3DGS重建的质量。具体而言，我们构建了一个大规模3DGS视频数据集以实现专门的微调。为弥合二维视频生成与三维多视角约束之间的差距，我们引入了一种相机条件下的几何先验。通过使用首帧和末帧作为边界锚点，并编码相应的相机关系，我们将空间结构显式注入视频生成流程。这种基于边界锚定、具有相机感知的先验引导网络朝向几何上有依据且跨视角保持一致的修复方向。大量实验表明，在基于视频先验的修复方法中，我们的方法在像素级和结构级保真度（PSNR/SSIM）上取得了最佳表现，并提升了多视角一致性，同时在感知质量（LPIPS）上保持竞争力。
...
### [14] [WorldMind: Decoupled Game World Model for State-Aware NPC Behavior](https://arxiv.org/abs/2608.21439v1)
...
中文摘要：游戏世界模型近年来在生成视觉连贯且动作可控的游戏视频方面展现出令人瞩目的能力。然而，现有模型中非玩家角色（NPC）的行为要么隐式地与视频生成纠缠在一起，要么通过外部控制信号被显式地设定。因此，游戏世界模型必须同时理解状态、规划NPC的响应并渲染其视觉结果，这限制了其生成响应式且状态感知的NPC行为的能力。挑战在于缺乏用于状态夯实决策的显式接口。为此，我们提出了WorldMind——据我们所知，这是游戏世界模型中首个用于状态感知NPC行为的解耦框架。WorldMind将交互式世界建模分为四个层级：理解层（Understanding Layer）从生成的帧中构建紧凑状态；决策层（Decision Layer）基于该紧凑状态进行推理以规划NPC的下一步动作；控制层（Control Layer）将动作转换为时间对齐的条件；生成层（Generation Layer）合成其视觉结果。通过以闭环交互回路重新连接各层级，WorldMind将NPC行为扎根于不断演化的游戏状态之中。我们进一步引入了BOSS-140K——一个由游戏视频与丰富的内部游戏状态配对组成的数据集，以及一个可大规模自动化采集数据的智能体。在BOSS-140K上的实验证明了可靠的紧凑状态重建和基于机制的规划能力，WorldMind在约70%的成对比较中因其更符合战术逻辑且更连贯的NPC行为而被优先选择。项目页面：https://teawhite.cn/worldmind_projectpage/
...
### [15] [DesignAgent3D: Interactive 3D Scene Editing via Designer-like Multimodal Reasoning](https://arxiv.org/abs/2608.21438v1)
DesignAgent3D：通过类设计师多模态推理进行交互式3D场景编辑

*Xiujin Liu,Tianyu Yang,Yilun Zhao,Xiangliang Zhang*

Main category: cs.CV

中文摘要：文本引导的3D场景编辑为修改重建环境提供了直观的界面，但由于自然语言设计请求通常在语义上不明确，且必须在杂乱的三维场景中落地，因此仍然具有挑战性。现有方法通常将任务表述为从单一提示词进行的一次性条件生成，无法解决模糊的用户意图或实现精确的空间定位。因此，它们存在严重的物体定位漂移、遮挡下的跟踪失败以及臭名昭著的多视角“贴纸效应”。为克服这些限制，我们提出了DesignAgent3D，一个交互式多模态智能体框架，将3D场景编辑重新表述为类设计师的“规划-感知-行动”（Plan-Perceive-Act）范式。该智能体首先通过用户交互进行规划，以澄清不明确的设计目标；然后通过将预期编辑定位到3D场景中的特定对象或区域进行感知；最后在保持场景一致性的同时应用受控的视觉修改来执行操作。编辑结果进一步集成到底层3D表示中，支持持久且多视角一致的新视角渲染。在NeRF和3D高斯泼溅（3D Gaussian Splatting）两种骨干上的大量实验表明，DesignAgent3D显著优于最先进的基线方法，实现了卓越的语义意图对齐、无懈可击的空间定位准确性以及高保真的多视角一致性。
...
### [16] [Aligning Human Sense: Calibrated Distributional Reward Learning for Video Generation](https://arxiv.org/abs/2608.21425v1)
对齐人类感知：面向视频生成的校准分布式奖励学习
...
中文摘要：视频生成是AI驱动内容创作的核心。将生成的视频与人类偏好对齐是评估生成质量的关键标准。尽管在视觉质量方面取得了显著进展，但仍存在三个关键挑战。首先，奖励信号的可靠性受到人类偏好数据质量的制约，而后者往往受到主观噪声和偏见的影响。其次，标准标量奖励模型将多方面的人类偏好压缩为单一数值，导致多个偏好维度之间动态权衡的丧失。第三，在策略优化中，广泛采用的KL散度主要施加局部约束，可能无法捕捉人类偏好的全局结构。为解决这些挑战，我们提出了一个面向视频生成的统一偏好感知学习框架。首先，我们引入精英引导过滤（elite-guided filtering）来校准偏好数据，并为奖励模型训练构建可靠的监督信号。然后我们将视频质量建模为多维奖励分布，以捕捉人类偏好中固有的不确定性，并使用Wasserstein距离将学习到的奖励分布与经验人类偏好分布对齐。最后，我们将基于Wasserstein的分布式对齐引入GRPO，引导策略优化以更好地匹配人类对视频偏好的全局结构。在奖励建模和视频生成上的实验表明，我们的方法提高了奖励信号的可靠性和生成视频的感知一致性。我们的代码可在 https://github.com/alignhs26/ahs 获取。
...
### [17] [EditStream: A Unified Autoregressive Framework for Interactive Video Generation and Editing](https://arxiv.org/abs/2608.21424v1)
...
中文摘要：交互式视频生成与编辑对于创意设计正变得越来越重要。在本报告中，我们介绍了EditStream：一个用于交互式视频生成与编辑的统一框架。EditStream通过灵活的任务特定条件设定，在单个基于DiT的模型中统一多种视频创建与操作任务，并将其进一步转化为一个快速的、少步自回归模型以实现高效流式处理。它支持文本生成视频（Text-to-Video）、图像生成视频（Image-to-Video）、视频到视频（Video-to-Video）、编辑传播（Editing Propagation）、参考引导的视频编辑（Reference-guided Video Editing）以及相机位姿变换（Camera Pose Change），从而在一个系统内实现对视频生成、变换和编辑的灵活控制。为了使该统一模型适用于交互式使用，我们开发了一种两阶段蒸馏方法，将速度矩匹配（VMM）与自回归展开相结合。VMM在学生模型所到达的中间状态处匹配条件速度矩，以保持生成质量和运动；而展开（unrolling）则使学生模型暴露于其自身的自回归预测，以提高时间稳定性。两者共同缓解了少步自回归视频生成中的常见挑战，包括过饱和、运动退化、时间不稳定以及训练复杂性。EditStream提供了一种实用且可扩展的解决方案，在基于扩散的高质量视频模型与交互式创意工作流之间架起了桥梁。
...
### [18] [Future Querying: Can LLMs Serve as Implicit Medical World Models?](https://arxiv.org/abs/2608.23248v1)
未来查询：大型语言模型能否充当隐式医学世界模型？

*Siri Willems,James Butterworth,Lore Goetschalckx,Peter Vrancx,Philippe Modard,Elke Giets,Ludovic Denoyer*

Main category: cs.CL

中文摘要：传统的临床预测模型依赖于针对特定任务的流程和经过整理的、结构化的数据，这些方法扩展性差，且未能充分利用非结构化文本。为解决这一问题，我们引入了未来查询这一范式，通过评估大型语言模型（LLMs）回答关于患者未来的时间索引临床查询的能力，来探究它们能否充当隐式医学世界模型。我们的框架基于端点无关的训练，对非结构化临床文档进行操作，使单一模型能够在无需手动特征工程或针对特定任务重新训练的情况下，回答患者轨迹上的多种临床查询。我们表明，小型、本地微调的开源权重模型能够匹敌或接近更大的专有系统，从而使该框架适用于保护隐私的本地部署。在一种新的合成医学报告数据集和来自MIMIC-IV数据集的真实ICU笔记上进行评估后，我们的结果为大型语言模型能够捕捉临床动态的某些方面提供了令人鼓舞的证据。
...
### [19] [GUI-Primitives: Diagnosing Spatial Reasoning Failures in Vision-Language GUI Grounding](https://arxiv.org/abs/2608.21832v1)
GUI-Primitives：诊断视觉-语言GUI接地中的空间推理失败

*Md Abrar Jahin,Md Rizwan Parvez*

Main category: cs.CL

中文摘要：计算机使用代理将自然语言指令在截图中进行接地以定位界面元素，然而现有基准测试并未区分模型是否将关系语言正确绑定到相应元素上。我们引入了GUI-Primitives，这是一个包含994个对比指令对的基准测试，覆盖图形用户界面中的七种空间关系（左/右、上/下、包含、对齐、邻近、列表序数、遮挡）。每个指令对保持截图和锚点不变，仅改变关系表达式，因此正确的目标会在两个指定候选之间移动。五位标注者验证了一个包含196个项目的子集（$κ= 0.94$ 良好形式性；$κ= 0.79$ 目标选择）。十九个视觉-语言模型的最优严格点在框内准确率仅为$32\%$。由于模型输出不受约束的坐标，我们根据每个预测落入的候选区域对其分类。预测落在两个候选区域之外的情况占项目的$60-92\%$。在落入候选区域的前提下，水平位置、垂直位置、邻近和列表序数的目标选择准确率达到0.82-0.90，而包含和遮挡的目标选择准确率与0.50无显著差异：大多数失败反映了候选定位问题而非关系理解问题。在十个模型中，基准测试准确率与ScreenSpot-Pro准确率相关（Spearman $ρ= +0.74$），这在当前样本量下是一种探索性关联。标记两个指定候选可将选择准确率提高35至57个百分点，这是一种提供候选集合的oracle诊断手段，而非可直接部署的方法。我们发布了该基准测试、预测结果和代码。
...
### [20] [Reward-Free Continual Adaptation for Resilient Space Robots](https://arxiv.org/abs/2608.23452v1)
面向弹性空间机器人的免奖励持续适应
...
中文摘要：空间机器人在极端环境中运行，硬件退化可能严重危及传统控制策略。虽然持续强化学习为在线适应提供了一种有前景的机制，但它在部署期间本质上需要访问奖励信号。然而，由于缺乏外部跟踪系统以及环境整体复杂性，在太空中精确计算奖励往往是不可行的。为解决奖励不可观测的挑战，我们引入了一种利用潜在状态世界模型的免奖励持续学习框架。通过在多样仿真环境中预训练基于模型的智能体，世界模型在其潜在空间内学习对奖励结构的稳健预测器。在部署到硬件严重退化的环境后，我们冻结观测编码器和奖励预测器，仅通过无监督回放更新世界模型的转移动力学。通过完全在由该更新后的世界模型生成的想象轨迹上训练策略，智能体无需接收新奖励即可适应改变的动力学。我们在遭受严重形态学故障的模拟行星穿越、轨道导航和精密装配任务中展示了我们的方法。
...
### [21] [InstructMove: A Text-Indispensable Benchmark for Instruction-Following Manipulation](https://arxiv.org/abs/2608.22990v1)
InstructMove：一个指令遵循操控的文本不可或缺基准
...
中文摘要：视觉-语言-动作（VLA）模型通过将机器人动作与自然语言指令条件化，使通用型机器人操控日益可行。此类通用性的一个关键检验在于策略是否真正遵循语言指令。然而，许多操控基准使这种能力处于欠定状态：目标物体或目的地通常在视觉上显著或具有唯一可行性，使得策略无需理解（grounding）指令即可成功。我们认为，指令遵循评测应当是文本不可或缺的（text-indispensable）：多个动作应在视觉和物理上均可行，而只有其中一个动作与语言指令一致。我们提出了 InstructMove，一个用于指令遵循操控的文本不可或缺基准。InstructMove 在带有语义干扰物的抓取-放置场景中实例化了这一原则，将指令遵循分解为类别识别、属性判别、空间推理和组合式抓取-放置。InstructMove 支持一个包含 InstructMove 训练数据和留出评测任务的训练-评测协议，并附带用于语言依赖性诊断的额外诊断项。使用具有代表性的 VLA 策略进行的实验表明，InstructMove 为诊断视觉捷径提供了一个受控测试平台，并且 InstructMove 仿真数据能够提升真实世界指令遵循操控性能。代码：https://github.com/HorizonRobotics/RoboOrchardSim
...
### [22] [SuperMap: A Spatio-Temporal SLAM System for Visual-Language Navigation](https://arxiv.org/abs/2608.22896v1)
...
中文摘要：机器人在人类环境中的导航需要一种时空语义表示，它能够调和开放词汇感知与长期环境变化之间的关系。虽然基础模型提供了强大的零样本识别能力，但其预测是间歇性的且依赖视角，将其天真地集成到建图流程中会导致身份漂移和语义随时间过时。我们提出SuperMap，一种用于语言引导导航的4D时空建图框架，它将高频几何SLAM与异步开放词汇感知相结合。我们的核心贡献是一个一致性驱动的建图引擎，它将3D感知的实例关联/重新激活与原理性的存在性与标签置信度更新相结合，从而在遮挡和场景变化下维持稳定的物体身份并修剪过时的地图内容。SuperMap生成可查询的4D场景图表示，通过支持对物体语义、关系的组合式查询，与视觉-语言模型自然交互。我们在基准测试和真实机器人上展示了SuperMap，包括具有出现/消失和重定位的动态场景，并提供了消融实验和运行时分析。我们将整个系统作为开源发布，为社区提供一个可部署的开放词汇时空建图基线。项目网站：superodometry.com/supermap。
...
### [23] [LD4WAM: Learning Latent Dynamics from Human Videos for World Action Models](https://arxiv.org/abs/2608.22403v1)
LD4WAM：从人类视频中学习潜动力学以构建世界动作模型
...
中文摘要：人类视频在训练世界动作模型（WAMs）中正发挥着越来越核心的作用，这得益于其多样性和相较于遥操作机器人数据更低的采集成本。然而，大多数WAMs仅通过预测像素级未来帧来从此类视频中学习，从而获得不可直接执行的动力学；而运动重定向虽然能恢复可直接执行的动作，却在不同实体之间留下了巨大的视觉鸿沟。因此，我们提出运动对齐潜动力学（motion-aligned latent dynamics）作为一种与实体无关（embodiment-agnostic）的表征，以弥合视频先验与低级动作之间的差距。我们进一步提出LD4WAM，它将一个通过语义重建和真实运动对齐训练的潜动力学模型（Latent Dynamics Model）与一个基于混合变换器（mixture-of-transformers，MoT）构建的世界动力学动作模型（World Dynamics Action Model）相结合，后者保留了完整的未来视频生成能力，并使用可学习查询从生成的未来帧中蒸馏这些潜动力学以进行动作条件化。LD4WAM在我们精心整理、包含超过5,000小时人类与机器人数据的统一数据集上完成预训练，在RoboTwin仿真中以及配备夹爪和灵巧手的真实机器人上均表现出色，同时能很好地泛化到未见过的物体和背景。
...
### [24] [Beyond Instance Slots: Semantically Rich World Models for Physical Interaction Planning](https://arxiv.org/abs/2608.22294v1)
超越实例槽位：用于物理交互规划的语义丰富世界模型

*Juntao Cheng,Jingkai Wang,Yijun Shen,Xiansheng Chen,Zhiwei Yu*

Main category: cs.RO

中文摘要：用于物理交互的世界模型通常被训练为预测未来的观测或潜在特征；然而，一个面向规划（planning-oriented）的模型必须回答一个根本不同的问题：候选动作是否会产生与任务一致的未来，同时保持基本关系。整体式（monolithic）状态表示掩盖了底层实体，而标准的实例级对象槽位（instance-level object slots）仅仅识别存在\emph{什么}，却没有指明每个实体在任务情境中扮演\emph{什么角色}。为弥合这一差距，我们提出了语义丰富世界模型（Semantically Rich World Model, SR-WM），这是一个以任务为条件的、围绕五种功能角色构建的世界模型：夹爪（gripper）、目标（target）、目标状态（goal）、关系（relation）和阶段（phase）。在SR-WM内部，一个视觉实体编码器从预训练的块级（patch）特征中提取软实体假设，使分割掩码能够作为可选的提议先验（proposal prior），而不强制将其作为必需的状态表示或推理输入。一个角色绑定器（role binder）随后将这些假设映射到特定任务的角色，而一个动作条件动力学模型（action-conditioned dynamics model）则预测角色转移以及细粒度语义，包括抓取/接触（grasp/contact）、谓词建立（predicate establishment）、关系保持（relation preservation）、夹具状态（fixture state）和阶段变化（phase change）。至关重要的是，这一统一角色状态为下游的多候选动作生成、阶段感知的重排序（stage-aware reranking）以及违反感知的后缀重采样（violation-aware suffix resampling）提供了基础。我们全面的评估协议涵盖所有四个LIBERO仿真套件、跨套件迁移、感知诊断和动作敏感性分析。最终，这一公式将对象中心预测（object-centric prediction）转化为一个语义接口，将视觉动力学与面向规划的决策制定连接起来。
...
### [25] [DreamMimic: Learning Visuomotor Whole-Body Loco-Manipulation via World Model](https://arxiv.org/abs/2608.22278v1)
DreamMimic：通过世界模型学习视觉运动全身移动操控

*Jie Yin,Xingyu Lai*

Main category: cs.RO

中文摘要：基于视觉的人形机器人全身移动操控（loco-manipulation）因部分可观测性、接触丰富的动力学以及从高维视觉输入中学习长时程行为的困难而具有挑战性。我们提出了 \href{https://github.com/DreamMimic/DreamMimic}{DreamMimic}，一个通过世界模型辅助蒸馏将特权教师策略蒸馏为基于视觉的人形控制器框架。我们并未采用 Dreamer 风格的 RSSM 进行规划，而是将其重新用于学习预测性潜在动力学，它既作为表征空间，又作为动作条件化的多步监督信号，同时向学生策略暴露紧凑的预测特征以减少长期漂移。除了针对本体感觉和视觉观测的标准重建目标外，我们还为特权状态、接触、物体状态和奖励估计添加了辅助预测头。这些预测头提供与智能体—物体交互和任务进度相关的额外监督，鼓励潜在表征保留对接触丰富的移动操控有用的信号。我们进一步引入了性能条件引导（PCG），一种奖励驱动的自适应蒸馏调度方法，它同时计算教师和学生策略的性能分数，以动态平衡引导与探索。PCG 既防止在具有挑战性的视觉环境中过早削弱教师策略，又防止教师过度干预。在 OMOMO 和 BEHAVE 上的实验表明，与强视觉基线相比，跟踪式移动操控性能得到提升，且部署时无需向学生暴露在线特权交互状态。定性仿真进一步考察了形态学和仿真器变化。这些结果表明，世界模型可以为接触丰富的人形行为中的视觉策略蒸馏稳定化提供一种有效的机制。
...
### [26] [BehaviorWorldGen: Closing the Loop between Action Models and World Simulators via Controllable Behavior-Aware Structured World Generation](https://arxiv.org/abs/2608.22187v1)
BehaviorWorldGen：通过可控行为感知的结构化世界生成，闭合动作模型与世界模拟器之间的循环
...
中文摘要：现代驾驶动作模型越来越多地在一个自我改进循环中得到提升，其中学习到的世界模拟器设想未来的观测，并将生成的数据反馈以优化动作模型。然而，该循环的瓶颈在于模拟器无法生成周围智能体在行为上合理的响应，导致生成的数据既在交互上不真实，又在分布上不均衡。我们提出了BehaviorWorldGen，一个通过可控行为感知的结构化世界生成来闭合动作模型与世界模拟器之间循环的框架。其核心组件是BehaviorFlow，一个以元动作条件驱动的交通流模型，它注入可解释的行为控制，并联合生成多智能体轨迹展开。BehaviorFlow实现指定的智能体行为，同时允许周围车辆对自车及彼此作出响应。生成的轨迹展开由世界模拟器渲染为真实的多视角观测，并与校正后的交互感知轨迹配对，用于动作模型的优化。由于BehaviorWorldGen使用结构化轨迹作为其模块之间的接口，它与多种动作模型和世界模拟器兼容。在世界生成、场景外推和策略优化方面的实验表明，该方法带来一致的改进，其中最大的收益集中在困难的交互场景中。
...
### [27] [Inferring Action from Future Latent State for Robotic Manipulation](https://arxiv.org/abs/2608.22067v1)
从未来潜在状态推断机器人操作动作
...
中文摘要：世界-动作模型（WAMs）在视频生成骨干网络上构建机器人控制，联合预测密集的未来视觉轨迹和机器人动作。我们认为视频生成对于世界-动作建模而言是一个不必要的中间目标。对于机器人操作，世界模型的目标不是再现世界在每个中间时刻的外观，而是预测在执行某个动作之后世界将达到的状态。中间帧仅描述了物理状态之间的视觉过渡，这消耗了大量的模型容量和计算资源，但并不直接指定机器人动作旨在产生的物理结果。在本文中，我们提出DELE-w0.5，它无需依赖视频生成即可从预测的未来状态推断机器人动作。具体而言，DELE-w0.5从其对应的紧凑未来潜在状态推断动作序列。未来潜在状态捕捉了机器人交互中与动作相关的物理结果，并作为世界建模与动作生成之间的显式桥梁。DELE-w0.5的核心设计原则是建模物理世界在机器人动作下如何变化，而不是其视觉外观如何逐帧演变。这种表述消除了密集视频表示引入的高维视觉冗余，因此能够实现更廉价的训练和低延迟推理。在四个长时程操作任务上的480次真实机器人试验中，我们的DELE-w0.5在所有对比策略中取得了最佳性能，达到了62.5的整体完整任务成功率和81.3的宏观有序阶段进度，分别比最强基线高出47.5和30.7个百分点。
...
### [28] [RiskWorld: Object-Centric Latent World Modeling for Autonomous Driving Risk Identification](https://arxiv.org/abs/2608.21414v1)
...
中文摘要：自动驾驶风险识别旨在判断哪个被观测对象可能对自我车辆构成安全关键威胁。现有方法通常预测场景级事故、从自我行为中间接推断风险对象，或在轨迹预测后进行几何检查，而并未直接利用所预测的自我—对象关系进行风险源定位。我们提出RiskWorld，一种以对象为中心的潜在世界模型，通过对每个候选对象相对于自我车辆的想象演化来识别风险。RiskWorld将预训练的预测性视频表示与结构化的自我—对象历史相结合，对观测到的交互进行情境化处理，并使用RSSM风格的潜在动力学将关系感知的对象状态滚动推演到未来。它将推演结果解码为对象级风险评分，并辅以未来的关系预测和时序风险预测作为支持。推理时仅使用截至当前时刻的观测数据，而已记录的未来数据则提供训练监督。在RiskBench上，RiskWorld取得了63.0\%的最佳总体F1分数和2.1\%的最低误报率。进一步的分析表明，学习到的推演过程能够在关键事件发生之前捕捉对象级风险的演化，同时RiskWorld的选择在过滤观测条件下保留了规划关键信息。
...
### [29] [XRFix: Exploring Performance Bug Repair of Extended Reality Applications with Large Language Models](https://arxiv.org/abs/2608.21718v1)
XRFix：利用大语言模型探索扩展现实应用程序的性能缺陷修复

*Jingwen Wu,Hanyang Guo,Hong-Ning Dai,Xiapu Luo*

Main category: cs.SE

中文摘要：作为一种新兴技术，扩展现实（Extended Reality）为用户提供了与虚拟和物理环境交互的沉浸式体验。与传统软件不同，XR应用程序的执行涉及更多计算复杂的操作，例如3D场景渲染、实时动画和过程模拟。在XR应用程序的软件开发过程中，低效的编码实践可能导致各种性能缺陷，降低用户体验，甚至引起晕动症。因此，迫切需要开发一种自动化程序修复框架，用于修复复杂XR程序中的性能缺陷。然而，实现这一目标并非易事，原因在于若干技术挑战：（1）缺乏真实的XR代码库和缺陷数据集，（2）缺乏精确的缺陷检测工具，以及（3）缺乏专门针对XR性能缺陷设计的有效缺陷修复工具。为应对这些挑战，我们提出了一种新颖的基于大语言模型的框架，即XRFix，用于修复开源XR程序的性能缺陷。我们首先基于23个开源XR项目的代码库构建了一个领域特定的性能缺陷语料库，以及一个包含104个真实缺陷的XR相关缺陷数据集。然后，我们定制了两个静态分析工具，用于精确检测C#脚本和资产文件中的缺陷。最后，我们设计了不同的提示词来指导大语言模型在三种不同复杂度的缺陷场景中修复XR缺陷，即单行级别、函数级别和类级别。我们在五个现成的大语言模型上进行了大量实验，以评估XRFix的缺陷修复性能。我们还将XRFix与三种最先进的APR方法进行了比较。通过静态分析、参考答案比较和人工检查，我们证明XRFix能够有效修复XR缺陷，优于最先进的APR方法。
...
### [30] [LpWM: A Case for Sparse Representations in World Models](https://arxiv.org/abs/2608.22764v1)
LpWM：世界模型中稀疏表示的一个案例
...
中文摘要：联合嵌入预测架构（JEPAs）学习用于规划的潜在动力学，并通过将特征匹配到最大熵分布（如各向同性高斯分布）来避免表示坍缩，从而产生稠密表示。然而，稠密表示是否是建模动力学最有利的几何形态尚不明确。在这项工作中，我们提出一个问题：另一种几何形态，即稀疏表示，能否使基于动作条件的潜在动力学更易于建模，以及这种表示会涌现出什么样的动力学结构。我们首先证明，在足够高维的独热（one-hot）潜在空间中，非线性Lipschitz动力学可以被基于动作条件的线性动力学任意好地逼近，且随着维度增长，展开误差趋于消失。这促使我们将分布式稀疏表示作为独热稀疏性的一种实用松弛。我们引入了LpWorldModel（LpWM），这是一种JEPA模型，使用整流分布匹配正则化（RDMReg）将编码器特征匹配到整流广义高斯分布，从而产生非负稀疏编码。实验表明，稀疏性降低了成功规划所需的预测器复杂度：在PushT上，在中等等预测器容量下，稀疏LpWM的规划成功率比稠密LeWM高出最多57%。这一优势还扩展到高斯分布匹配之外，LpWM在多种预测器族中均优于稠密VICReg表示。我们进一步发现，学习到的稀疏表示是模式分解的（mode-factored），其支撑集编码离散的动力学区域，而特征幅度捕获区域内连续的连续状态。综上所述，这些结果表明稀疏表示可以降低控制所需的预测器复杂度，同时揭示可解释的结构。
...
### [31] [MOSH-WM: Mask-Grounded Soft-Hamiltonian Dynamics for Object-Centric World Models](https://arxiv.org/abs/2608.22750v1)
MOSH-WM：基于掩码约束的软哈密顿动力学用于以对象为中心的世界模型

*Zhekai Wang,Haoxiang Huang,Xiang Liu,Zhikang Chen,Yueqing Sun,Qi Gu,Shiji Zhou,Miao Liu,Sen Cui*

Main category: cs.LG

中文摘要：以对象为中心的世界模型通过演化一组实体槽来预测未来视频，但接收动力学监督的变量往往是不受约束的视觉特征。我们提出\method{}，一种基于掩码约束的软哈密顿世界模型，该模型使其类位置状态显式依赖于槽所属的图像支撑区域。一个冻结的视频槽编码器生成槽和掩码；掩码所属支撑区域的空间矩形成规范状态$Q$，时间差分形成$P$，学习到的能量为有界的学习增量提供软方向偏置。与解码器相关的外观和身份信息被分别存储在因果视觉上下文中。一个门控组合器与有界残差将该上下文与传播的相位状态相结合，以重构与解码器兼容的槽。在OBJ3D上，给定六个观测帧并评估后续30帧时，\method{}相对于最强以对象为中心的基线将LPIPS降低了25.0\%，将空间MSE降低了33.7\%。在CLEVRER上，给定六个观测帧并评估后续十帧时，相应的降低分别为14.5\%和18.7\%。按时间分辨率分解的视觉和对象状态测量表明，完整模型在整个30帧闭环推演过程中误差累积更慢。项目页面：https://github.com/moshwm-anon/-moshwm-anon.github.io。
...
### [32] [Mol-JEPA: A multimodal Joint Embedding Predictive Architecture for Molecules](https://arxiv.org/abs/2608.22642v1)
...
中文摘要：尽管分子基础模型近年来取得了进展，但仍存在若干局限性，例如化学上无效的数据增强、模态坍缩以及生化环境表征不完整等问题。为应对这些挑战，我们提出了 \textbf{Mol-JEPA}，一种用于学习分子世界模型的可扩展框架。我们的模型不依赖次优的分子扰动，而是利用模态掩码从分子结构、细胞表型、结合亲和力、ADMET概况、量子化学模拟及其他药物发现数据中提取信息。在多个基准测试中，我们证明Mol-JEPA学习到的表征展现出强劲的性能，凸显了通过潜在空间预测整合生化上下文的价值。
...
### [33] [Tracing the Unlabeled Storm: Cross-Variable Transfer in a Lagrangian Atmospheric JEPA Framework](https://arxiv.org/abs/2608.22358v1)
...
中文摘要：深层大气对流支配着南亚季风变率，然而直接从零膨胀、重尾分布的降水中学得其潜在世界模型，会产生次优的预测性表示。连续的大气代理变量，如外逸长波辐射（OLR），能更连贯地表达这种对流组织。我们通过\emph{跨变量代理学习}来解决这一不匹配问题：M-JEPA，一种多尺度季风联合嵌入预测架构（Monsoon Joint-Embedding Predictive Architecture），在跟踪移动对流系统的拉格朗日补丁上的五个连续代理场上进行预训练——全程不涉及降水监督。所得的冻结表示通过一个包含并行概率分支和确定性分支的共享解码器主干，迁移至逐日降水预报。由于预训练期间降水严格不可观测，下游技能直接衡量潜变量推演（latent rollout）中捕获的预测信息。一个冻结主干探测框架配有两个对照组（一个在降水单独上训练的相同架构，以及一个随机初始化的主干），将迁移效应明确归因于代理预训练：直接降水训练的CRPS误差高出$36\%$（$7.52$对$5.54$\,mm/day）。与51成员的ECMWF业务集合预报相比，迁移后的模型在统计上可分辨的CRPS优势（$6.81$对$6.89$\,mm/day）和更高的Brier技能评分（$+0.05$对$-0.04$）上取得领先，使用$15.4$M参数在单个消费级GPU上运行，优势集中在大降水阈值和细空间尺度上，而集合预报在邻域技能和点指标上的确定性参照仍保持优势。该结果提供了一个植根于季节内动力学的具有竞争力的季风降水预报，以及一个用于评估迁移大气表示诊断框架。
...
### [34] [On the Capability Separation Between World-Model Policy Learning and Imitated World-Action Models](https://arxiv.org/abs/2608.22197v1)
论世界模型策略学习与模仿世界-动作模型之间的能力分离

*Yang Yu*

Main category: cs.LG

中文摘要：世界-动作模型预测未来结果，然后推断相应的动作。尽管这种分解可以改善表示学习和数据效率，但在两者均使用相同的观测性演示进行训练时，它是否能比直接行为克隆提供更强的控制能力仍不清楚。我们比较了直接行为克隆策略、模仿训练的世界-动作策略，以及使用以动作条件化的世界模型优化的策略。在控制器类别层面，每个世界-动作策略都可以展平为具有相同闭环轨迹分布的直接随机策略。在群体层面，在可实现性、精确优化、共同部署信息和分布保持部署的条件下，直接行为克隆和世界-动作模仿都能恢复观测性行为策略。因此，未来预测改变了学习分解，但并未改变无限制的外部策略类别或理想的模仿目标。以动作条件化的世界模型学习的不同之处在于，它根据指定动作预测结果，并通过控制目标对它们进行比较。我们刻画了不基于候选动作进行条件化的未来模型所具有的不可约的特定于动作的预测误差，确定了世界-动作联合模型能够恢复干预性前向模型的条件，并表明观测性演示在一般情况下不能识别动作效应。最后，我们构建了一个环境族，其中每个观测性学习者的最坏情况遗憾均为正，而一次信息丰富的干预则允许零遗憾。因此，关键区别在于预测与观测行为相关的未来与预测指定动作的后果以用于策略优化之间。
...
### [35] [Reading the Room: Implicit Confusion Encoding in Recurrent World Model States](https://arxiv.org/abs/2608.21582v1)
读懂情境：循环世界模型状态中隐式困惑编码

*Donald Aadithiyan*

Main category: cs.LG

中文摘要：基于RSSM架构构建的世界模型（如DreamerV3）保持一个循环隐藏状态$h_t$，其训练目标仅为降低预测误差。我们证明该状态同时追踪自身的困惑，且隐藏于显而易见之处：它与$h_t$方差最大的方向近乎正交，因此任何基于方差的方法都无法察觉。它在功能上区别于集成分歧（用于标记新输入）和重构误差（用于标记当前不佳的预测）。在一个保持预测误差不变而困惑变化的测试中，$h_t$上的线性探针能发现该信号（AUROC 0.72，5次运行），而集成基线得分低于随机水平。对近期高误差步骤的折扣计数可解释探针输出的80%（$R^2=0.80$）。我们通过直接编辑$h_t$并观察行为变化，确认该信号被因果性地使用而不仅是存在，其中包括使用来自其他轨迹的真实值而非合成编辑的检查。其几何结构和闭合形式可推广到三个控制任务；决定性的分离测试本身仅在其中一个任务上干净成立，而其实际用途——决定何时检查现实而非信任想象——仅可推广到三个任务中的两个。
...
### [36] [ReWorld: An Interactive World Model with Long-Horizon Memory](https://arxiv.org/abs/2608.23565v1)
...
中文摘要：交互式世界模型必须跟随用户的操作，记住它已展示过的场景，并以实时方式流式生成。这种张力是结构性的：控制需要短时视野，而记忆需要无界视野。ReWorld在训练期间将两者分离，并在推理时对它们进行约束。混合逐头注意力窗口将大多数注意力头限制在近期范围内，而一小部分全局注意力头对整个历史进行关注，随机头路由防止任一能力被绑定到特定注意力头上；随机块丢弃使稀疏的历史分布内化。在推理时，整个过去生活在固定预算之下：一个受界定的KV缓存，由姿态索引的地标库支撑，模型从中检索最接近当前姿态的地标。一个度量尺度对齐的数据引擎将八个数据来源——Unreal渲染的飞越场景、游戏漫游和真实世界影像——置于同一物理动作尺度上，因此在每个来源中相同的按键都会使相机移动相同的距离，回文轨迹则提供记忆训练所需的回访证据。随后，限定于LoRA适配器的分布匹配蒸馏将采样压缩至四步：一个主干同时服务于高保真多步模式和实时交互模式，以704x1280的分辨率流式生成覆盖照片级逼真、游戏风格和风格化世界的视频。在涵盖动作跟随、长时回忆和视频质量的三轴协议下，与六个近期交互式世界模型相比，它取得了最佳的控制保真度（$11.95^\circ$旋转误差和最佳的相机运动一致性）以及最佳的生成质量；在长达一分钟的外出往返展开（$64$秒，$384$个潜在变量）中，其固定的12块缓存仍能重新生成起始视图——而在这种展开长度下，滑动窗口早已将证据逐出，全KV注意力也已耗尽内存。
...
### [37] [Correcting a learned physical invariant improves world-model rollouts](https://arxiv.org/abs/2608.23526v1)
修正学到的物理不变量可改善世界模型的自回归推演

*Richard Bao*

Main category: cs.AI

中文摘要：世界模型能够预测视频，却未必学到能够可靠保持的物理动力学。我们测试了一个仅用摆锤视频训练的冻结DreamerV3是否学到了一个其自身潜在状态转移近似视为守恒的标量。无标签搜索在多个独立训练的保守模型中恢复了相同的能量类似不变量，而同样的流程在匹配的阻尼模型中则找不到可比的不变量。在自主自回归推演过程中，该量发生漂移。将潜在状态投影回其初始水平集可降低全部三个保守模型中的推演误差，而匹配的随机约束通常反而增加误差。这些结果区分了具有动力学意义的不变量与仅仅是可解码的相关量，并揭示了一个具体的失效模式：世界模型可以从像素中学习物理约束，却在向前想象时违反该约束。
...
### [38] [From Generation to Simulation: How Far Are World Models from Being True Simulators?](https://arxiv.org/abs/2608.23070v1)
...
中文摘要：随着扩散模型和大规模视频生成的快速发展，生成式世界模型日益被期望取代传统模拟器，包括物理引擎、游戏引擎和强化学习环境。然而，从生成到仿真之间仍存在的差距尚缺乏系统性的评估。我们提出了一项基于能力的研究，使用外部衡量标准：传统模拟器的八项能力，即资产构建、物理引擎、交互、可控性、稳定性、状态反馈、多样性和评估指标。我们追溯了三条主要技术路线——潜在动力学、视频生成和联合嵌入预测——并将2018年至2026年6月期间发表的恰200篇代表性工作映射到这些能力上。我们的分析表明，世界模型在特定场景的交互和可控性方面已实现功能性替代，但在物理定律的形式化保证、结构化状态反馈和可复现的长期演化方面仍逊于传统模拟器。状态反馈是最被忽视的跨路线缺陷：在163篇实现论文中，只有6篇暴露了用于查询实体状态或物理参数的运行时接口。我们确定了六个研究方向：形式化物理、统一动作接口、一等公民状态反馈、长期稳定性、下游效用评估和跨路线混合。项目页面：https://github.com/AtongWang/world-model-simulators
...
### [39] [Where World Models Break: Natural-Input Failure Discovery](https://arxiv.org/abs/2608.22421v1)
世界模型在何处失效：自然输入故障发现
...
中文摘要：世界模型预测基于动作条件的未来状态，并作为下游规划与控制的关键内部模拟器。然而，世界模型的灾难性预测失效可能会危险地传播到整个控制流水线中，因为后续智能体或模型的训练以及决策制定高度依赖于这些世界模型所预测的连续环境演化。现有评估忽略了这一系统性风险：通过聚合来自通用查询的良性生成结果的平均误差，它们未能对模型在罕见或未观测到的条件-动作组合下的灾难性崩溃进行压力测试。为弥补这一缺口，我们将自然输入故障发现问题形式化：在有限查询预算下，寻找能诱发严重预测风险的环境有效条件与动作前缀，验证这些失效是否在新种子下重现，并测试其在邻近有效编辑下的持续性。发现此类关键失效在计算上具有挑战性，因为有效的条件-动作组合呈指数级爆炸，使得穷举搜索或标准采样在噪声回放成本高昂的情况下不可行。为解决这一问题，我们提出BasinLens，它利用有效输入的内在结构——其中每个坐标具有环境定义的语义类型和容许域——将不确定性引导的全局搜索与类型化的局部替换相结合。在多种基准测试和世界模型家族上，BasinLens揭示了常规评估无法发现的、可重现且局部持续的失效模式，表明平均情况的基准测试可能掩盖世界模型驱动控制中的重要脆弱性。
