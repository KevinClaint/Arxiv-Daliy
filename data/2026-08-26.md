...
### [1] [LeFlow: Generative Latent Flow Planning for World Models](https://arxiv.org/abs/2608.24855v1)
LeFlow：用于世界模型的生成式潜在流规划

*Hsiang-Wei Huang,Jianxu Shangguan,Junbin Lu,Jenq-Neng Hwang*

Main category: cs.CV

中文摘要：潜在世界模型本质上是强大的编码器，能够将图像像素转换为潜在嵌入，然而现有的世界模型在动作规划时仍然依赖于在线轨迹优化：对于每个状态-目标对，迭代优化器都会从零开始运行，以搜索最优动作序列，将世界模型视为黑箱模拟器。这种方法在每次重新规划时都要重新付出完整的迭代优化成本，并且不会在查询之间复用任何规划经验。在这项工作中，我们提出一个问题：一旦潜在世界模型被学习完成，规划本身是否可以被摊销化。我们提出了LeFlow，它学习一种可复用的潜在轨迹先验，直接在世界模型的潜在动力学空间中运作。LeFlow将规划重新定义为条件式潜在轨迹生成：一个整流流模型在当前嵌入与目标嵌入之间推演出未来的潜在路径，一个逆动力学解码器将潜在状态转移转化为动作片段，而冻结的世界模型通过自回归展开来验证每个候选方案。在四个主要的以目标为条件的像素控制基准上，LeFlow用摊销化的潜在规划和固定预算的展开选择取代了迭代式的动作空间优化，在将规划时间降低一个数量级的同时取得了持续的成功率提升。我们的结果表明，潜在世界模型不仅应支持预测，还应支持可复用的规划先验。我们的代码可在https://github.com/hsiangwei0903/LeFlow获取。
...
### [2] [Game2World Engine: Unlocking In-the-Wild Gameplay Videos for World Model Training](https://arxiv.org/abs/2608.24680v1)
...
中文摘要：视频游戏为视频世界模型提供了可扩展的训练数据来源，涵盖了多样的环境、复杂的交互以及丰富的野外游戏视频。然而，原始游戏画面将游戏世界与屏幕空间界面纠缠在一起，引入了特定于游戏的偏差和不相关的动态，阻碍了世界模型的训练。为解决这一问题，我们提出了GameUI-Taxonomy和G2WEngine——一个全栈框架，将游戏UI的定位与去除形式化。G2WEngine自动从真实游戏视频中提取可复用的UI资产，并在干净画面上合成时间上连贯的UI叠加层。利用该引擎，我们构建了Game2World，包含96K条具有精确重建目标的合成配对视频，以及来自303款游戏的1,079条野外片段，用于现实评估。其资产库包含来自21个分类类别的5,132个经过验证的UI元素，收集自1,010个具有代表性的游戏画面帧。基于Game2World，我们提出了GameCleaner，一种无需掩码的游戏UI去除模型，将多模态语义理解与视频编辑能力相结合。与基于掩码的方法不同，GameCleaner直接识别并去除多样化的HUD元素，同时保留底层场景内容与时间动态。在一项受控试点实验中，基于无UI游戏画面训练的世界模型在VideoReward上比基于UI叠加数据训练的模型整体提高了6.83%。在UI去除评估中，GameCleaner在合成视频上取得了95.36的平均AAR，比最强的时间掩码基线高出57.3%，并在野外场景中取得了80.05的最佳AAR，同时实现了99.8%的背景保留。这些结果展示了将互联网游戏视频转化为高质量世界模型训练数据的可扩展潜力。代码、数据集和模型将可在 https://github.com/Dongping-Chen/Game2World 获取。
...
### [3] [SceneReGen: Generative Reconstruction of 3D Scenes from a Single Image](https://arxiv.org/abs/2608.23930v1)
SceneReGen：从单张图像生成式重建3D场景

*Zefan Tian,Yuteng Ye,Yiheng Zhang,Yuhang Yang,Xueqiang Lv,Shizhou Zhang,Le Liu,Di Xu*

Main category: cs.CV

中文摘要：单图像3D场景重建必须补全部分观察到的物体，并将它们连贯地放置在共享的、与观察对齐的场景坐标系中。物体级生成先验提供了强大的补全能力，但其以中心对齐、尺度归一化的输出通常表达在物体坐标系中，这在物体生成与场景重建之间造成了根本性的表征差距。我们提出SceneReGen，一种生成式重建框架，将场景重建重新诠释为在共享的、与观察对齐的场景坐标系中生成并组装完整物体资产。SceneReGen通过选择性位姿分解来解决生成-重建差距：每个物体的观察朝向直接编码在生成的网格中，而平移和尺度则根据实例级和全局场景证据进行估计。给定一张场景图像和实例掩码，几何编码器提取密集线索；可学习的形状查询条件化一个预训练的基于DiT的3D生成器，以生成处于其观察朝向的完整网格，而位置查询则融合物体和场景特征以在共享坐标系中组装它们。在3D-FUTURE评估子集上，SceneReGen在评估方法中取得了最优的场景级CD、场景级F-Score和3D包围框IoU，与最优物体级CD持平，并在物体级F-Score中排名第二。自主驾驶和具身AI场景中的定性输出进一步展示了超越室内家具的以资产为中心的重建潜力。
...
### [4] [GlanceWAM: Sparse Test-Time Imagination for World-Action Models](https://arxiv.org/abs/2608.23927v1)
...
中文摘要：视频生成模型为机器人学习提供了丰富的物理先验，然而现有的世界-动作模型（WAM）面临一个根本性的权衡：在控制频率下同步生成视频在延迟上不可行，而放弃测试时视觉想象则会牺牲任务成功率。我们证明，当视觉想象在关键路径之外异步生成并直接在潜在空间中消费时，能够同时实现实时推理和更高的成功率。我们提出GlanceWAM，它在单一视频DiT中将想象与控制解耦：一个异步提议器以慢时钟向前预视，在后台想象几秒后的单个前瞻帧，而动作头以控制频率（48毫秒）纯粹在潜在空间中解码动作块，无需阻塞。通过非干扰注意力掩码（隔离视频表征）和适应异步预视老化的鲁棒时域训练，GlanceWAM打破了速度-成功率困境。仅基于示范进行训练，它在24任务RoboCasa厨房基准上达到72.2%（超过同步Cosmos Policy的67.1%和无想象联合训练的64.4%），在LIBERO上达到99.0%，在NVIDIA A100 GPU上以每块48毫秒执行（比同步基线快24倍）。代码可在https://github.com/linhanwang/GlanceWAM获取。
...
### [5] [Object Counting Across Modalities: Taxonomies, Benchmarks, Applications, and Open Challenges](https://arxiv.org/abs/2608.23845v1)
跨模态物体计数：分类体系、基准、应用与开放挑战

*Joana Konadu Owusu,Shivanand Venkanna Sheshappanavar*

Main category: cs.CV

中文摘要：物体计数方法已迅速从类别特定的密度回归转变为开放词汇、基于基础模型的计数器。这些方法现在能够对各种视觉和文本提示中的实例进行枚举。虽然这一转变标志着概念上的重大进展，但我们的综述认为，关于普遍通用性的宣称已超越了评估基础设施的发展。大多数进展指标依赖于少数几个已趋于饱和的基准，而这些基准被模型利用以获取统计规律性。新引入的诊断数据集揭示了语义接地、时间身份以及带遮挡的空间推理方面的系统性失败。为解决这些失败，我们引入了一个五轴分类体系（模态、机制、提示方式、监督水平和泛化设置）。我们利用这一分类体系对包括显微镜、遥感、人群计数和农业等应用领域的文献进行审计。这将当前的主要挑战形式化为六个结构性矛盾。在此基础上，我们提出了一个关于组合式场景理解、主动计数智能体以及统一多模态评估协议的研究路线图。主要当务之急是构建一个稳健的评估基础设施，以区分开放世界泛化与特定于基准的优化，而不仅仅是简单的渐进式工程改进。
...
### [6] [Primate vision reveals a missing principle for robust dynamic AI](https://arxiv.org/abs/2608.23790v1)
灵长类视觉揭示了稳健动态人工智能所缺失的一条原则

*Matteo Dunnhofer,Christian Micheloni,Kohitij Kar*

Main category: cs.CV

中文摘要：智能视觉系统如何在将物体的外观与运动方式相结合的同时，在外观发生变化时仍然保持稳健？我们通过将人类感知以及猕猴下颞叶皮层的神经活动与基于图像和视频的神经网络表示进行比较来回答这一问题，这些网络涵盖识别、分割、光流处理和预测性世界建模。时间整合改善了物体表示，但大多数视频识别模型在运动结构得以保留而外观受到干扰时泛化能力很差。人类和猕猴的下颞叶皮层仍然保持稳健。值得注意的是，预测性世界模型将强大的跨外观泛化能力与对下颞叶皮层最接近的对应关系相结合，在神经保真度上优于其他视频建模方法。然而，没有任何模型能够再现从早期的外观主导响应向后期外观不变的运动编码的皮层转换。这些结果确定了将运动逐步整合进物体表示中是稳健动态视觉的一条原则，并表明预测性学习是人工系统中实现这一计算的一条有前景的途径。
...
### [7] [Platonic Representation Hypothesis on World Models](https://arxiv.org/abs/2608.23720v1)
世界模型中的柏拉图式表征假说
...
中文摘要：世界模型在感知和模拟复杂环境方面已展现出巨大的潜力。尽管其性能强劲，但其学习到的表征的基本性质仍然知之甚少。在本文中，我们通过提出预测一致性假设来研究这一领域中的柏拉图式表征假说：我们假定，对共享状态转移目标的优化构成一种选择性压力，促使异构模型收敛于共享的潜在结构。通过使用DINO世界模型（DINO-WM）进行的系统性实验（在该实验中我们改变视觉编码器以创建异构模型），我们发现能力强的世界模型会朝着几何上相似的内部结构演化。此外，通过模型拼接，我们证明一个世界模型的内部特征可以被映射到另一个世界模型，且性能退化有限，这为功能兼容性提供了证据。我们的研究结果表明，对预测一致性的追求可以促进世界模型之间共享的、转移兼容的潜在结构。
...
### [8] [Scaling Reinforcement Learning for Diffusion Models via Velocity Matching](https://arxiv.org/abs/2608.23664v1)
通过速度匹配实现扩散模型的强化学习扩展
...
中文摘要：奖励微调正成为将扩散模型适配到人类偏好和特定任务目标的重要工具，但现有方法在很大程度上继承了来自大型语言模型的策略梯度机制。与自回归模型不同，扩散模型无法为生成的样本提供可计算的似然。因此，当前方法要么从随机去噪转移中构建轨迹似然，要么用证据下界近似端点似然，引入了额外的计算和算法复杂性。我们证明，这种基于似然的机制对于有效的扩散模型奖励微调并非必要。我们提出基于奖励的速度匹配（RVM），一种简单的无轨迹更新方法，直接作用于速度场。RVM强化与高奖励生成相关的方向，抑制低奖励的方向，并包含一个可选的锚项来控制相对于参考速度的漂移。值得注意的是，它提供了一个通用框架，可以将包括RAM和DiffusionNFT在内的近期微调方法作为特例恢复出来。在多种大规模扩散模型奖励微调任务中，RVM在训练成本大幅降低的情况下，与基于轨迹的策略梯度方法相比具有竞争力或表现更优。我们进一步发现，一旦速度更新被简化，具体损失变体的重要性不如奖励和锚项的设计。对于视频生成，标准的偏好奖励可能倾向于视觉上干净但近乎静态的输出；引入一种新的动态跟踪奖励可以在提升整体VBench性能的同时大幅改善运动。这些结果表明，扩散模型的可扩展奖励微调更适合在原生速度表示中进行表述，而不是作为基于似然的策略优化。
...
### [9] [Finite-Sample Metric Non-Collapse for Geometrically Supervised Latent World Models in Control](https://arxiv.org/abs/2608.07265v2)
控制中几何监督的潜在世界模型的有限样本度量非坍缩

*Alain Bensoussan,Minh-Nhat Phung,Minh-Binh Tran*

Main category: math.OC

中文摘要：我们为非线性确定性系统的几何监督潜在模型建立了有限样本学习到控制的理论。几何监督仅在训练期间使用：模拟器状态、本体感觉或具有独立验证的度量和方向误差界的状态估计提供可观测状态距离和切向方向，而部署仍然以观测和动作作为条件。我们引入了一个仅编码器的局部—全局度量枢轴（hinge），用于强制方向分辨率和分离状态判别。在正则可观测因子、覆盖、有限容量逼近以及一致$C^{1,1}$假设下，一个可计算的单侧正则化机制具有强选择性质：以高概率，每个近似经验最小化器同时是逐点co-Lipschitz的，并且与受控动力学一致地近似半共轭。逼近误差、采样误差和优化误差保持显式且分离。范数约束的张量积B样条类构造性地实现了逼近假设，将平均残差控制转换为一致界的内插指数是尖锐的。一个模块化的确定性推论将学到的证书转移到轨迹、有限时域代价、学到的代价头以及优化器保证上，而一个经过验证的有限网格结果支持更尖锐的模型特定认证。受控实验隔离了坍缩和折叠，量化了解析证书的储备，并展示了恢复度量分辨率的控制收益。主要贡献是从近似经验优化到度量忠实性、一致受控动力学以及同一学习模型的可靠规划的一个完整有限样本蕴涵。
...
### [10] [How Do Professional Editors Evaluate the Editing Quality of AI-Generated Cinematic Video Ads?](https://arxiv.org/abs/2608.24329v1)
专业编辑如何评估AI生成电影式视频广告的剪辑质量？

*Po-Ming Law,Weizhi Li,Arpit Narechania*

Main category: cs.HC

中文摘要：在社交媒体上，我们经常会遇到采用电影式剪辑手法以引发情感反应的短视频广告。虽然AI工具正开始自动生成此类电影式广告，但我们缺乏一个细粒度的框架来评估这些广告。在本文中，我们首先对社交媒体视频广告格式进行特征描述，并在我们的语料中识别出电影式广告作为一种反复出现的格式。随后，我们分析了电影式广告的时长、镜头结构、音频与文本元素以及剪辑手法，以指导一个两阶段生成流水线，其中大语言模型（LLM）首先生成镜头计划，视频生成模型再渲染视频。利用该流水线，我们为35个真实品牌生成了70条电影式广告，并招募了专业视频编辑来评判其剪辑选择。从他们的评判中，我们得出了剪辑质量的六个维度：叙事推进、视听协调与声音设计、视觉构图与图形、镜头间连续性、信息与品牌一致性，以及时间节奏与韵律。我们讨论了这些维度如何指导AI生成电影式广告的剪辑感知生成、人工评估和自动化评估。
...
### [11] [XP-JEPA: Cross-Predictive Physics Grounding for Forecastable Latent Dynamics](https://arxiv.org/abs/2608.24044v1)
...
中文摘要：潜世界模型通过预测候选动作如何转化学习到的表征来进行规划。然而，在自预测模型中，编码器和预测器被联合优化，可能会共同适应那些易于预测但仅受场景物理演化微弱约束的潜状态转移。我们提出了交叉预测JEPA（XP-JEPA），它将视觉潜动力学锚定在特权物理轨迹上。XP-JEPA分别编码视觉观测和物理状态，通过一个共享的动作条件预测器推进两者，并将每个预测与两种未来表征相匹配。这一目标鼓励两种模态之间的统一潜动力学，并锚定在底层的物理转移上。训练后丢弃物理分支，部署时仅保留纯视觉模型。在覆盖六个评估子任务族的多任务套件上，XP-JEPA将新拟合预测器的滚转漂移从$0.361$降至$0.104$，并将平均控制成功率从$53.6\%$提升至$78.2\%$。直接进行物理状态回归可提高位置可解码性，但预测能力和控制能力仍接近纯视觉基线。因此，交叉预测物理锚定可以在测试时无需特权输入的情况下，产生更适合基于滚转控制的可预测潜动力学。
...
### [12] [Neurosymbolic Alignment for Physiologically-Safe Clinical Language Models](https://arxiv.org/abs/2608.24534v1)
面向生理安全临床语言模型的神经符号对齐
...
中文摘要：临床大语言模型（LLM）能够生成事实层面看似合理但生理上不安全的建议。我们研究是否可以通过将偏好优化基于结构化生理知识而非纯文本监督来改进安全对齐。方法：我们提出神经符号对齐（Neurosymbolic Alignment），一个训练时框架，将70亿参数的临床LLM与基于847K节点生物医学知识图谱的HGNN生理世界模型（Physiological World Model）相结合。候选回复使用稳态约束、多跳路径合理性和药物相互作用惩罚进行评分，所得排序驱动迭代式on-policy ORPO更新。评估在临床安全基准（CSB）上进行，该基准包含2,500个用于生成式临床推理中生理约束违反的场景。结果：相对于ORPO，所提方法将CSS从69.5%提升至90.8%（+21.3个百分点），在盲法子集上将医师评估的HR从14.1%降至5.1%，并将DID从72.8%提升至91.6%。这些提升得到与HGNN无关的规则引擎安全评分（RSS：86.4%，较ORPO提升+21.2个百分点；与CSS的r=0.97一致性）的佐证。该方法还在所有安全指标上超过GPT-4（5-shot），尽管参数规模小10倍，并在CSS上优于推理时自我修正流水线（SFT+SelfCorrect）11.4个百分点。在合成EHR式噪声下，保留了84.2%的CSS。消融分析表明，HGNN评分（-16.2个百分点）和迭代训练（-11.5个百分点）是主要贡献因素。PhysioScore针对200个临床医生标签的校准得到ECE = 0.038和kappa = 0.91。结论：训练时的生理学基础化在受控评估下为开放权重临床LLM带来了可量化且可独立验证的安全改进。需要在真实临床数据上进行外部验证，以确定这些增益能否迁移到部署场景。
...
### [13] [When Seeing Is Not Enough: Benchmarking Interactive Visual Grounding in LVLMs](https://arxiv.org/abs/2608.23978v1)
当“看见”仍不足够：大型视觉语言模型中的交互式视觉定位基准测试

*Zhengxiang Wang,Owen Rambow*

Main category: cs.AI

中文摘要：视觉定位通常被评估为从信息丰富的指称表达式到视觉目标的一次性映射。这种设定忽略了现实世界中指称的一个核心属性：目标信息往往不完整、存在歧义，并通过交互逐步确立。我们引入了一个针对大型视觉语言模型（LVLMs）中交互式视觉定位的受控评估框架，该框架变化了预先提供多少目标信息以及必须通过对话获取多少信息。在四种基于人类标注的视觉情境和四种交互协议下，当前LVLMs的表现显著低于任务级的人类基线。当后续问题能够细化或修正初始目标描述时，交互会有所帮助。当未提供初始描述且必须通过提问获取目标信息时，性能最低，这表明主动的、由问题驱动的定位仍然困难。LVLMs的校准性也很差，其报告的置信度往往超过其实证准确率。后续研究在不同描述来源（人类与AI）、推理努力、重复交互、描述提供者和视觉情境下确认了这些模式。总体而言，交互式视觉定位仍然是一项重要挑战，需要视觉匹配、信息寻求与综合能力。
...
### [14] [Do LLMs Understand Limit Order Book Dynamics?](https://arxiv.org/abs/2608.23706v1)
大语言模型能否理解限价订单簿动态？
...
中文摘要：在合成限价订单簿（LOB）数据上训练的大语言模型（LLM）在生成有效的LOB事件序列方面取得了接近完美的得分。然而，该LLM的隐式世界模型未能学习LOB的状态。这一缺陷导致了在使用LLM预测未来LOB事件时产生有偏的估计和虚假的可预测性。我们的分析采用了针对LLM世界模型的新型测试，将先前工作从确定性设定扩展到LOB所需的随机动力学。
...
### [15] [Do Robotic World Models Really Follow Actions? Diagnosing and Aligning Action-Conditioned Generation for Policy Learning](https://arxiv.org/abs/2608.24885v1)
机器人世界模型真的遵循动作吗？面向策略学习的动作条件生成诊断与对齐

*Sixiang Chen,Jiaming Liu,Jixian Wu,Yichen Guo,Tinghao Wang,Siyuan Qian,Hao Chen,Jiajun Cao,Jian Tang,Shanghang Zhang*

Main category: cs.RO

中文摘要：动作条件世界模型日益被用作策略评估与改进的学习模拟器，然而其有效性依赖于一个未经验证的假设：生成未来状态能够忠实反映任意有效动作。现有基准通常局限于专家示范，导致非专家动作遵循未能得到充分评估。为弥补这一空白，我们提出WorldEcho，通过视觉完整性（visual integrity）和SE(3)轨迹对齐（trajectory alignment），在更广的动作分布上探测动作遵循能力。我们的诊断表明，现有世界模型能合理执行专家动作，但在多样化的非专家轨迹上表现不佳，要么忽略被命令的动作，要么产生视觉上无效的展开（rollout）。我们进一步提出WorldSync，沿三个互补方向增强动作遵循：分布覆盖（distributional coverage）、表征锚定（representational grounding）和干预效应对齐（intervention-effect alignment）。它拓宽了关于动作后果的训练分布，通过行动强迫专家（Action-Forcing Expert）将中间视频表征锚定于动作所诱发的机器人动力学，并将动作干预下预测的变化与真实未来状态的相应变化对齐。在RoboTwin基准和真实机器人任务上的实验表明，WorldSync提升了WorldEcho指标，并能作为迭代策略改进中更可靠的模拟器，使策略实现更高的成功率。
...
### [16] [NVIDIA Cosmos-H-Dreams: Real-Time Generative Physics Simulation for Surgical Robotics](https://arxiv.org/abs/2608.24199v1)
NVIDIA Cosmos-H-Dreams：用于手术机器人的实时生成式物理仿真
...
中文摘要：手术机器人的生成式仿真仍然缺乏实时交互能力。物理机器人实验（通常涉及动物或尸体实验室）耗时、昂贵且难以复现，而经典仿真器难以捕捉照片级真实外观和可变形组织动力学。我们通过 Cosmos-H-Dreams 弥补了这一差距，这是一个集成的实时手术世界模型系统，结合了动作条件生成模型、教师到学生的蒸馏方案，以及基于 NVIDIA FlashDreams 流式推理库构建的部署栈。从 Cosmos-H-Surgical-Simulator（一个在大规模 Open-H-Embodiment 语料库上微调的多具身动作条件手术视频世界模型）出发，我们在具身和手术流程特定数据上对检查点进行后训练。通过将所得的双向教师模型蒸馏为带有 Self Forcing 的因果、少步学生模型，我们将被动的视频生成器转变为可控的手术仿真器，在单个 NVIDIA RTX PRO 6000 Blackwell 工作站 GPU 上以约 160 推理 FPS 进行流式输出。至关重要的是，Cosmos-H-Dreams 与控制器无关：任何能发出机器人运动学数据流的接口都可以驱动它。我们展示了通过 WebRTC 的浏览器键盘、通过 WebXR 的 Meta Quest 头显、商业手术机器人控制台（如 CMR Surgical 的 Versius）以及闭环运行的已学习策略的实时控制。据我们所知，这是第一个支持实时人工和政策控制的交互式手术世界模型。人类操作员和政策都能够在合成的世界中行动并实时观察后果。我们将 Cosmos-H-Dreams 作为开放的手术仿真系统发布，为手术教育、可扩展合成数据生成以及未来的术中决策支持提供共同基础。
...
### [17] [TrAct: Bridging Robot Control and Visual Prediction with Visual Tracks](https://arxiv.org/abs/2608.24101v1)
TrAct：以视觉轨迹连接机器人控制与视觉预测

*Zhi Cao,Howard Ji,Kevin Zhang,Kuangzhi Ge,Li Fei-Fei,Jiajun Wu,Huang Huang*

Main category: cs.RO

中文摘要：机器人动作本质上具有具身特异性，与图像空间中的视觉变化仅有弱对齐关系，这限制了其作为机器人世界模型条件信号的有效性。相比之下，视觉轨迹提供了一种与具身无关的任务相关点在场景中运动方式的表征，为准确且空间精确的未来视频预测提供了密集的图像空间指导。基于这一观察，我们提出了TrAct，一个基于世界模型的机器人决策框架，该框架使用视觉轨迹作为控制与预测之间的中间接口。TrAct由三个组件组成：一个视觉-语言-动作-轨迹模型（VLAT），它从当前观测和语言指令中联合预测候选动作及对应的视觉轨迹；一个轨迹条件世界模型（TWM），它以所提出的轨迹为条件预测未来的视觉结果；以及一个视觉-语言奖励模型（VLAC），它对预测结果进行打分。在推理时，VLAT生成候选动作-轨迹对，TWM展开其视觉后果，VLAC选择预测结果最能满足指令的轨迹；随后机器人执行与该选定轨迹配对的动作。在所提出的LIBERO-INTEGRAL基准和真实世界Franka操作实验上，与强VLA基线π₀.₅相比，TrAct在仿真中将成功率从27%提升至55%，在真实世界任务中将成功率从49%提升至76%。此外，与动作条件世界模型（AWM）相比，TWM持续提升了视频预测质量。这些结果表明，视觉轨迹为机器人控制与视觉预测之间提供了有效的共享接口，能够实现更准确的世界建模和更强的机器人泛化能力。
...
### [18] [DreamLedger: Execution-Settled Credit Files for World-Model Imagination in Robot Decision Loops](https://arxiv.org/abs/2608.23863v1)
DreamLedger：面向机器人决策回路中世界模型想象的执行结算信用档案

*Xianyao Li,Ruitong Tian,Rui Min,Fang Xu,Jing Du*

Main category: cs.RO

中文摘要：机器人开始基于世界模型预测采取行动，但可靠性仍通过即时的、模型内部的信号来表达。DreamLedger 则将可靠性视为一种持久化的部署对象：一份执行结算的信用档案，记录被消耗的预测在多大程度上得到兑现，并按运行条件、区域和预测时域建立索引，且在每次使用前进行查阅。每次被消耗的预测都被登记为一项索赔；可归因的结果在零标注成本下依据到达的现实进行结算，归因阶段排除受测量污染的结果，一个结算监督的头部补充稀疏的区间。由此产生的信用对消耗进行门控：低信用预测会缩短依赖时域或触发额外的观测；每一次依赖事件都通过依赖票据和可回放的日志保持可审计。我们在三个模拟域（室内飞行、桌面操作、2D 导航）中，通过安装在未经修改的 DreamerV3、TD-MPC2 和 V-JEPA 2-AC 上，以及在真实的 Franka 机械臂上，对 DreamLedger 进行了评估。在所有 12 个留出的条件-时域单元中，索赔失败与剂量呈单调关系。与盲目消耗相比，信用门控规划将烧掉的想象（被消耗但随后无法兑现的索赔）减少了 62%（95% 置信区间 43-81%），且成功率相同，碰撞率相当。在匹配的风险目标下，持久化账簿将操作任务中的验证探针从每回合 1.00 降至 0.36，成功率分别为 0.94 与 0.98；基于结算的校准保留了中等的、与种子一致的工作点，而原始即时门控则不然。同一信任层可跨解码器、潜变量和令牌空间接口运行，包括在真实机器人帧上结算的 V-JEPA 2-AC。在硬件上，结算在真实感知和接触噪声下仍然可运行，部署失败循环可在线重新定价，所有 1,062 次已登记的消耗均可从审计日志中回放。
