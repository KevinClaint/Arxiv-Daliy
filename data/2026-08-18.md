...
### [1] [HarnessEval-W: Agentifying the Evaluation of Visual Worlds](https://arxiv.org/abs/2608.16859v1)
HarnessEval-W：将视觉世界的评测代理化
...
中文摘要：一个基准测试应当提供的不仅仅是一个标量分数：使评测值得信赖的是为分数提供依据的推理过程。这对于世界模型尤为关键，因为评判一次推演（rollout）需要理解物理、因果关系和世界状态是否正确演化。人类能够自然地发现此类违规，然而现有基准测试均未自动化这一能力：指标是以暴力计算方式得到的，没有留下可供检查或验证的推理链。我们引入了HarnessEval-W，一条代理化的评测流水线，它将LLM生态中的harness范式引入世界模型基准测试。HarnessEval-W并非应用固定的评分标准，而是解读每个评测案例的上下文，将评测问题分解为可度量的子问题，并生成专门的子代理，每个子代理配备定制化的上下文和诊断工具，以对其自身子问题进行推理。父代理随后验证收集到的证据并将其总结为最终裁决。这一分层工作流将每次评测转化为透明的证据树，其完整的推理链为结果提供依据。我们将HarnessEval-W应用于18个具有代表性的世界模型，涵盖330个评测案例。其判断与人类偏好高度一致，同时为每个生成的推演提供可验证的细粒度诊断。我们将整条流水线作为实时基准测试开源，并邀请更广泛的社区参与贡献，随着世界模型的演进不断扩充新的技能和评测案例。
...
### [2] [PersonaShot: Benchmarking Person-Centric Narrative Continuity in Multi-Shot Video Generation](https://arxiv.org/abs/2608.16717v1)
...
中文摘要：视频生成正在从单镜头片段快速演进为多镜头叙事，其中人物角色充当核心的叙事锚点。然而，现有基准主要评估角色外观或单个镜头的质量，而未测量物理状态和情感状态在镜头切换间是否保持一致。它们也很少提供针对特定标准的评估方法，尽管物理连续性、面部动态和电影关系需要不同的视觉、时间和关系证据。为应对这些局限性，我们提出了PersonaShot，这是首个面向多镜头视频生成中叙事连续性的人物中心基准。PersonaShot包含约1,000个多镜头片段和16项指标，涵盖物理连续性、情感动态和电影语法。\textbf{\textit{1）叙事连续性基准：}}我们从三个时间层面评估角色连贯性：镜头内状态、镜头间转换和序列级轨迹。\textbf{\textit{2）与人类对齐的专家评估器：}}我们将大型多模态教师模型的推理蒸馏为轻量级的、针对特定标准的评估器，每个评估器以其指标所需的视觉、时间或关系证据为基础，并与专家人工判断对齐。\textbf{\textit{3）系统评估与洞见：}}我们的评估揭示了各最先进模型之间不同的能力画像，以及感知质量与镜头间叙事连续性之间的明显差距。即使是视觉上引人注目的视频，也经常在镜头间表现出物理状态重置、突变的情绪转换和断裂的电影关系。人工研究进一步表明，我们的评估器与专家判断之间具有高度一致性。
...
### [3] [SQuad: Sub-Quadratic Attention Distillation for Efficient Video Generation](https://arxiv.org/abs/2608.16585v1)
...
中文摘要：视频扩散变换器（Video Diffusion Transformers，DiTs）将其大部分计算开销花费在自注意力（Self-Attention）操作内部，该操作的成本随潜在令牌（latent token）数量 $n$ 呈二次方增长，即 $\mathcal{O}(n^2)$。对于视频生成任务而言，令牌数量庞大，因此这一项主导了运行时间和内存开销，从而限制了我们可以生成的分辨率和时长。线性的 $\mathcal{O}(n)$ 和低秩的 $\mathcal{O}(nk)$ 自注意力替代方案以更廉价的核来取代完整的 softmax $QK^T$，但很少能恢复原始模型的表达能力，从而留下了一个顽固的质量差距。受此启发，我们提出了SQuad，一个次二次注意力蒸馏（Sub-Quadratic Attention Distillation）框架，在蒸馏后的注意力中实现了 $\mathcal{O}(n\sqrt{n})$ 的复杂度，自然地平衡了效率与表达能力之间的权衡。我们不是从头训练自己的视频DiT（这代价极其高昂），而是通过两阶段蒸馏将预训练的全softmax自注意力DiT拟合到我们提出的SQuad-Attention中：首先是流匹配监督微调（Flow-Matching Supervised Fine-Tuning，SFT），随后是改进的分布匹配蒸馏（DMD2），后者还使采样更加高效。在Wan~2.2 5B文生视频模型上，SQuad在VBench上匹配了二次方教师模型（$83.20$ 对 $83.08$），同时将每步每块的注意力FLOPs削减了约 $67\times$，注意力延迟削减了约 $11\times$，端到端DiT延迟削减了2$\times$，并且仅用 $6$ 次神经功能评估（NFEs）即可生成视频，而默认需要 $100$ 次。
...
### [4] [MLLM-Guided Semantic Correction for Text-to-Video Generation](https://arxiv.org/abs/2608.16513v1)
多模态大语言模型引导的文本到视频生成语义修正

*Junhao Chen,Zheqi Lv,Keting Yin,Shengyu Zhang,Zhou Zhao,Feiyang Chen,Xinyu Duan,Baoxing Huai,Fei Wu*

Main category: cs.CV

中文摘要：扩散模型和Transformer架构的最新进展推动了文本到视频生成的显著进步。然而，这些模型常常会出现语义错误，例如对象缺失、属性错误或动作不匹配。尽管一些语义修正方法在采样前进行优化或在采样后进行细化，但在视频生成过程中如何检测和修正语义偏差仍然研究不足。在本文中，我们提出了一种无需训练、可解释的生成中途修正框架，将多模态大语言模型（MLLM）反馈直接集成到扩散采样循环中。我们的框架通过在视频合成过程中注入语义评估信号来实现扩散轨迹修正，使模型能够通过持续的自我反思来优化生成内容。我们提出了两个关键模块：语义评估监督器（Semantic Assessment Supervisor），用于生成中间预览帧以进行语义评估和偏差诊断；以及语义修改助手（Semantic Modification Assistant），通过可控的潜在轨迹干预在推理过程中修正语义漂移。我们的方法在不修改模型参数的情况下提高了语义对齐、视觉保真度和时间一致性。我们通过跨多个基准的大量实验验证了我们方法的有效性。
...
### [5] [Defake-o3: From Speculative Rationales to Verifiable Evidence for Explainable AIGI Detection](https://arxiv.org/abs/2608.16259v1)
Defake-o3：从思辨性理由到可验证证据的可解释AIGI检测

*Bowen Deng,Jiahui Zhan,Yikun Ji,Haozhen Yan,Jianfu Zhang*

Main category: cs.CV

中文摘要：图像生成模型的快速发展要求AI生成图像（AIGI）检测器不仅要准确，还要具备可解释性和可靠性。虽然基于多模态大语言模型（MLLM）的检测器能够提供自然语言解释，但现有方法往往生成思辨性理由：它们依赖模糊或幻觉性伪影，遗漏最新生成器产生的细微局部缺陷，并且无法提供可被视觉验证的证据。我们提出Defake-o3，一种从思辨性理由转向可验证证据的可解释AIGI检测器。它将交互式视觉搜索与验证器引导的证据对齐相结合：模型迭代放大可疑区域以检查细粒度细节，同时证据验证器（Evidence Verifier）基于人类验证标注训练，提供强化学习奖励，倾向于有依据的证据并惩罚无根据的主张。为支持这一目标，我们构建了GroundFake数据集，专为基于依据的可解释检测设计，包含局部化边界框证据、基于视觉依据和伪影特异性的人类验证、修正后的推理轨迹，以及有效/无效证据监督。我们进一步引入FakeFrontier，一个由真实图像和10个近期生成器输出构建的分布外基准，并配套一个基于MLLM的协议用于评估证据质量和说服力。在GroundFake、FakeFrontier及其他分布外基准上的实验表明，Defake-o3同时提升了检测准确性和解释质量，产生更局部化、可验证且更具说服力的证据。
...
### [6] [GaussianDWM++: Language-Grounded 3D Gaussian Driving World Model for Unified Scene Understanding, Editing, and Multi-Modal Generation](https://arxiv.org/abs/2608.16234v1)
GaussianDWM++：语言锚定的3D高斯驾驶世界模型，用于统一场景理解、编辑与多模态生成

*Tianchen Deng,Xuefeng Chen,Shuang Wu,Qu Chen,Jiajun Zhu,Bo Dai,Jianfei Yang,Hesheng Wang*

Main category: cs.CV

中文摘要：驾驶世界模型（DWMs）近来随着生成模型的发展而迅速进步，但现有的大多数方法主要侧重于条件场景生成，缺乏显式的3D场景理解、语言锚定推理以及可控的4D编辑能力。此外，常用的点云、占据栅格或BEV表示难以实现文本信息与底层3D场景结构之间的细粒度对齐。为解决这些局限性，我们提出了一种基于基础特征的高斯驾驶世界模型，在单一框架内统一了场景理解、语言锚定推理、可控4D编辑和多模态生成。具体而言，我们引入了一种基础特征高斯分词器，直接将近端Qwen/SigLIP视觉-语言特征蒸馏到3D高斯基元中，构建了一个紧凑的开放词汇高斯语义场。我们进一步设计了一种几何感知的高斯适配器，将重要性感知的层次选择与基于文本条件的Perceiver式交叉注意力相结合，将稠密的高斯基元聚合为紧凑的世界令牌。为提高表示兼容性，我们引入了基于KL的高斯-图像分布对齐目标，将高斯世界令牌与基础图像令牌对齐。基于对齐后的高斯表示，我们的框架进一步支持指令可控的场景编辑，包括天气条件生成和动态车辆操控。在更广泛的驾驶基准上的大量实验表明，我们的方法在场景理解、视觉锚定、规划导向推理和可控4D生成任务上均达到了最先进的性能。我们将公开发布代码和数据集于Github上。
...
### [7] [KeyID: Decoupled Drafting and Keyframe Editing for Identity-Preserving Video Generation](https://arxiv.org/abs/2608.16154v1)
KeyID：解耦草稿生成与关键帧编辑的身份保持视频生成

*Jianjie Luo,Yiming Zhong,Haoming Shen,Yupeng Xiao,Zhenguo Yang*

Main category: cs.CV

中文摘要：身份保持视频生成（IPVG）要求合成既忠实于参考主体又忠实于文本提示的视频。现有方法往往受制于高昂的调优成本或有限的输入级增强，难以在复杂、长序列动作中维持严格的身份一致性。为解决这些局限，我们提出了KeyID，一种免训练的身份保持视频生成框架，将视频动态的合成与身份的注入解耦。具体而言，KeyID包含两个组成部分：（1）参考感知视频生成，该模块生成与多个参考对齐的身份无关视频草稿；（2）身份保持关键帧编辑，该模块通过稀疏关键帧校正及随后的运动插值来整合目标身份。通过从密集帧级监督转向稀疏关键帧级细化，KeyID有效解决了提示遵循与身份保真之间的容量冲突。至关重要的是，我们的模块化设计允许无缝扩展到多主体参考和复杂序列动作生成，而无需额外训练。KeyID优于先前工作，并在官方挑战基准上通过了自动化和人工评估验证，最终在ACM多媒体2026 IPVG大挑战赛的第二赛道（序列动作）中获得亚军。源代码可在https://github.com/WISLab-GDUT/KeyID获取。
...
FlowDance：基于并行姿态流与RGB流的音乐驱动舞蹈视频生成

*Genying Li,Boda Lin,Jiachen Li,Zijian Jia,Haojie Zheng,Yiming Wang,Shuchen Weng,Si Li*

Main category: cs.CV

中文摘要：音乐驱动的舞蹈视频合成旨在根据给定的音乐片段使参考人物动起来。该任务具有挑战性，因为它要求模型同时学习音乐到动作的对应关系、保持身份的人物动画、时间一致性以及视觉上逼真的视频生成。我们提出了FlowDance，一种音乐驱动的舞蹈视频生成框架，通过并行姿态流和RGB流将显式运动建模与参考保持的视觉合成相结合。我们进一步引入了时间步感知的姿态注入，以在去噪步骤间调整结构引导，以及持续的身份注入，以在长视频中保持参考外观。为支持该任务，我们还构建了一个经流行度筛选的高分辨率野外舞蹈视频数据集，包含同步音乐、RGB视频、3D人体运动、相机参数和投影2D姿态标注。大量实验表明，FlowDance在舞蹈动作生成和音乐驱动的舞蹈视频合成方面均取得了强劲的性能。
...
### [9] [ChainSpace: A Chained-Reasoning Paradigm for Spatial Intelligence](https://arxiv.org/abs/2608.15788v1)
...
中文摘要：空间智能要求基础模型在与物理世界交互时保持连贯的空间状态。然而，现有的以数据为中心的方法通常将空间推理视为独立的问答实例，这使得模型能够通过捷径进行作答，并为持续的空间理解提供的监督十分有限。为解决这一问题，我们提出了ChainSpace，一种链式推理范式，将空间推理构建为保持状态的多轮过程。在该范式中，空间问题被组织成逻辑上受约束且彼此一致一致的链，后续问题依赖于前几轮中建立的空间约束。遵循这一原则，我们构建了ChainSpace-Bench，一个手工标注的真实世界多轮基准，并配以链感知指标（Chain-Aware Metric），以及ChainSpace-Pipeline，一个基于模拟器的链式结构监督生成框架，用于空间智能训练。实验表明，ChainSpace-Bench能够揭示孤立的问答准确率所无法捕获的链级失败。此外，仅凭借相对少量的模拟器生成的链式数据，由ChainSpace-Pipeline训练的模型在ChainSpace-Bench上取得了开源模型中的最佳性能，并在多个外部空间智能基准上具有竞争力的迁移表现。这些结果确立了ChainSpace作为一种有效的范式，能够实现更忠实的评估和更具数据效率的空间智能学习。
...
### [10] [RigidBench: Evaluating Rigid-Body Physics in Video Generation Models](https://arxiv.org/abs/2608.15555v1)
...
中文摘要：视频模型日益被用于预测场景中接下来会发生什么，然而常用于比较其输出的指标在预测物体是否运动正确方面几乎没有说明力。运动、几何、身份、背景稳定性和视觉相似性可能各自独立地失效，但整帧评分往往将这些误差混在一起。我们提出了RigidBench，一个基于模拟器的基准，将生成的延续与来自相同初始帧和运动描述的参考推演进行比较。其五项刚体任务在物体、材料、视角以及室内和室外场景上各不相同，并提供逐帧掩码、深度、6自由度轨迹和接触信息用于评分。我们使用十项测量在相同的100个示例上评估了八个模型，这些测量将这些方面分开考量。由此产生的排名强烈依赖于测量内容：没有模型在所有十项上均领先，并且在模型均值上，更高的SSIM伴随着更大的3D轨迹误差（r = 0.89）。RigidBench还包括5,000个具有精确模拟器状态的训练视频，我们使用这些视频对Wan 2.2 TI2V-5B进行微调与分析。完全微调将3D轨迹误差降低了约20%，而SSIM几乎没有变化，同时教师强制探测和定向干预表明，物体位置在整个Wan扩散Transformer中都有表示，并被其去噪计算所使用。
...
### [11] [Efficient Audio-Visual Generation via Synchrony-Aware Cross-Modal Sparse Attention](https://arxiv.org/abs/2608.15522v1)
通过同步感知的跨模态稀疏注意力实现高效音视频生成

*Shengchuan Gao,Teng Hu,Bohao Feng,Luchen Li,Wenqiang Wang,Hongqian Deng,Ran Yi*

Main category: cs.CV

中文摘要：近年来的音视频生成模型可以在统一的扩散过程中合成同步的视频和声音，但由于长视频令牌序列需要在去噪步骤中反复进行注意力计算，其推理成本仍然很高。针对视频生成模型，已有多种加速技术被开发出来，包括低位量化、注意力稀疏化和特征缓存等。然而，由于这些方法最初是为视频生成而设计的，直接将其应用于音视频模型会忽略音频和视频分支之间的交互，从而可能破坏音视频同步。我们提出了一种面向高效音视频生成的同步感知加速框架。我们的关键观察是，双向音频-视频交叉注意力揭示了两个分支之间的结构化交互，高响应通常集中在少数与声音相关的视觉区域和时间区域。在该交互模式的指导下，我们引入了一种受保护的稀疏注意力策略，该策略对同步关键的令牌保留高保真度计算，同时对冗余的注意力交互进行稀疏化。通过在加速过程中显式考虑跨模态依赖性，我们的方法在提升推理效率的同时，保持了视频质量、音频质量和音视频同步。
...
### [12] [Geometry-Aware Spatio-Temporal Context Modeling for 4D Occupancy Forecasting](https://arxiv.org/abs/2608.15279v1)
...
中文摘要：4D占用预测对3D场景的时空演化进行建模，对于自动驾驶至关重要，尤其是在边缘案例模拟中。现有方法通常依赖于离散分词化后接自回归预测，但在静态结构的几何形变以及预测时间跨度上的时间一致性方面存在不足。在本工作中，我们提出了一种基于渐进式显式-隐式生成和双路径时空建模的几何感知时空上下文建模方法（GAST），用于4D占用预测。具体而言，生成模块通过姿态驱动的扭曲、运动感知特征调制和基于注意力的特征细化，逐帧生成具有高几何保真度和语义合理性的占用结果。随后，时空模块通过全局上下文聚合增强空间一致性，同时通过时间动态提取捕捉场景演化。这一统一设计使得历史重建和未来预测能够以端到端方式联合优化。在Occ3D-nuScenes上的大量实验证明了我们方法的优越性，在mIoU上超过最先进方法7.67%，在IoU上超过6.44%，同时实现了2.84倍的加速，并在长期预测中保持了强劲的性能。
...
### [13] [Do Visual Grounding Decoders Need Feed-Forward Networks? A Controlled Study over Frozen Vision-Language Features](https://arxiv.org/abs/2608.15061v1)
视觉定位解码器需要前馈网络吗？基于冻结视觉-语言特征的控制研究

*Tarun Tomar*

Main category: cs.CV

中文摘要：一旦预训练的视觉-语言模型已编码图像和语言上下文，视觉定位解码器中的前馈网络（FFN）是否仍会增加必不可少的计算量？我们在冻结的VLM特征上比较了一个四块纯注意力解码器（A4）、一个匹配的四块注意力加FFN解码器（S4）以及一个八块纯注意力参数控制模型（A8）。在RefCOCOg和Ref-Adv-s上，A4与S4相当或略有超出。FineCops-Ref在IoU@0.5上显示A4存在0.52个百分点的轻微劣势（95%置信区间[0.12，0.95]偏向S4），但A8弥补了这一差距并最终比S4高出0.26个百分点。官方FineCops水平并未显示差距呈单调递增。A4将可训练解码器参数减少了44.4%，并将缓存解码器延迟降低了10.1%，尽管端到端延迟仍以主干网络为主。这些结果关乎可训练的定位解码器，而非完整的纯注意力视觉-语言模型。
...
### [14] [Qwen-Video-Edit: Instruction-Based Video Editing by Repurposing an Image Editing Model](https://arxiv.org/abs/2608.14790v1)
...
中文摘要：基于指令的视频编辑通常构建在视频预训练的生成骨干网络之上：视频扩散Transformer需要以相当大的代价进行适配，才能以源视频和编辑指令为条件进行生成。在本报告中，我们探索了一条不同的路径，并表明一个强大的基于指令的图像编辑模型可以直接在视频VAE潜变量上操作来编辑视频。从Qwen-Image-Edit出发，我们将Wan~2.1视频VAE的潜变量帧排列成一张大型虚拟图像的瓦片，对每个瓦片复用编辑器的图像位置编码，并用一对轻量级的输入/输出投影桥接两个潜空间，该投影利用编辑器自身的patchify和unpatchify层进行热启动，从而在初始化时，一个（静态）视频被嵌入的方式与模型已经理解的一张图像完全相同。然后，整个系统在公开的Ditto-1M编辑三元组数据上进行微调，Wan~2.2的少量去噪步骤作为可选的时序增强器。我们用一个零训练观察链来论证该设计：现成的图像编辑器已经可以将以联系表形式呈现的视频进行编辑；它对于表的令牌是来自一次联合编码还是来自在潜空间中拼接的逐帧编码并不敏感；它甚至能以清晰可辨的程度对真实的视频潜变量进行零样本编辑，仅留下微调所需弥合的保真度差距。我们的结果表明，尽管在训练视频潜空间上投入巨大，逐帧视频潜变量仍然与图像域足够接近，使得成熟的图像编辑先验只需极少的适配即可迁移。项目页面：https://yunpeng1998.github.io/Qwen-Video-Edit-Page；代码：https://github.com/yunpeng1998/Qwen-Video-Edit；模型：https://huggingface.co/yunpeng1998/Qwen-Video-Edit。
...
### [15] [PolyComp: A Polycube-based Benchmark for Compositional 3D Spatial Reasoning in Multimodal Models](https://arxiv.org/abs/2608.14741v1)
PolyComp：面向多模态模型中组合式三维空间推理的基于多立方体的基准测试

*Siddharth Patel*

Main category: cs.CV

中文摘要：我们提出了PolyComp，一个程序化生成并经过验证的基准测试，用于考察视觉识别和组合式空间推理能力。在每个问题中，模型必须识别四个选项中哪一个展示了可以组合成目标实体的一对多立方体组件。该基准测试包含来自四个几何家族的120个问题，每个问题有三种不同的呈现格式，使用单张图像或多张图像。随机猜测基线为25%。在三种呈现方式下（每个模型360个呈现问题），使用最大努力模式的GPT-5.6 Sol达到50.0%的准确率（95%问题聚类置信区间43.3-56.7%），每个呈现问题的平均成本为0.951美元；使用最大努力模式的Claude Fable 5达到39.4%的准确率（33.1-46.1%），成本为0.701美元；使用高思考级别的Gemini 3.1 Pro Preview达到27.5%的准确率（22.8-32.5%），接近25%的随机猜测基线，成本为0.350美元。观察到的准确率在不同几何家族之间的差异大于不同呈现格式之间的差异。我们提出了一个问题开发和评估协议、成本和token核算方法，并发布了这120个问题。
...
### [16] [Equilibrium Forcing: Adaptive Video Generation Without Noise Conditioning](https://arxiv.org/abs/2608.14706v1)
均衡强迫：无需噪声条件化的自适应视频生成

*Hansen Jin Lillemark,Alex Rojas,Zachary Novack,Runqian Wang,Yilun Du,Yian Ma,Taylor Berg-Kirkpatrick,Rose Yu*

Main category: cs.CV

中文摘要：基于扩散（Diffusion）和流匹配（Flow Matching）的标准自回归视频生成算法依赖于僵硬的训练目标和静态采样调度，限制了推理过程对数据的适应能力。我们提出了均衡强迫（Equilibrium Forcing, EqF），一种无需噪声水平条件化的简化视频去噪生成模型框架。EqF开创了用于无噪声条件生成的模块化训练与推理时间设计，将学习去噪场与采样过程解耦。这种灵活性使得推理时间算法能够以闭环方式运行，通过适应来自样本的反馈，在具有挑战性的自回归视频生成基准上提升视频质量和一致性。广泛的分析清楚地阐明了去除噪声水平条件化如何使EqF的数据依赖推理特性超越标准噪声水平条件化去噪视频方法的性能。
...
### [17] [ReRef-3D: A Benchmark for Spatial Referring Expression-Guided 3D Scene Rearrangement](https://arxiv.org/abs/2608.16011v1)
...
中文摘要：我们提出了ReRef-3D，一个用于3D场景中语言引导放置的基准。它包含998个CLEVR衍生场景中的33,826条指令，涵盖16种放置类别以及直接、一跳和二跳指代。每条指令必须被解析为一个有效的新放置位置。鉴于一条指令定义的是可接受放置的区域而非单一坐标，我们的评估将预测插入场景中，重新计算关系，并检验关系满足度和物理有效性。每条指令还包括一个经过验证的自然化改写。在微调之后，LLaVA-3D、3D-LLM和PlaceIt3D分别对68.3%、31.6%和22.4%的指令生成了有效的放置。跨模型而言，关系满足度超过了物理有效性；诸如最近和介于之间等关系最为困难；而措辞对性能的影响最小。
...
### [18] [DeepInsight II: One Trace from Benchmark to Robot](https://arxiv.org/abs/2608.16556v1)
DeepInsight II：从基准到机器人的一条轨迹

*Siyi Li,Yuchen Kang,Wuliang Wang,Zhengjie Zhang,Jiangpin Liu,Jianhao Yao,Jie Chen*

Main category: cs.AI

中文摘要：在整个物理AI技术栈中，评估成熟度与部署风险呈反向关系：基础模型拥有成熟、标准化的评测框架，而实际部署所依赖的具身层仍分散在各自为政的、面向特定基准的模拟器、具身形态和接口之中。第一份DeepInsight报告（v1）通过三个抽象概念——任务、资源和结果——统一了整个技术栈的评估，但其定量证据集中在基础模型层；导航与操作（系统1）和全身控制（系统0）仍停留在仿真案例研究层面，物理执行不在其实证范围之内。DeepInsight II保持该底层框架不变，并对具身这一半进行量化。首先，它在两个导航和四个操作基准上，按照其原生协议复现了已发布检查点（checkpoint）的参考结果。其次，MotionBench将四个已发布的全身控制器置于同一工作负载和度量契约之下，然后将一个合格的族内队列从并行仿真推进到匹配的真实机器人试验，其中仿真和物理轨迹共享一个父轨迹标识，同时保留各自执行域特定的记录，使仿真到现实（sim-to-real）的差距成为一种天然的归约，而非跨工具链的对账。第三，一项组合式系统2-1-0研究将轨迹定位扩展到五个基于证据的交接标签，每个标签对应一个具体的修复动作，并带有可测量的可修复性判据，以及在同一属性归因下在硬件可观测状态下进行测试的物理片段。因此，本文的贡献并非新的评估架构，而是从基准执行到匹配的机器人证据和面向修复的诊断之间的实证连续性。
...
### [19] [DriveCache: Action-Aware Caching for Driving World Model Inference](https://arxiv.org/abs/2608.16354v1)
...
中文摘要：驾驶视频生成模型通过预测可控的未来场景来支持自动驾驶开发，用于仿真、规划评估和离线数据生成。基于扩散的驾驶生成器在去噪步骤中反复评估大型骨干网络，这限制了生成吞吐量。现有的扩散加速方法降低了这一成本，但通用设计忽略了生成前可用的驾驶信号，如自车速度和规划轨迹。跨驾驶运动的实验表明，缓存容忍度随自车平移和旋转、去噪进度以及连续复用长度而变化。我们提出DriveCache，一种无需训练、动作感知的控制器，利用规划运动在场景间分配复用，并通过动态规划在标定的响应预算下将其分配到去噪步骤中。因果漂移检查在生成偏离标定时刷新特征并重新规划剩余调度。在三种生成器配置下，DriveCache相较于所评估的缓存方法改善了整体保真度-效率权衡。我们的代码将公开提供。
...
### [20] [Toward AI-Friendly Cartography: Understanding How Color Design Influences Foundation Model Spatial Reasoning on Sequential Choropleth Maps](https://arxiv.org/abs/2608.15736v1)
迈向人工智能友好的制图学：理解颜色设计如何影响基础模型在顺序分级统计图上的空间推理

*Yonghe Sun,Zhenjia Liu,Hua Liao,Wenjia Xu,Nai Yang,Weihua Dong,Zhiwei Wei*

Main category: cs.AI

中文摘要：基础模型（FMs）日益支持多模态和地理空间推理，然而，为人类感知设计的制图原则是否同样适用于机器仍不清楚。聚焦于顺序分级统计图，我们考察了色调调色板、颜色排序和明度对比如何影响基础模型的空间推理。我们构建了一个受控基准，包含5,760幅地图和28,800个问题，涵盖属性识别、空间识别、比较、排序和模式界定，并评估了21个开源和专有多模态基础模型。结果表明，色调选择的影响有限且不一致，而打乱顺序颜色排序则会显著降低性能，尤其是在比较和排序任务中。降低明度对比同样持续损害推理能力，而将对比度提高到超过充分可分离性之上仅带来边际收益。LoRA微调提高了整体准确性，但保留了这些相对敏感性。额外的析因实验进一步表明，错误源于颜色与图例解码、空间推理，以及主题属性与空间结构的整合。这些发现表明，常规顺序排序和充分对比对于机器地图理解仍然重要，并为人工智能友好的制图设计提供了经验性指导。
...
### [21] [Physiological World Models for Human State Transitions](https://arxiv.org/abs/2608.15309v1)
人体状态转变的生理世界模型
...
中文摘要：连续多模态感知现在使得人体生理状态可以在日常生活中被持续观测，而不仅仅是在偶尔的临床就诊期间。然而，大多数健康人工智能系统被设计用于识别当前状态、评估风险或分析单个生物标志物。它们并不直接建模生理状态如何响应真实世界事件、行为、情境和干预措施而发生变化。在此，我们提出生理世界模型（Physiological World Model，PWM），一个以事件为条件的框架，用于在整个人体的层面上学习这些变化。我们引入人体状态转变令牌（HumanState Transition Token），一种结构化的、带质量评分的单元，它连接事件发生前的生理状态与事件或动作、相关情境和干预信息、事件发生后的生理轨迹、观察到的结果以及数据质量。我们描述了从状态表征到有界干预规划的四个能力层级，以及四个数据采集和验证协议。我们还提出了六项基准任务，涵盖人体状态（HumanState）表征、跨多个时间尺度的预测、个体化反应预测、替代干预的模拟、有界规划以及分布偏移下的可靠性。总体而言，该框架为个性化健康管理、行为干预设计和临床医生监督的决策支持提供了一条实用路径，同时明确区分预测与因果推断，并明确阐明不确定性、安全性、治理和使用限制。
...
### [22] [Constitutive Priors for Machine Intelligence: A Legitimacy Theory of the Artificial Physical World](https://arxiv.org/abs/2608.15147v1)
...
中文摘要：机器智能已征服了符号世界，却在物理世界停滞不前。这种停滞是结构性的：物理人工智能面临冷启动僵局——没有数据就没有智能，没有已部署的智能就没有数据。我们的论断是：这一僵局是真实存在的，但其分布并不均匀，而例外有一个名字：人工物理世界。建筑物、工业设施和基础设施是有意构成并有文档记录的：设计的产物在交付时附带可读的档案，这些档案先于其实例并构成其实例；在此，规范先于实例被颁布，而非从实例中平均得出。四项贡献。（i）从四世界本体论出发，我们推导出构成性先验框架的合法性标准：当且仅当对象域是有意构成的并且留下了可读的档案时，先验提取才是合法的；该标准可通过适切方向（direction of fit）加以检验——偏离构成性规范是世界中的违规，而非对模型的修正。（ii）我们确立了分层下界：任何此类框架至少具有四层——语法、概念、知识、实例——因为四种建构目标配对为互不相容的载体。（iii）我们在五个工业领域和一个32类失效模式词汇表上登记了部署主张。（iv）我们将该框架押注于五项可证伪的预测，其中核心一项可在公开工程记录上检验：若其失败，则框架失败。半形式化论证支撑了这些主张（附录A）：无档案世界中规则覆盖的Gold型边界，封闭概念层上失效归约的可判定性结果，以及证书锚定演算的边界定理。大型语言模型在此找到了受尊重的地位——作为档案的读者，而非档案本身。三篇配套作品中的第一篇；配套作品将探讨有意搁置的问题。
...
### [23] [SCOPE: Score-Isolated Agentic Optimization for Video World Models](https://arxiv.org/abs/2608.15043v1)
SCOPE：面向视频世界模型的分数隔离智能体优化

*Yuhua Jiang,Jiaming Wang,Qingbin Liu,Feifei Gao*

Main category: cs.AI

中文摘要：视频世界模型越来越多地被用作规划和具身决策的模拟器，然而在推理时对其改进却引入了一个微妙的评估问题：提示词、采样器、验证器和选择器可能共同演化，使得难以归因收益或防止保留反馈塑造最终策略。我们提出\scope（\emph{\scopefullname}），一个用于冻结视频世界模型的可审计推理时自适应的框架。\scope将外部控制表示为类型化状态，仅通过有开发证据支持的有限变更来更新该状态，并在保留评估之前冻结生成的策略。在Physics-IQ基准上，\scope相较于完全冻结的基线提升了$+14.24$（95\%置信区间$[+8.10,+21.23]$）。受控消融实验进一步识别出场景规范、采样和学习选择带来的收益，而与最强匹配智能体基线的差距仍未解决。跨主干和前瞻性评估揭示了一个互补的结果：有用的推理时更新确实存在，但其收益并不能均匀地迁移到所有模型和设置中。综合来看，这些发现表明，可靠的推理时自适应不仅需要更好的提案，还需要一个原则性的机制来决定哪些更新应成为部署系统的一部分。代码可在https://github.com/YuhuaJiang2002/SCOPE获取。
...
### [24] [Generated Context versus Governed State: Functional Conditions for Accountable Longitudinal Clinical Reasoning](https://arxiv.org/abs/2608.14804v1)
生成语境对治理状态：可问责的纵向临床推理的功能条件

*Augusto Bernardo Pissarra,Victor Lorena de Farias Souza*

Main category: cs.AI

中文摘要：大语言模型（LLMs）已成为临床人工智能的主导界面，然而它们所暴露的界面（文本进、文本出、一次一个上下文窗口）并未维持对患者当前真实情况的显式、持久且受治理的表示。本文论证，纵向临床推理是在部分可观测条件下的状态估计问题，临床人工智能成功或失败的关键轴心并非模型阅读病历的流畅度，而是其所推理的患者状态的治理程度。我们区分了生成语境与治理状态；分离了临床人工智能习惯性混淆的五种对象（真实状态、观测、证据、信念和模拟状态）；定义了一个分层治理标准，任何临床人工智能系统都可以据此接受审计；并表明可问责性的操作性定义可分解为四项信息要求：具有感知时间版本化的不可变证据账本、与累积证据相区分的信念状态、观测过程模型，以及主张层面的因果类型标注。我们明确指出，这一分解是分析性的而非必然性定理，其价值在于概念卫生：它将“可问责的临床人工智能”从口号转化为审计工具。一个六级成熟度框架将系统可治理的内容与其可计算的内容区分开来，将当前以LLM为中心的实践定位为高能力但低成熟度。本文完全自成一体：框架提出的四个研究问题在引言中陈述，结论记录了本文针对每个问题所确立的内容；未来工作将发展架构的可构建核心以及通往完整临床世界模型的研究计划。本文未声称任何实证结果。
...
### [25] [AnyTalk: Speech Animation for Arbitrary Characters Leveraging a Video Generation Model](https://arxiv.org/abs/2608.16143v1)
AnyTalk：利用视频生成模型实现任意角色语音动画

*Kwan Yun,Serin Yoon,Sunjin Jung,Jung Eun Yoo,Inyup Lee,Junyong Noh*

Main category: cs.GR

中文摘要：我们提出AnyTalk，一种为任意角色生成3D语音动画的新方法，无需任何动画数据。现有的音频驱动的3D语音动画方法依赖于特定角色的训练数据或费力的骨骼绑定/网格重建，而AnyTalk通过利用在大规模视频数据集上训练的视频扩散模型来规避这些限制。我们首先通过我们的角色特定微调（CsF）技术，将预训练的视频扩散模型适配到目标角色。通过在3D角色的渲染图像与置零的音频嵌入（表示"无运动"）上微调，我们在保留大规模视频扩散模型的运动先验的同时，消除了对动画数据的需求。然后，我们通过所提出的优化过程估计混合变形参数，将生成的说话头视频提升为3D语音动画。AnyTalk能够跨多种面部网格和混合变形配置实现唇形同步动画，显著减少了人工工作和数据需求。我们进一步通过将AnyTalk蒸馏为一个精简网络AnyTalkRT来增强其可用性，从而实现实时性能。通过利用说话头视频生成，我们的方法拓宽了任意角色获得音频驱动语音动画技术的途径。代码公开于 https://serin-yoon.github.io/projects/anytalk/。
...
### [26] [SingDance: Compositional Zero-Shot Singing-and-Dancing Video Generation with Role-Aware Audio Conditioning](https://arxiv.org/abs/2608.16220v1)
SingDance：角色感知音频条件下的组合式零样本歌唱与舞蹈视频生成

*Tao Feng,Xu Li,Xiangyang Luo,Ming Wen,Huadai Liu,Chen Zhang,Wei Xue*

Main category: cs.SD

中文摘要：从参考图像、文本提示和音频轨道生成个性化舞蹈视频，需要音乐条件下的身体动作。歌唱与舞蹈增加了第二项要求：可见主体还必须表达歌声。现有的音乐条件方法主要关注编舞，而语音驱动模型通常假设可见主体产生输入的语音，这使得这一组合场景在很大程度上未被探索。我们提出SingDance，一个统一的视频扩散框架，将可控的语音发声表述为一种语义角色：可见主体要么是发声者（source），即产生语音信号的个体，要么是聆听者（listener），即从画外表演者那里接收语音的个体。硬紧凑路由选择与任务相关的语音、音乐和角色条件，通过逐帧联合音频注入进行组合；发声者和聆听者保留相同的语音通路。训练使用不对称监督：画面内的说话视频和精心策划的画外对话回应视频建立角色控制，而器乐和基于歌曲的纯舞蹈视频建立音乐条件下的身体动作。目标歌曲/发声者配置在训练期间从未出现。在推理时，为歌曲分配发声者角色会将分别学习的发声能力和歌曲条件舞蹈能力组合起来，实现组合式零样本歌唱与舞蹈。实验证明了强大的动作-节拍对齐和视觉保真度、在保持音乐对齐身体动作的同时可靠地配对切换语音发声，以及与评估中最强的语音驱动基线相比，以显著更少的生成时参数实现了极具竞争力的唇形同步。
...
### [27] [Adding Voice Cloning to Text-to-Audio-Video Models with a Single Zero-Initialised Layer](https://arxiv.org/abs/2608.15690v1)
通过单个零初始化层为文本到音视频模型添加声音克隆能力

*Ivan Mikheev,Viacheslav Vasilev,Anna Dmitrienko,Alexey Letunovskiy,Ivan Kirillov,Kirill Chernyshev,Denis Dimitrov*

Main category: cs.SD

中文摘要：文本到音视频（T2AV）生成模型能够根据文本描述生成视频及其配乐，但无法控制输出中由谁的声音来发言。我们证明，通过在基础T2AV模型的音频骨干网络之上添加一个单一的零初始化线性层、进行相对较短时间表的微调，并在推理时以一段简短的参考录音作为条件，即可将基础T2AV模型转变为声音克隆模型。参考信息通过两个互补的信号注入：其扩散潜变量被前置到音频流中，同时一个全局说话人嵌入对目标音频的令牌进行调制。在涵盖30位说话人的674个说话人-文本配对基准上，我们与五个强基线声音克隆文本到语音系统进行了对比：我们增强后的5B模型在三个独立的验证网络（ECAPA-TDNN、WavLM-SV、Resemblyzer）上取得了最高的说话人编码器余弦相似度（SECS），并在统计上显著优于所有基线。该架构的一个副产物是，音频通路可以在推理时脱离视频通路独立评估，相对完整的音视频扩散循环实现了约30倍的加速，同时保持了声音克隆行为。
...
### [28] [$τ_0$-VLA: a Hierarchical Robot Foundation Model with World-Model-Guided Test-Time Computation](https://arxiv.org/abs/2608.16885v1)
$τ_0$-VLA：一种具有世界模型引导测试时计算的分层机器人基础模型
...
中文摘要：长时程机器人操作要求机器人既能可靠地执行单个技能，又能在扩展任务中连贯地将它们排序组合。大多数分层视觉-语言-动作（VLA）模型通过单次前向传播做出每个这样的决策，没有机制为困难或关键的选择分配额外的计算量。我们提出了$τ_0$-VLA，一种分层机器人基础模型，它将高层子任务生成表述为一个可通过世界模型引导的测试时计算进行可扩展推理的问题。在每个推理步骤中，高层策略使用执行记忆生成一个子任务，并在需要时在提交输出之前对备选方案进行搜索。随后，低层策略在多种机器人形态上执行所生成的子任务。该策略在40,115小时的异构真实世界数据上通过多模态协同训练进行训练。在域内和分布偏移设置中，分配额外的测试时计算量显著提高了下一子任务预测的准确率，并且这些收益转化为在长时程机器人操作任务上更高的闭环成功率。
...
### [29] [Neurosymbolic Embodied Agents](https://arxiv.org/abs/2608.16794v1)
神经符号具身智能体
...
中文摘要：语言模型和视觉-语言模型能够生成看似合理的具身计划，但并不能保证其可执行性，因为其输出可能违反环境动力学或作用于错误接地（grounding）的实体。我们提出了一种神经符号智能体，它将长时程家庭任务分解为面向任务的视觉探索和受限符号规划。在第一阶段，视觉-语言模型和探索框架从自我中心（egocentric）观测和接地交互中获取与目标相关的谓词和实例绑定，从而产生符号初始状态。在第二阶段，一个PDDL转移模型将解码限制为仅生成可扩展适用动作的令牌。随后，蒙特卡洛树搜索使用领域无关的规划启发式来评估可执行的延续动作。由此产生的计划在转移模型下按构造即是可执行的，其对环境的迁移取决于正确的视觉接地。在VirtualHome和ALFWorld上，开放的4B-27B模型在两个环境中均超过90%的成功率，而我们最小的智能体在每个环境中的表现都大幅优于27B的直接视觉策略。约束和搜索被证明是互补而非可互换的：在ALFWorld中，单独使用两者任一方法只能解决不到三分之一的任务，而两者结合可解决超过95%的任务。该方法使用的生成令牌数也比扩展推理（extended thinking）少数倍，模型可见的图像数远少于直接交互，且残留失败被定位到状态获取环节而非计划生成环节，全程无需任何专门训练。
...
### [30] [Orbit-Planner: Towards Latent World Models for On-Orbit Obstacle Avoidance of Satellite Agents](https://arxiv.org/abs/2608.16651v1)
Orbit-Planner：面向卫星智能体在轨避障的潜在世界模型

*Zhijian Li,Chao Ren,Peijin Wang,Xian Sun*

Main category: cs.RO

中文摘要：用于在轨导航任务的卫星智能体需要利用有限的星上观测来预测碰撞风险。然而，传统规划器通常依赖预定义地图和固定的环境假设，限制了其在动态在轨场景中的适应性。在本文中，我们提出Orbit-Planner，一种用于在轨避障的两阶段潜在世界模型。Orbit-Planner学习以动作为条件的航天器动力学，以在潜在空间中进行未来状态推演，并引入物理探测器（Physics Probe）从想象出的潜在轨迹中解码物理状态变化。实验表明，Orbit-Planner能够进行长时程潜在推演，并从想象轨迹中恢复物理状态。在Isaac Sim中的闭环避障导航中，其成功率达到了91.7%。代码可在https://github.com/ZhijianLi2003/Orbit_Planner获取。
...
### [31] [HiPHI: A Large-Scale Benchmark for High-Precision Human Motion and Object-Interaction](https://arxiv.org/abs/2608.16222v1)
HiPHI：面向高精度人体运动与物体交互的大规模基准数据集
...
中文摘要：人形智能需要对极其多样化的全身运动空间和基于物理的交互进行学习。然而，现有的具身数据集从根本上仍存在局限：互联网规模的视频数据缺乏精确的物理状态和交互基础，而实验室运动数据集虽然具有高保真度，但行为覆盖范围狭窄。这种不匹配为可扩展的人形策略学习造成了关键瓶颈。我们提出了HiPHI，一个600小时以上规模的高保真全身人体运动数据集，旨在系统性地最大化人体运动与交互流形（manifold）的覆盖范围。HiPHI以FrameNet为理论指导——这是一个组织人类原始动作（primitives）的语言学框架。HiPHI采用光学动作捕捉流水线构建，为全身人体运动提供亚毫米级空间标记点跟踪精度，并提供网格级物体轨迹。我们进一步引入了一套基准测试套件，用于评估运动空间多样性、交互基础、物体一致性以及物理AI应用。我们的分析表明，与现有运动数据集相比，HiPHI显著扩展了运动覆盖范围，同时保持了高保真的交互质量，并为在真实世界具身任务中训练、评估和泛化人形策略建立了可扩展的数据基础，类似的扩展同样适用于计算机图形学中的运动先验模型。
...
### [32] [Low-Rank Dynamics-Effective Latent Carriers for Counterfactual Rollout in Learned World Models](https://arxiv.org/abs/2608.15156v1)
低秩动力学有效潜在载体：用于学习世界模型中的反事实推演

*Yang Liu,Yuming Chen*

Main category: cs.RO

中文摘要：世界模型可以预测未来，但并未明确其隐藏状态中的哪些部分实际驱动了这些预测。我们探究一个小的、可直接寻址的隐藏状态变化是否能使学习到的世界模型置于预期的反事实轨迹上，然后让模型自行延续该未来。我们在一个受控的双物体、二维碰撞环境中研究了一个具有192维隐藏状态的循环世界模型。对于一个有界的局部速度编辑族，我们首先验证模型能够原生地表示并展开编辑后的未来。然后，我们从仅基于训练数据的事实到反事实的隐藏差分中构造候选低秩载体，并学习一个从事实状态和请求编辑到载体系数的映射。在已登记的秩网格上，秩4是满足完整开发面板标准的最小测试秩。在锚点处对秩4补丁进行单次修补足以重定向12步自主推演，无需未来观测、教师强制或重复校正。冻结过程满足预先登记的跨独立训练检查点的复制规则，并且在相邻干预时间下仍然可用。随机等范数、错误物体和错误时间的对照不能解释该效应。位置编辑压力测试提供了一个负面对照：预期的位置补丁可以通过原始推演标准，但无补丁和随机对照也可以通过相同的标准，并且错误物体的特异性未被确立。因此，仅成功的编辑是不够的。我们用“动力学有效”来描述一种干预，它在自主推演下以持续且目标特异的方式改变模型的未来计算。秩4结果识别了针对所测试的速度编辑族的一个紧凑干预接口，而非一个封闭的四维状态或内在状态维度。
...
### [33] [MotionGS-SLAM: Event-Modulated Gaussian Splatting for Motion-Blur Robust SLAM](https://arxiv.org/abs/2608.15024v1)
MotionGS-SLAM：事件调制的高斯溅射实现运动模糊鲁棒SLAM

*Zhiqiang Hu,Shouren Huang,Masatoshi Ishikawa*

Main category: cs.RO

中文摘要：当前的基于视觉的SLAM系统在运动模糊破坏视觉输入时会灾难性地失效，因为它们试图解决从退化观测中恢复清晰内容这一不适定逆问题。我们提出了MotionGS-SLAM，它通过范式转变从根本上重新构想运动模糊处理：与其去除模糊伪影，我们将该挑战重新表述为在渲染管线中生成式建模模糊形成的良好约束前向问题。通过利用事件相机微秒级的时间分辨率和抗运动模糊的特性，我们引入了一种新颖的事件调制高斯核，该核根据精确的运动线索动态调整每个高斯的栅格化。我们的双调制机制将二维高斯投影从各向同性的点状变换为各向异性、运动对齐的椭圆笔触（空间调制），同时根据局部速度自适应地变化曝光积分采样密度（时间调制）。这种基于物理的方法通过模糊感知的光度约束和事件约束，实现了曝光内相机轨迹和三维场景几何的联合优化。大量实验表明，在剧烈高运动条件下，该方法在轨迹精度和地图质量方面相比最先进的方法有显著提升。
...
### [34] [Evidence of Absence: Cross-Modal Abductive Risk Perception to Sustain World Models When Vision Fails](https://arxiv.org/abs/2608.14952v1)
缺席的证据：视觉失效时通过跨模态溯因风险感知维持世界模型

*Cong Xu,Ravi Sankar*

Main category: cs.RO

中文摘要：一个结构化的世界状态（实体、关系、上下文和预测线索）被设计用于在感知退化时保留对预测至关重要的内容，但它预设了观测数据来填充它；当主要视觉模态被遮挡或退化时，这些观测数据可能缺失。我们探讨如何通过将预期共同证据的缺席视为隐藏原因的证据，从互补模态维持世界模型。该溯因框架与模态无关；本文以声学方式实例化它。一个麦克风阵列前端估计发动机和轮胎声源的方位，并提取接近速率证据（当存在稳定音调时为多普勒效应，否则为宽带接近读数）；"签名存在、视觉共同证据缺席"这一事件随后触发对隐藏道路使用者的溯因推理，发出校准的风险提示而非控制指令。隐藏状态的可恢复性被作为可辨识性问题进行分析，将共享信息与模态特有信息区分开来，线索提示被形式化为在明确的虚警预算下的Neyman-Pearson检测。在真实遮挡接近记录中，该方法在盲交叉口平均在视线进入前1.7秒发出警告，以比已发表声学基线少42%的虚警达到其持续窗口变体的检测率，一旦进入视野后中位定位误差为3.4度，校准良好（预期校准误差为0.034），并在将仅视觉通道降至0.03的分阶段视觉退化下保持危险感知高于0.87。我们还测量了该方法的局限性：校准几乎无损地迁移到未见过的交叉口，但签名分类器不能，且移动载体自身噪声是部署的约束瓶颈。
...
### [35] [FloodReasonBench: Benchmarking VLM Reasoning Segmentation for Embodied Flood Response at the Edge](https://arxiv.org/abs/2608.15410v1)
FloodReasonBench：面向边缘端具身洪水响应的VLM推理分割基准测试

*Rajat Bhattacharjya,Yoomee Jung,Minwoo Kim,Sing-Yao Wu,Eli Bozorgzadeh,Nalini Venkatasubramanian,Nikil Dutt*

Main category: cs.DC

中文摘要：推理分割使视觉语言模型（VLM）能够将任务相关的语言请求转化为像素级视觉定位，为具身智能体提供了一种自然的感知接口。然而，现有基准测试主要聚焦于通用视觉场景，忽视了洪水响应平台所面临的领域与资源约束。我们提出了FloodReasonBench，一个面向边缘端具身洪水响应的VLM推理分割基准测试。其核心是FloodResponseSeg，一个基于真实场景和响应相关目标构建的洪水专项推理分割数据集。除任务准确率外，该基准测试还刻画了在轻量级视觉编码、分层分割推理和压缩中间表示下的推理分割流水线。我们观察到，在通用的预适配设置中，存在显著的依赖分区的准确率差异，而洪水适配的目标工作负载设计空间在各分区之间表现出明显更紧凑的准确率范围。在NVIDIA Jetson AGX Xavier上的评估进一步揭示了推理分割准确率、边缘端延迟、能耗和通信开销之间的权衡，从而实现了对边缘工作点的质量约束选择。综合来看，这些结果为面向资源受限的边缘端具身洪水响应的推理分割提供了任务级和系统级的刻画。
...
### [36] [CaliBench: Are the Stochastic Dynamics of Video World Models Physically Calibrated?](https://arxiv.org/abs/2608.16829v1)
...
中文摘要：视频世界模型通过生成式采样来近似物理结果的随机分布，但现有基准要么对单个生成结果评分，要么在整个数据集上粗略地比较分布，导致特定现象的细粒度偶然不确定性（aleatoric uncertainty）未被测试。我们提出了CaliBench，它在一个物理上可解释的离散空间中对结果进行评分——例如箱索引（bin index）、骰子面、花色、颜色——而非像FID那样在学习的特征空间中进行评分，从而直接测量与已知参考分布的距离。我们构建了参考分布以闭合形式已知的结果空间（二项式高尔顿板、伯努利分叉、均匀骰子/扑克牌/彩票、偏斜的欧式轮盘颜色），从而能够进行精确的校准检验。我们将性能分解为单一准确率指标所混淆的两个正交轴：可评分性（scorability），即生成结果中可产生可评分结果的比例；以及校准度（calibration），即该样本上相对于参考分布的总变差距离。卡方检验用于评估显著性；由于校准度是其零假设，该检验只能证明失校准（miscalibration）的存在，并且在每单元N=32时只能检测出较大的偏差。我们将其应用于九个场景和六个图像到视频模型（WAN-2.7、SeeDance-2.0、HappyHorse-1.0、Veo 3.1、Runway Gen-4.5、Cosmos3-Super），每个模型生成32次。模型一致地将概率质量集中在少数几个结果上，而非复现参考分布。大多数场景-模型组合显著失校准，极端情况下坍缩为单一结果，正如Veo 3.1在骰子场景中的表现。在轮盘场景中，生成的画面常常使球的位置模糊不清，导致多个模型的可评分性较低。性能因场景而异：没有模型在全部九个场景中占据主导地位。我们发布了该协议和一个指标（平均归一化总变差，mnTV），用于将新模型与我们的结果进行比较。
...
### [37] [UniTAC: Universal Task-Aware Compression via Weighted Distortion Measures](https://arxiv.org/abs/2608.16696v1)
...
中文摘要：物理AI系统（如自动驾驶车辆和机器人）依赖于在紧张的带宽、延迟和能量预算下及时交换高维感知信号。由于驱动下游决策的任务会随时间演变，任务专用编解码器具有脆弱性，而在现场为每个任务重新训练编解码器是不可行的。我们提出UniTAC，这是一个单一的 learned 图像编解码器，涵盖从通用（任务无关）到任务专用操作的范围，可在运行时重新定向而无需重新训练。任务被抽象为逐分量的重要性向量，例如从任何下游模型的梯度归因中导出，并作为低开销的辅助信息进行传输，同时调节编码器和解码器。UniTAC在广泛的、随机的此类向量族上针对加权重建失真仅训练一次，保持固定的主干网络和单一的可供人类查看的重建，其保真度通过交换注入的向量而导向当前活动任务。我们分析了潜在的加权率失真问题，刻画了对角加权失真何时是任务一致的，以及权重与任务敏感性之间的关系。在此指导下，我们设计了一种Vision Transformer（ViT）编解码器，其词元级条件化原生地实现了这种权重驱动的编码。在0.034 bpp的局部任务上，单一的UniTAC模型达到91.4%的准确率，仅比基于任务的编解码器（93.3%）低1.9%，且高于通用编解码器（76.9%）。
...
### [38] [SCALE: State-Calibrated Latent Embeddings for JEPA Planning in the Right Geometry](https://arxiv.org/abs/2608.16287v1)
SCALE：面向JEPA规划的正确几何中的状态校准潜在嵌入

*Jiaming Hu,Yan Zheng,Tian Wang*

Main category: cs.LG

中文摘要：联合嵌入预测式世界模型通过在表示本身定义的代价函数下，将预测的终态嵌入与目标嵌入进行评分比较来进行规划。获得非坍缩表示的两种主要策略是：继承预训练特征空间（如DINO-WM），以及结合反坍缩正则化端到端学习嵌入（如带有SIGReg的LeWorldModel（LeWM））。这两种策略在不同任务上展现出互补的优势。尽管任务相关的状态可以从两种模型的完整嵌入中解码，但DINO-WM的主成分通常比LeWM保留更多的状态信息。由于欧几里得规划代价由高方差方向主导，这一差异会影响状态对候选选择产生作用的强度。我们提出SCALE（State-CAlibrated Latent Embeddings，状态校准潜在嵌入），以使端到端的LeWM表示具备在DINO-WM中观察到的有利几何特性。SCALE通过将采样的成对潜在距离与标准化任务相关状态空间中的距离相关联来诱导该特性，而无需替换LeWM学习到的编码器。在五个任务、三种规划求解器和五种计算预算下，SCALE在每个任务-求解器组合上均优于LeWM的平均表现。一种潜在到状态的回归对照组在完整嵌入可解码性上匹配甚至超越SCALE，但基本不改变潜在-状态距离对齐，且带来的规划增益一致性较差。SCALE仅增加了一个轻量级的训练时正则化器，且不增加任何规划时开销。这些结果表明，规划不仅取决于任务相关信息是否存在，还取决于它是否塑造了规划器所使用的几何结构。
...
### [39] [Paired Exact-Reset Evaluation of a Prediction-Derived Medium-to-Full World-Model Cascade](https://arxiv.org/abs/2608.14650v1)
预测派生中型到全型世界模型级联的配对精确重置评估

*Malo de Pastor*

Main category: cs.LG

中文摘要：现有的自适应推理和世界动作模型系统使用廉价阶段的输出或预测的未来状态来分配额外的计算。我们研究一个更窄的问题：在配对精确重置的物理结果下，中型（Medium）派生的接口能否预测何时切换到单独冻结的全型（Full）预测器，从而足以改善特定任务的决策损失以证明顺序开销的合理性？我们的贡献是一个配对评估和审计协议，而非一条新的通用路由规则：所有候选动作均从相同的重置状态执行，中型和全型作用于相同的候选集和任务，其配对的物理损失差定义了路由目标。在一个全新的 PushT 数据集上（V106；1,600 个状态，39 个任务，三对检查点），一个冻结的预测接口路由器相对于独立的 Medium、独立的 Full 以及一个具有延迟优势的任务专用路由器，降低了包含开销的决策成本。随后，我们前瞻性地封存第二个 1,600 个状态的 PushT 确认集（V107），并使用任务、当前 DINO 特征的维度匹配投影以及全部五个候选动作（不对 DINO 编码器延迟计费）针对更强的当前状态控制进行测试。该预测接口将计价的物理决策成本降低了 0.002549（状态聚类 95% 区间 [-0.002867, -0.002238]；单侧 95% 上界 -0.002286），三对检查点均呈现负效应。受控的 PyBullet 审计独立支持一个复合的任务-预测-体制路由器。该顺序路由器仍慢于固定策略，其优势仅限于低计算价格场景。证据支持所测预测接口在超出一个人为偏袒的当前 DINO 控制之外具有增量路由信息，但不足以支持因果充分性、计算节省、闭环价值或跨族普适性。
